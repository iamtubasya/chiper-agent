 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.chiperflux/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.chiperflux', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (CHIPER_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.CHIPER_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.chiperflux', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.CHIPER_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.chiperflux', 'document_cache');
const AUDIO_CACHE_DIR = process.env.CHIPER_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.chiperflux', 'audio_cache');
...
// `chiper update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *Chiper Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Chiper Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore Chiper' own reply messages in self-chat mode to avoid loops.
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `chiper_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
