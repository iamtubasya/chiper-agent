#!/usr/bin/env python3
"""Live test harness for Chiper Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any chiper imports.
ORIGINAL_HOME = os.environ.get("CHIPER_HOME")
ORIGINAL_AUTH = Path.home() / ".chiperflux" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.chiperflux/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.chiperflux/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="chiper_ts_live_"))
    chiper_home = home_dir / ".chiperflux"
    chiper_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, chiper_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".chiperflux" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, chiper_home / ".env")
        ...
        load_chiper_dotenv(chiper_home=str(Path.home() / ".chiperflux"))
    ...
    (chiper_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return chiper_home
...
def reset_module_state():
    """Drop cached modules so the new CHIPER_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "chiper_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["CHIPER_HOME"] = str(home)
...
def main():
    ...
    # Restore original CHIPER_HOME
    if ORIGINAL_HOME is not None:
        os.environ["CHIPER_HOME"] = ORIGINAL_HOME
    else:
