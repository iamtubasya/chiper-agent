...
if [ -n "${CHIPER_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$CHIPER_INSTALL_DIR"
...
#   code at /usr/local/lib/chiper-agent, command at /usr/local/bin/chiper,
#   data still at /root/.chiperflux (CHIPER_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.chiperflux/chiper-agent"
            echo "                   default (root, Linux): /usr/local/lib/chiper-agent"
            echo "  --chiper-home PATH  Data directory (default: ~/.chiperflux, or \$CHIPER_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Chiper installs the code under"
            echo "  /usr/local/lib/chiper-agent and links the command into"
            echo "  /usr/local/bin/chiper (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$CHIPER_HOME"
            echo "  (default /root/.chiperflux).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$CHIPER_HOME/chiper-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs chiper setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo -e "${GREEN}${BOLD}"
    echo "┌──────────────────────────────────────────────┐"
    echo -e "│    ${NC}${BOLD}@ChiperFlux${GREEN}${BOLD} |${YELLOW}${BOLD} ⚕ Chiper Agent Installer ${GREEN}${BOLD}   │"
    echo "├──────────────────────────────────────────────┤"
    echo "│  An open source AI agent From Nous Research. │"
    echo "└──────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# Decide where the repo checkout + venv live, and where the `chiper` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $CHIPER_HOME/chiper-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $CHIPER_HOME/chiper-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/chiper-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $CHIPER_HOME/chiper-agent — then preserve it)
#
# Always no-op when the user set --dir or $CHIPER_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in CHIPER_HOME.
    if is_termux; then
        INSTALL_DIR="$CHIPER_HOME/chiper-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$CHIPER_HOME/chiper-agent/.git" ]; then
            INSTALL_DIR="$CHIPER_HOME/chiper-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/chiper-agent)"
            return 0
        fi
        ...
        # /usr/local/bin/chiper wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/chiper"
        log_info "  Data:    $CHIPER_HOME (unchanged)"
...
get_chiper_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/chiper" ]; then
        echo "$link_dir/chiper"
    else
        echo "chiper"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/iamtubasya/chiper-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/chiper-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/chiper-uv-installer.$$.sh")"
...
# module compiled in. Chiper' session store (chiper_state.py) creates FTS5
...
ensure_fts5() {
    ...
    if [ -n "${PYTHON_PATH:-}" ] && _python_has_fts5 "$PYTHON_PATH"; then
        ...
        # etc.). Install proceeds — Chiper degrades gracefully and disables
        # only full-text session search — but warn so it isn't a silent gap.
        log_warn "Could not obtain an FTS5-capable Python. Chiper will run, but"
...
check_node() {
    ...
    if [ -x "$CHIPER_HOME/node/bin/node" ]; then
        export PATH="$CHIPER_HOME/node/bin:$PATH"
        local found_ver=$("$CHIPER_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Chiper-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.chiperflux/node/..."
    ...
    # Place into ~/.chiperflux/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$CHIPER_HOME/node"
    mkdir -p "$CHIPER_HOME"
    mv "$extracted_dir" "$CHIPER_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$CHIPER_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$CHIPER_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$CHIPER_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$CHIPER_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$CHIPER_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.chiperflux/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Chiper install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Chiper Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="chiper-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Chiper behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"chiper-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up chiper command..."

    if [ "$USE_VENV" = true ]; then
        CHIPER_BIN="$INSTALL_DIR/venv/bin/chiper"
    else
        CHIPER_BIN="$(which chiper 2>/dev/null || echo "")"
        if [ -z "$CHIPER_BIN" ]; then
            log_warn "chiper not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$CHIPER_BIN" ]; then
        log_warn "chiper entry point not found at $CHIPER_BIN"
    ...
    # Create a user-facing shim for the chiper command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $CHIPER_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$CHIPER_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/chiper"
    cat > "$command_link_dir/chiper" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$CHIPER_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/chiper"
    log_success "Installed chiper launcher → $command_link_display_dir/chiper"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "chiper command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v chiper' \
                >/dev/null 2>&1; then
        ...
        log_info "chiper not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Chiper Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Chiper Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Chiper Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so chiper works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.chiperflux directory structure (config at top level, code in subdir)
    mkdir -p "$CHIPER_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.chiperflux/.env (top level, easy to find)
    if [ ! -f "$CHIPER_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$CHIPER_HOME/.env"
            log_success "Created ~/.chiperflux/.env from template"
        else
            touch "$CHIPER_HOME/.env"
            log_success "Created ~/.chiperflux/.env"
        fi
    else
        log_info "~/.chiperflux/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$CHIPER_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.chiperflux/config.yaml (top level, easy to find)
    if [ ! -f "$CHIPER_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$CHIPER_HOME/config.yaml"
            log_success "Created ~/.chiperflux/config.yaml from template"
        fi
    else
        log_info "~/.chiperflux/config.yaml already exists, keeping it"
    fi
...
-->
SOUL_EOF
        log_success "Created ~/.chiperflux/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.chiperflux/"

    # Seed bundled skills into ~/.chiperflux/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.chiperflux/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.chiperflux/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$CHIPER_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$CHIPER_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.chiperflux/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$CHIPER_HOME/.env"
    ...
    {
        echo ""
        echo "# Chiper Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Chiper will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (chiper --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'chiper setup' after install."
        return 0
    fi
    ...
    # Run chiper setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m chiper_cli.main setup < /dev/tty
    else
        python -m chiper_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$CHIPER_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Chiper to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$CHIPER_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'chiper whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                CHIPER_CMD="$(get_chiper_command_path)"
                $CHIPER_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'chiper whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'chiper gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $CHIPER_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $CHIPER_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: chiper gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: chiper gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $CHIPER_CMD gateway > "$CHIPER_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.chiperflux/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: chiper gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: chiper gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $CHIPER_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $CHIPER_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $CHIPER_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}chiper${NC}              Start chatting"
    echo -e "   ${GREEN}chiper setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}chiper config${NC}       View/edit configuration"
    echo -e "   ${GREEN}chiper config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}chiper gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}chiper update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'chiper' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'chiper' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'chiper' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$CHIPER_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$CHIPER_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$CHIPER_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$CHIPER_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$CHIPER_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Chiper for pip install"
    ...
    CHIPER_CMD="$(command -v chiper 2>/dev/null || echo "")"
    if [ -n "$CHIPER_CMD" ]; then
        log_info "Running chiper setup..."
        "$CHIPER_CMD" setup
    else
        log_warn "chiper command not found on PATH"
        log_info "Try: python -m chiper_cli.main setup"
    fi
}
...
main() {
    ...
    echo "git" > "$CHIPER_HOME/.install_method"
}
