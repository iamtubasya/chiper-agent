...
def get_chiper_home() -> Path:
    ...
    When ``CHIPER_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``CHIPER_HOME`` explicitly (see the systemd
    template in ``chiper_cli/gateway.py`` and the kanban dispatcher in
    ``chiper_cli/kanban_db.py``).  See https://github.com/NousResearch/chiper-agent/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_chiper_home()
        ...
        if active and active != "default":
            ...
            msg = (
                f"[CHIPER_HOME fallback] CHIPER_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass CHIPER_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
    ...
    return _get_platform_default_chiper_home()
...
def get_default_chiper_root() -> Path:
    ...
    In profile mode where ``CHIPER_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.chiperflux/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_chiper_home()
    env_home = os.environ.get("CHIPER_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # CHIPER_HOME is under ~/.chiperflux (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — CHIPER_HOME itself is the root
    return env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``CHIPER_OPTIONAL_SKILLS``.
    """
    override = os.getenv("CHIPER_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_chiper_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``CHIPER_OPTIONAL_MCPS``.
    """
    override = os.getenv("CHIPER_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_chiper_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``CHIPER_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<CHIPER_HOME>/skills`` last-resort
    """
    override = os.getenv("CHIPER_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_chiper_dir(new_subpath: str, old_name: str) -> Path:
    ...
    Args:
        new_subpath: Preferred path relative to CHIPER_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to CHIPER_HOME (e.g. ``"image_cache"``).
...
def display_chiper_home() -> str:
    """Return a user-friendly display string for the current CHIPER_HOME.
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``CHIPER_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/chiper-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{CHIPER_HOME}/home`` when the profile-home directory exists."""
    chiper_home = get_chiper_home_override() or (env or {}).get("CHIPER_HOME") or os.getenv("CHIPER_HOME")
    if not chiper_home:
        return None
    profile_home = os.path.join(chiper_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("CHIPER_REAL_HOME") or os.getenv("CHIPER_REAL_HOME", "")).strip()
    if explicit:
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{CHIPER_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{CHIPER_HOME}/home`` when it exists, preserving the
...
def is_container() -> bool:
    ...
    See: NousResearch/chiper-agent#47111
    """
...
def get_skills_dir() -> Path:
    """Return the path to the skills directory under CHIPER_HOME."""
    return get_chiper_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under CHIPER_HOME."""
