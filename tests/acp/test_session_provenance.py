...
def test_root_session_no_compression(db):
    ...
    assert prov["currentChiperSessionId"] == "root1"
    assert prov["rootChiperSessionId"] == "root1"
    assert prov["parentChiperSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_chiper_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentChiperSessionId"] == "old"
    assert prov["rootChiperSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousChiperSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootChiperSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootChiperSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_chiper_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousChiperSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"chiper"}
    assert "sessionProvenance" in meta["chiper"]
    assert meta["chiper"]["sessionProvenance"]["currentChiperSessionId"] == "root1"
