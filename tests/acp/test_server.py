...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "chiper-agent"
        assert resp.agent_info.version == CHIPER_VERSION
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "chiper_cli.models.parse_model_input",
...
class TestPrompt:
    ...
    async def test_prompt_propagates_chiper_session_id_env(self, agent, monkeypatch):
        ...
        def mock_run(user_message, conversation_history=None, task_id=None, **kwargs):
            ...
            captured["env"] = os.environ.get("CHIPER_SESSION_ID")
        ...
        assert captured["env"] == new_resp.session_id, (
            "CHIPER_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("CHIPER_SESSION_ID") is None, (
            "CHIPER_SESSION_ID must be restored after the agent call so "
    ...
    async def test_prompt_restores_prior_chiper_session_id(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("CHIPER_SESSION_ID", "outer-sess")
        ...
        def mock_run(*args, **kwargs):
            captured["inner"] = os.environ.get("CHIPER_SESSION_ID")
            return {"final_response": "ok", "messages": []}
        ...
        assert os.environ.get("CHIPER_SESSION_ID") == "outer-sess"
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert CHIPER_VERSION in result
