...
class TestRunJobSessionPersistence:
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("TELEGRAM_HOME_CHANNEL", raising=False)
        monkeypatch.delenv("CHIPER_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("CHIPER_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("CHIPER_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("CHIPER_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("CHIPER_CRON_AUTO_DELIVER_CHAT_ID") or None
                seen["thread_id"] = get_session_env("CHIPER_CRON_AUTO_DELIVER_THREAD_ID") or None
        ...
        assert os.getenv("CHIPER_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("CHIPER_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("CHIPER_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
    ...
    def test_run_job_clears_stale_auto_delivery_thread_id_between_jobs(self, tmp_path, monkeypatch):
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen.append(
                    {
                        "platform": get_session_env("CHIPER_CRON_AUTO_DELIVER_PLATFORM") or None,
                        "chat_id": get_session_env("CHIPER_CRON_AUTO_DELIVER_CHAT_ID") or None,
                        "thread_id": get_session_env("CHIPER_CRON_AUTO_DELIVER_THREAD_ID") or None,
                    }
                )
                return {"final_response": "ok"}
...
class TestParallelTick:
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """CHIPER_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("CHIPER_CRON_MAX_PARALLEL", "1")
        call_times = []
