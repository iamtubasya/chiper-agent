...
- Path resolution (absolute, relative to CHIPER_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp CHIPER_HOME."""
    chiper_home = tmp_path / ".hermes"
    chiper_home.mkdir()
    (chiper_home / "cron").mkdir()
    (chiper_home / "cron" / "output").mkdir()
    (chiper_home / "scripts").mkdir()
    monkeypatch.setenv("CHIPER_HOME", str(chiper_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "CHIPER_DIR", chiper_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", chiper_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", chiper_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", chiper_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.chiperflux/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("CHIPER_SESSION_PLATFORM") is None
        assert os.environ.get("CHIPER_SESSION_CHAT_ID") is None
        assert os.environ.get("CHIPER_SESSION_CHAT_NAME") is None
