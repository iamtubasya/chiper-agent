...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    chiper_state = ModuleType("chiper_state")
    chiper_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "chiper_state", chiper_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Chiper"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Chiper"
