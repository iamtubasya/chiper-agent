...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Chiper passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("CHIPER_HOME", "/users/alice/.chiperflux/profiles/backend-worker")
        monkeypatch.setenv("CHIPER_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "CHIPER_KANBAN_DB",
            "/users/alice/.chiperflux/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.chiperflux/kanban/boards/smoke"]'
            in cmd
        )
