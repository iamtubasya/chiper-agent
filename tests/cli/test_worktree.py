...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("chiper/chiper-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned chiper/* and pr-* branches."""

    def test_prunes_orphaned_chiper_branch(self, git_repo):
        """chiper/chiper-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "chiper/chiper-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "chiper/chiper-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "chiper/chiper-deadbeef" in result.stdout
        ...
        assert "chiper/chiper-deadbeef" in orphaned
        ...
        assert "chiper/chiper-deadbeef" not in result.stdout
