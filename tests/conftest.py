"""Shared fixtures for the chiper-agent test suite.
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    "CHIPER_INTERACTIVE",
    "CHIPER_QUIET",
    "CHIPER_TOOL_PROGRESS",
    "CHIPER_TOOL_PROGRESS_MODE",
    "CHIPER_MAX_ITERATIONS",
    "CHIPER_SESSION_PLATFORM",
    "CHIPER_SESSION_CHAT_ID",
    "CHIPER_SESSION_CHAT_NAME",
    "CHIPER_SESSION_THREAD_ID",
    "CHIPER_SESSION_SOURCE",
    "CHIPER_SESSION_KEY",
    ...
    "CHIPER_TIMEZONE",
    "CHIPER_REDACT_SECRETS",
    "CHIPER_BACKGROUND_NOTIFICATIONS",
    "CHIPER_EXEC_ASK",
    "CHIPER_HOME_MODE",
    "CHIPER_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.chiperflux/kanban.db instead of the per-test CHIPER_HOME.
    "CHIPER_KANBAN_DB",
    "CHIPER_KANBAN_BOARD",
    "CHIPER_KANBAN_HOME",
    "CHIPER_KANBAN_WORKSPACES_ROOT",
    "CHIPER_KANBAN_LOGS_ROOT",
    "CHIPER_KANBAN_TASK",
    "CHIPER_KANBAN_WORKSPACE",
    "CHIPER_KANBAN_RUN_ID",
    "CHIPER_KANBAN_CLAIM_LOCK",
    "CHIPER_KANBAN_DISPATCH_IN_GATEWAY",
    "CHIPER_TENANT",
    ...
    "CHIPER_DASHBOARD_OAUTH_CLIENT_ID",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and CHIPER_HOME to per-test tempdirs so code that
    reads ``~/.chiperflux/*`` can't touch the real one, and pins TZ/LANG so
    ...
    #    ``~/.chiperflux/*`` via ``get_chiper_home()`` now gets the tempdir.
    #
    ...
    (fake_chiper_home / "memories").mkdir()
    (fake_chiper_home / "skills").mkdir()
    monkeypatch.setenv("CHIPER_HOME", str(fake_chiper_home))
    ...
    #    ~/.chiperflux/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import chiper_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_chiper_home(_hermetic_environment):
...
# environment and finds the developer's live ``chiper-gateway`` process
...
# correlated with a ``tests/chiper_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> chiper-gateway``
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live chiper-gateway systemd unit. Mock "
