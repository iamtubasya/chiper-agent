"""Tests for get_chiper_home() profile-mode fallback warning.

Regression test for https://github.com/NousResearch/chiper-agent/issues/18594.

When CHIPER_HOME is unset but an active_profile file indicates a non-default
profile is active, get_chiper_home() should:
  1. STILL return ~/.chiperflux (raising would brick 30+ module-level callers)
...
class TestGetHermesHomeProfileWarning:
    ...
    def test_named_profile_unset_home_warns_once(
        ...
        fresh_constants.get_chiper_home()
        fresh_constants.get_chiper_home()
        err2 = capsys.readouterr().err
        assert "CHIPER_HOME fallback" not in err2

    def test_chiper_home_set_suppresses_warning(
