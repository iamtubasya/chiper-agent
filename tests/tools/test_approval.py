...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("chiper_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("chiper_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"CHIPER_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_custom_chiper_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $CHIPER_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_chiper_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$CHIPER_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestChiperConfigWriteProtection:
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.chiperflux/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.chiperflux/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.chiperflux/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place_absolute_chiper_home_config(self):
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.chiperflux/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_chiper_home_config(self):
        config_path = get_chiper_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.chiperflux/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_chiper_home_env(self):
        env_path = get_chiper_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.chiperflux/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.chiperflux/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.chiperflux/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.chiperflux/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.chiperflux/config.yaml")
        assert dangerous is False
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_chiper_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $CHIPER_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.chiperflux/config.yaml/.env must require approval — the
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.chiperflux/chiper-agent && source venv/bin/activate && python -m chiper_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m chiper_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m chiper_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "chiper_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart chiper-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("CHIPER_GATEWAY_SESSION", "CHIPER_CRON_SESSION",
                      "CHIPER_YOLO_MODE",
                      "CHIPER_SESSION_KEY", "CHIPER_INTERACTIVE")
        }
        os.environ.pop("CHIPER_YOLO_MODE", None)
        os.environ.pop("CHIPER_INTERACTIVE", None)
        # CHIPER_CRON_SESSION takes priority over CHIPER_GATEWAY_SESSION in
        ...
        os.environ.pop("CHIPER_CRON_SESSION", None)
        os.environ["CHIPER_GATEWAY_SESSION"] = "1"
        os.environ["CHIPER_SESSION_KEY"] = self.SESSION_KEY
