...
def _restore_tool_modules():
    original_chiper_home = os.environ.get("CHIPER_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_chiper_home is None:
            os.environ.pop("CHIPER_HOME", None)
        else:
            os.environ["CHIPER_HOME"] = original_chiper_home
        _reset_modules(("tools", "chiper_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="chiper-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": chiper_home / "modal_snapshots.json",
