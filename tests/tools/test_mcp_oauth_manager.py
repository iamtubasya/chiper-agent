...
def test_chiper_provider_subclass_exists():
    """ChiperMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _CHIPER_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _CHIPER_PROVIDER_CLS is not None
    assert issubclass(_CHIPER_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_chiper_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ChiperMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _CHIPER_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("CHIPER_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _CHIPER_PROVIDER_CLS is not None
    assert isinstance(provider, _CHIPER_PROVIDER_CLS)
    assert provider._chiper_server_name == "srv"
