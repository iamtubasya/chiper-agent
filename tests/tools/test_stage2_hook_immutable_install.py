...
Hosted/container Chiper keeps user-writable state under CHIPER_HOME
(/opt/data). The installed source, venv, TUI bundle, and node_modules under
/opt/chiper must remain root-owned/non-writable by the runtime chiper user so
...
def test_stage2_does_not_chown_install_tree_to_chiper(stage2_text: str) -> None:
    ...
    for install_tree in (
        ...
        assert install_tree not in stage2_text, (
            f"stage2 must not chown {install_tree} back to chiper; "
            "the Dockerfile keeps /opt/chiper immutable and writable state "
            "belongs under CHIPER_HOME"
        )
...
def test_stage2_documents_immutable_install_contract(stage2_text: str) -> None:
    ...
    assert "CHIPER_DISABLE_LAZY_INSTALLS=1" in stage2_text
    assert "/opt/chiper" in stage2_text
