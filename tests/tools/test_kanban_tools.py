...
Verifies:
  - Tools are gated on CHIPER_KANBAN_TASK: a normal chat session sees
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the CHIPER_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("CHIPER_SESSION_ID", raising=False)
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("CHIPER_KANBAN_TASK", tid)
    return tid
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["chiper_cli/kanban.py"],
...
def test_complete_stamps_worker_session_id_from_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("CHIPER_SESSION_ID", "session-trusted")
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to CHIPER_PROFILE env we set in the fixture
...
def test_create_inherits_worker_dir_workspace(monkeypatch, worker_env):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("CHIPER_KANBAN_TASK", self_tid)
...
def test_create_no_worker_task_stays_scratch(monkeypatch, worker_env):
    """Orchestrator/CLI callers (no CHIPER_KANBAN_TASK) still default to
...
def test_create_session_id_arg_overrides_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("CHIPER_SESSION_ID", "from-env")
    from tools import kanban_tools as kt
    from chiper_cli import kanban_db as kb
    out = kt._handle_create({
...
# ---------------------------------------------------------------------------
#
# A worker process has CHIPER_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no CHIPER_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # CHIPER_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("CHIPER_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("CHIPER_KANBAN_RUN_ID", str(run2.id))
...
def multi_board_env(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("CHIPER_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("CHIPER_KANBAN_TASK", raising=False)
    monkeypatch.setenv("CHIPER_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
