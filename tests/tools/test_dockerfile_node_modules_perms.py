...
Older images made /opt/chiper/ui-tui and /opt/chiper/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV CHIPER_TUI_DIR=/opt/chiper/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R chiper:chiper /opt/chiper/ui-tui" not in text
    assert "chown -R chiper:chiper /opt/chiper/node_modules" not in text
