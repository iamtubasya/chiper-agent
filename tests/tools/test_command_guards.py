...
def _clean_state():
    ...
    saved = {}
    for k in ("CHIPER_INTERACTIVE", "CHIPER_GATEWAY_SESSION", "CHIPER_EXEC_ASK", "CHIPER_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["CHIPER_INTERACTIVE"] = "1"
        session_key = os.getenv("CHIPER_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No CHIPER_INTERACTIVE or CHIPER_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
...
class TestGatewayApprovalAllowPermanent:
    ...
    def _capture_gateway_payload(self, command, session_key):
        ...
        os.environ["CHIPER_SESSION_KEY"] = session_key
        try:
            check_all_command_guards(command, "local")
        finally:
            os.environ.pop("CHIPER_GATEWAY_SESSION", None)
            os.environ.pop("CHIPER_EXEC_ASK", None)
            os.environ.pop("CHIPER_SESSION_KEY", None)
