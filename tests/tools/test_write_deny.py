...
class TestWriteDenyExactPaths:
    ...
    def test_chiper_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.chiperflux/.env`` (whose credentials are inherited by every profile)
        ...
        # Sanity check: CHIPER_HOME does point to the profile dir, not the root.
        from chiper_constants import get_chiper_home, get_default_chiper_root
        assert get_chiper_home() == profile_home
        assert get_default_chiper_root() == root
