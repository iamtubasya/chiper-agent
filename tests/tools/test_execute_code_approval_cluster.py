...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad CHIPER_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: CHIPER_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("CHIPER_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("CHIPER_CRON_SESSION", "1")
    monkeypatch.delenv("CHIPER_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_chiper_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "CHIPER_HOME": "/h", "CHIPER_PROFILE": "p",
        "CHIPER_CONFIG": "/c.yaml", "CHIPER_ENV": "/e",
        # other CHIPER_* → dropped (broad prefix removed)
        "CHIPER_BASE_URL": "https://x", "CHIPER_INTERACTIVE": "1",
        "CHIPER_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("CHIPER_HOME", "CHIPER_PROFILE", "CHIPER_CONFIG", "CHIPER_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "CHIPER_BASE_URL", "CHIPER_INTERACTIVE", "CHIPER_KANBAN_DB",
...
def test_env_scrub_logs_dropped_chiper_vars(caplog):
    """Dropping a non-allowlisted, non-secret CHIPER_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset CHIPER_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "CHIPER_HOME": "/h",          # allowlisted → kept, not logged
        "CHIPER_BASE_URL": "https://x",   # dropped → logged
        "CHIPER_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "CHIPER_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "CHIPER_HOME" in out and "PATH" in out
    assert "CHIPER_BASE_URL" not in out and "CHIPER_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "CHIPER_BASE_URL" in msgs and "CHIPER_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "CHIPER_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped CHIPER_* vars."""
    import logging
