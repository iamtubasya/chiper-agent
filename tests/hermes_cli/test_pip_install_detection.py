...
def test_managed_install_takes_precedence(tmp_path):
    """When CHIPER_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("chiper_cli.config.get_managed_system", return_value="NixOS"), \
         patch("chiper_cli.config.get_chiper_home", return_value=tmp_path):
        from chiper_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from chiper_cli.config import recommended_update_command_for_method
...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $CHIPER_HOME.
    ...
    with patch("chiper_cli.config.get_managed_system", return_value=None), \
         patch("chiper_cli.config.get_chiper_home", return_value=home):
        from chiper_cli.config import detect_install_method
...
def test_home_docker_stamp_ignored_when_not_containerized(tmp_path):
    ...
    'docker' into the shared $CHIPER_HOME. With no code-scoped stamp, a host
    ...
    with patch("chiper_cli.config.get_managed_system", return_value=None), \
         patch("chiper_cli.config.get_chiper_home", return_value=home), \
         patch("chiper_cli.config._running_in_container", return_value=False):
        from chiper_cli.config import detect_install_method
        assert detect_install_method(project_root=code) == "git"
...
def test_home_docker_stamp_honored_inside_container(tmp_path):
    ...
    with patch("chiper_cli.config.get_managed_system", return_value=None), \
         patch("chiper_cli.config.get_chiper_home", return_value=home), \
         patch("chiper_cli.config._running_in_container", return_value=True):
        from chiper_cli.config import detect_install_method
...
def test_home_non_docker_stamp_still_honored_for_backcompat(tmp_path):
    ...
    install that historically stamped 'git'/'pip' into $CHIPER_HOME keeps
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    """stamp_install_method writes next to the code, not into $CHIPER_HOME."""
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    home.mkdir()
