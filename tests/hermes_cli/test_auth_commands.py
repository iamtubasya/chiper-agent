...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from chiper_cli.auth_commands import auth_add_command
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "chiper_cli.auth._nous_device_code_login",
        lambda **kwargs: {
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "chiper_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "chiper_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("chiper_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "chiper_cli.auth._xai_oauth_loopback_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from chiper_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    ...
    config_text = (chiper_home / "config.yaml").read_text()
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((chiper_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from chiper_cli.auth import _reset_config_provider
    ...
    with patch("chiper_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from chiper_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "chiper_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("chiper_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("chiper_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("chiper_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "chiper_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    updated = json.loads((chiper_home / "auth.json").read_text())
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("chiper_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((chiper_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.chiperflux/.env (not the shell), the
    ...
    (chiper_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (chiper_home / ".env").read_text().strip() == ""
...
def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from chiper_cli.auth import is_source_suppressed
    from chiper_cli.auth_commands import auth_add_command
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import chiper_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_chiper_pkce_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (chiper_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (chiper_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (chiper_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_credential_sources_registry_has_expected_steps():
    ...
    required = {
        ...
        "~/.chiperflux/.anthropic_oauth.json",
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        "chiper_cli.copilot_auth.get_copilot_api_token",
