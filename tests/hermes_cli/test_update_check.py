"""Tests for the update check mechanism in chiper_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "chiper-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("chiper_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    """
    import chiper_cli.banner as banner

    # No local git checkout -> the PyPI path is exercised (pip-install class).
    fake_banner = tmp_path / "chiper_cli" / "banner.py"
    ...
    monkeypatch.setenv("CHIPER_HOME", str(tmp_path))
    monkeypatch.delenv("CHIPER_REVISION", raising=False)
    with patch("chiper_cli.banner.subprocess.run") as mock_run, \
         patch("chiper_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CHIPER_HOME", str(tmp_path))
    with patch("chiper_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_official_ssh_origin_uses_https_probe(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="git@github.com:NousResearch/chiper-agent.git\n")
    ...
    with patch("chiper_cli.banner.subprocess.run", side_effect=fake_run):
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CHIPER_HOME", str(tmp_path))
    with patch("chiper_cli.banner.subprocess.run") as mock_run:
        with patch("chiper_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when CHIPER_HOME has no git repo."""
    import chiper_cli.banner as banner
    ...
    # Point CHIPER_HOME at a temp dir with no chiper-agent/.git
    monkeypatch.setenv("CHIPER_HOME", str(tmp_path))
    with patch("chiper_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    CHIPER_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    with patch("chiper_cli.config.detect_install_method", return_value="docker"), \
         patch("chiper_cli.banner.subprocess.run") as mock_run, \
         patch("chiper_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_check_for_updates_non_docker_still_checks(tmp_path, monkeypatch):
    ...
    with patch("chiper_cli.config.detect_install_method", return_value="pip"), \
         patch("chiper_cli.banner.subprocess.run") as mock_run, \
         patch("chiper_cli.banner.check_via_pypi", return_value=1) as mock_pypi:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"CHIPER_HOME": str(default_home)}):
