"""Regression tests for #34107 — Docker UID/GID handling in ensure_chiper_home.

When Chiper runs in Docker with ``CHIPER_UID=1000`` / ``CHIPER_GID=911``,
the entrypoint chowns the top-level ``CHIPER_HOME`` once at startup. But
...
The fix is a ``_chown_to_chiper_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_chiper_uid_gid
# ---------------------------------------------------------------------------


class TestResolveChiperUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("CHIPER_UID", "1000")
        monkeypatch.setenv("CHIPER_GID", "911")
        from chiper_cli.config import _resolve_chiper_uid_gid
        uid, gid = _resolve_chiper_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("CHIPER_UID", raising=False)
        monkeypatch.delenv("CHIPER_GID", raising=False)
        from chiper_cli.config import _resolve_chiper_uid_gid
        uid, gid = _resolve_chiper_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("CHIPER_UID", "not-a-number")
        monkeypatch.setenv("CHIPER_GID", "911")
        from chiper_cli.config import _resolve_chiper_uid_gid
        uid, gid = _resolve_chiper_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("CHIPER_UID", "")
        monkeypatch.setenv("CHIPER_GID", "")
        from chiper_cli.config import _resolve_chiper_uid_gid
        uid, gid = _resolve_chiper_uid_gid()
        assert uid is None
        assert gid is None
...
# ---------------------------------------------------------------------------
# _chown_to_chiper_uid
# ---------------------------------------------------------------------------


class TestChownToChiperUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CHIPER_UID", "1000")
        monkeypatch.setenv("CHIPER_GID", "911")
        from chiper_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_chiper_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_chiper_uid(d)  # must not raise
