"""Tests for `chiper chat --safe-mode` — pristine troubleshooting runs.
...
_VARS = ("CHIPER_SAFE_MODE", "CHIPER_IGNORE_USER_CONFIG", "CHIPER_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in chiper_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("CHIPER_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under CHIPER_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("CHIPER_SAFE_MODE", "1")
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("CHIPER_SAFE_MODE", raising=False)
        from chiper_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under CHIPER_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `chiper chat`."""
