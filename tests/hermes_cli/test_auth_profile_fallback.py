...
When ``CHIPER_HOME`` points to a named profile, ``read_credential_pool()``
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from chiper_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    chiper_home = tmp_path / "classic"
    chiper_home.mkdir()
    monkeypatch.setenv("CHIPER_HOME", str(chiper_home))

    _write(chiper_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (CHIPER_HOME == global root), no fallback path runs.
    ...
    _write(chiper_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from chiper_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: CHIPER_HOME is set to a custom path that is NOT under
    # ~/.chiperflux/profiles/ — get_default_chiper_root() returns CHIPER_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from chiper_cli.auth import read_credential_pool, write_credential_pool
