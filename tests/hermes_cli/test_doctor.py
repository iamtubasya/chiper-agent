...
import chiper_cli.doctor as doctor
import chiper_cli.gateway as gateway_cli
from chiper_cli import doctor as doctor_mod
from chiper_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "CHIPER_HOME", chiper_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("CHIPER_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("CHIPER_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_chiper_home(tmp_path, provider)
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from chiper_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("chiper_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "chiper_cli.doctor", raising=False)
        from chiper_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale CHIPER_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"CHIPER_MAX_ITERATIONS={ghost}\n")
        (chiper_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "CHIPER_HOME", chiper_home)
        monkeypatch.setattr(doctor_mod, "get_chiper_home", lambda: chiper_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("CHIPER_MAX_ITERATIONS", str(os_environ_value))
        else:
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale CHIPER_MAX_ITERATIONS" in out
        env_after = (chiper_home / ".env").read_text(encoding="utf-8")
        assert "CHIPER_MAX_ITERATIONS" not in env_after
