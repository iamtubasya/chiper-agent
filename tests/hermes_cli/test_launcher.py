"""Tests for the top-level `./chiper` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./chiper` should use `chiper_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "chiper"
    called = []

    fake_main_module = types.ModuleType("chiper_cli.main")

    def fake_main():
        called.append("chiper_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "chiper_cli.main", fake_main_module)
