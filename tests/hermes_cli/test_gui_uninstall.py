"""Tests for chiper_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import chiper_cli.gui_uninstall as gu


def _make_agent(chiper_home: Path) -> Path:
    ...
    (agent_root / "chiper_cli" / "__init__.py").write_text("")
...
def _make_gui_build(chiper_home: Path) -> None:
    ...
    (chiper_home / "chiper-agent" / "node_modules").mkdir(parents=True)
    (chiper_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(chiper_home: Path) -> None:
    (chiper_home / "config.yaml").write_text("x: 1\n")
    (chiper_home / ".env").write_text("KEY=secret\n")
    (chiper_home / "sessions").mkdir()
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(chiper_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(chiper_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    removed = gu.uninstall_gui(chiper_home)
    ...
    assert not (chiper_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "chiper_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (chiper_home / "config.yaml").exists()
    assert (chiper_home / ".env").exists()
    assert (chiper_home / "sessions").exists()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(chiper_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(chiper_home)
    ...
    assert summary["chiper_home"] == str(chiper_home)
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    CHIPER_HOME with all the destructive externals stubbed out.
    """
    ...
    monkeypatch.setattr(uninstall, "get_chiper_home", lambda: chiper_home)
    ...
    from chiper_cli import gui_uninstall as gu_mod
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    ...
    assert not chiper_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m chiper_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_chiper_home", lambda: chiper_home)
    ...
    assert not (desktop / "release").exists()
    assert not (chiper_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "chiper_cli").exists()
    assert (chiper_home / "config.yaml").exists()
