...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    ("chiper-agent-prod-abc.fly.dev", False, True),
])
def test_should_require_auth_truth_table(host, allow_public, expected):
    from chiper_cli.web_server import should_require_auth
...
def test_start_server_public_without_insecure_records_auth_required(monkeypatch):
    ...
    """
    from chiper_cli.dashboard_auth import clear_providers
    clear_providers()
    _stub_uvicorn_run(monkeypatch)
    web_server.app.state.auth_required = None
...
def test_start_server_surfaces_nous_skip_reason_when_unconfigured(monkeypatch):
    ...
    'set CHIPER_DASHBOARD_OAUTH_CLIENT_ID', not 'install a plugin'."""
    from chiper_cli.dashboard_auth import clear_providers
    ...
    assert "CHIPER_DASHBOARD_OAUTH_CLIENT_ID" in msg
    assert "nous:" in msg
