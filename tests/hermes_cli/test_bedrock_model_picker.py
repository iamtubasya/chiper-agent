...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (CHIPER_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via CHIPER_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in CHIPER_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from chiper_cli.providers import CHIPER_OVERLAYS
        assert "bedrock" in CHIPER_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from chiper_cli.providers import CHIPER_OVERLAYS
        assert CHIPER_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from chiper_cli.providers import CHIPER_OVERLAYS
        assert CHIPER_OVERLAYS["bedrock"].auth_type == "aws_sdk"
