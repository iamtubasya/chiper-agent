...
from chiper_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<chiper_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<chiper_home>/plugins/``; we derive
    ``<chiper_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <CHIPER_HOME>/config.yaml.
        # Config is always read from CHIPER_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        chiper_home_str = os.environ.get("CHIPER_HOME")
        if chiper_home_str:
            chiper_home = Path(chiper_home_str)
        else:
            chiper_home = base.parent
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("chiper_cli.plugins._plugin_manager", mgr):
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("chiper_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("chiper_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("chiper_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("chiper_cli.plugins.PluginContext.dispatch_tool.__module__", "chiper_cli.plugins"):
...
class TestPluginDebugLogging:
    """CHIPER_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("CHIPER_PLUGINS_DEBUG", raising=False)
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With CHIPER_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("CHIPER_PLUGINS_DEBUG", "1")
        from chiper_cli import plugins as plugins_mod
