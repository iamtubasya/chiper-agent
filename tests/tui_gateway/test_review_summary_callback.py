    ...
    with patch.dict(
        "sys.modules",
        {
            "chiper_constants": MagicMock(
                get_chiper_home=MagicMock(return_value="/tmp/chiper_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'chiper-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'chiper-release' patched"
    }
