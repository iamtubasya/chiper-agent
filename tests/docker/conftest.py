...
Override the image with ``CHIPER_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("CHIPER_TEST_IMAGE", "chiper-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``CHIPER_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("CHIPER_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged chiper user
# ---------------------------------------------------------------------------
#
# Background: every Chiper runtime path inside the container drops to UID
# 10000 (the ``chiper`` user) via ``s6-setuidgid chiper``. ``docker exec``
...
# fails (PermissionError swallowed) for chiper, so a test that ran as root
...
# helpers below run every probe under ``-u chiper`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: chiper).
