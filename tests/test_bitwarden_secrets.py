...
binary download are exercised manually by `chiper secrets bitwarden
...
def chiper_home(tmp_path, monkeypatch):
    """Point Chiper at an isolated home directory."""
    home = tmp_path / ".chiper"
    home.mkdir()
    monkeypatch.setenv("CHIPER_HOME", str(home))
    # Some modules cache get_chiper_home; clear if needed.
    import chiper_constants
    if hasattr(chiper_constants, "_CHIPER_HOME_CACHE"):
        chiper_constants._CHIPER_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(chiper_home, monkeypatch):
...
def test_install_bws_happy_path(chiper_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(chiper_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(chiper_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from chiper_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under chiper_home/cache/."""
    home = tmp_path / ".chiper"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
