...
class TestToolsetConsistency:
    ...
    def test_chiper_platforms_share_core_tools(self):
        """All chiper-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on chiper-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["chiper-cli", "chiper-telegram", "chiper-discord", "chiper-whatsapp", "chiper-slack", "chiper-signal", "chiper-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_chiper_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("chiper-whatsapp")

    def test_chiper_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("chiper-api-server")
