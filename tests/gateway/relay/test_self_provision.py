...
def _arm(monkeypatch, *, managed=True, url="wss://connector.example/relay", token="nas-token"):
    monkeypatch.setattr("chiper_cli.config.is_managed", lambda: managed)
    monkeypatch.setattr(relay, "relay_url", lambda: url)
    monkeypatch.setattr("chiper_cli.auth.resolve_nous_access_token", lambda: token)
...
def test_token_failure_is_non_fatal(monkeypatch):
    ...
    monkeypatch.setattr("chiper_cli.auth.resolve_nous_access_token", _boom)
