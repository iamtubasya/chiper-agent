...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        ...
        assert "brew upgrade chiper-agent" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _chiper_home to tmp_path and project_root to a dir without .git
        ...
        with patch("gateway.run._chiper_home", tmp_path):
            ...
            # project_root will be the real chiper-agent dir (which HAS .git).
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((chiper_home / ".update_pending.json").read_text())
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (chiper_home / ".update_pending.json").exists()
        assert not (chiper_home / ".update_exit_code").exists()
...
class TestUpdateCommandPlatformGate:
    ...
    async def test_blocks_programmatic_interface(self, monkeypatch):
        ...
        monkeypatch.setenv("CHIPER_MANAGED", "")
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (chiper_home / ".update_output.txt").write_text("still running")
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (chiper_home / ".update_pending.json").write_text(json.dumps(pending))
        (chiper_home / ".update_output.txt").write_text("x" * 5000)
        (chiper_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (chiper_home / ".update_pending.json").write_text(json.dumps(pending))
        (chiper_home / ".update_output.txt").write_text("Traceback: boom")
    ...
    async def test_no_adapter_for_platform_preserves_markers(self, tmp_path):
        ...
        assert not (chiper_home / ".update_pending.claimed.json").exists()
