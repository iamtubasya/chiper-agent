...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/chiper-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    chiper_meta = (fm.get("metadata") or {}).get("chiper") or {}
    tags = chiper_meta.get("tags") or []
    related = chiper_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `chiper skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Chiper Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Chiper ships with a large built-in skill library copied into `~/.chiperflux/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Chiper also syncs bundled skills on `chiper update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.chiperflux/skills/` tree, it is still shipped with Chiper; restore it with `chiper skills reset <name> --restore`.",
        "",
