...
| Google Gemini（OAuth） | `google-gemini-cli` | `hermes model`（Google OAuth；可选：`CHIPER_GEMINI_PROJECT_ID`） |
...
| Qwen Portal（OAuth） | `qwen-oauth` | `hermes model`（Qwen Portal OAuth；可选：`CHIPER_QWEN_BASE_URL`） |
