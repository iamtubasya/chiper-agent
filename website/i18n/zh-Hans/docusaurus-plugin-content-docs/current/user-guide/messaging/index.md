...
在 `~/.chiperflux/gateway.json` 中配置各平台的覆盖设置：
...
在 `~/.chiperflux/config.yaml` 中控制显示多少工具活动信息：
...
当运行后台会话的 agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，网关可以向你的聊天推送状态更新。通过 `~/.chiperflux/config.yaml` 中的 `display.background_process_notifications` 控制：
...
1. **网关日志**（`~/.chiperflux/logs/gateway.log` 或 systemd / launchd 单元日志）。搜索平台名称以及 `circuit breaker`、`paused` 或 `disabled`。触发事件包含失败次数和最后一个错误。
