...
| `hermes checkpoints` | 检查/修剪/清除 `~/.chiperflux/checkpoints/`（`/rollback` 使用的影子存储）。不带参数运行可查看状态概览。 |
...
| `hermes update` | 拉取最新代码并重新安装依赖（git 安装），或检查 PyPI 并执行 `pip install --upgrade`（pip 安装）。`--check` 预览而不安装；`--backup` 在拉取前对 `CHIPER_HOME` 进行快照。 |
...
| 选项 | 说明 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 时：对**每个 profile** 的 gateway 执行操作，而不仅限于活跃的 `CHIPER_HOME`。当你并行运行多个 profile 并希望在 `hermes update` 后全部重启时很有用。 |
...
:::tip WSL 用户
使用 `hermes gateway run` 而非 `hermes gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s hermes 'hermes gateway run'`。详见 [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-chiper-gateway-start-fails)。
:::
...
hermes slack manifest --write      # 写入 ~/.chiperflux/slack-manifest.json
...
创建 Hermes 配置、skill、会话和数据的 zip 归档。备份不包含 chiper-agent 代码库本身。
...
查看、跟踪和过滤 Hermes 日志文件。所有日志存储在 `~/.chiperflux/logs/`（非默认 profile 存储在 `<profile>/logs/`）。
...
将 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.chiperflux`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件名（`clawdbot.json`、`moltbot.json`）。
...
| `--no-backup` | 跳过迁移前对 `~/.chiperflux/` 的 zip 快照（默认情况下，在应用前会将单个还原点归档写入 `~/.chiperflux/backups/pre-migration-*.zip`；可用 `hermes import` 恢复）。 |
...
**pip 安装：** `hermes update` 自动检测基于 pip 的安装——查询 PyPI 获取最新版本并运行 `pip install --upgrade chiper-agent`，而非 `git pull`。PyPI 发布跟踪标记版本（主要/次要版本），而非 `main` 上的每个 commit。使用 `--check` 查看是否有更新的 PyPI 版本可用，而不安装。
...
- **配对数据快照。** 即使 `--backup` 关闭，`hermes update` 也会在 `git pull` 前对 `~/.chiperflux/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用 `hermes backup restore --state pre-update` 回滚。
- **旧版 `hermes.service` 警告。** 如果 Hermes 检测到预重命名的 `hermes.service` systemd 单元（而非当前的 `chiper-gateway.service`），会打印一次性迁移提示，帮助你避免循环重启问题。
