...
hermes update            # 执行 pip install --upgrade chiper-agent
```
...
1. **配对数据快照** — 保存一份轻量级的更新前状态快照（涵盖 `~/.chiperflux/pairing/`、飞书评论规则及其他运行时修改的状态文件）。可通过 [快照与回滚](../user-guide/checkpoints-and-rollback.md) 中描述的快照恢复流程进行恢复，或从 Hermes 写入 `~/.chiperflux/` 目录旁的最新快速快照 zip 文件中提取。
