...
description: "How Chiper stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `chiper_cli/cron.py` | CLI `chiper cron` subcommands |
...
Jobs are stored in `~/.chiperflux/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `chiper cron` commands are run or during active CLI sessions.
...
```python
# ~/.chiperflux/scripts/check_competitors.py
...
2. **Environment variable** — `CHIPER_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.chiperflux/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `chiper cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `chiper cron` CLI provides direct job management:

```bash
chiper cron list                    # Show all jobs
chiper cron create                  # Interactive job creation (alias: add)
chiper cron edit <job_id>           # Edit job configuration
chiper cron pause <job_id>          # Pause a running job
chiper cron resume <job_id>         # Resume a paused job
chiper cron run <job_id>            # Trigger immediate execution
chiper cron remove <job_id>         # Delete a job
```
