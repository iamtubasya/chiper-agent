...
The Zed ACP Registry path launches the same adapter through `uvx --from 'chiper-agent[acp]==<version>' hermes-acp`, pointed at the `chiper-agent` PyPI release.
...
- `chiper_cli/main.py` — `hermes acp` CLI subcommand
