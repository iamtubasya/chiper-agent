...
| `hermes secrets` | Manage external secret sources (currently Bitwarden Secrets Manager) for pulling API keys at process startup instead of from `~/.chiperflux/.env`. |
...
| `hermes checkpoints` | Inspect / prune / clear `~/.chiperflux/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
...
| `hermes update` | Pull latest code and reinstall dependencies (git installs), or check PyPI and `pip install --upgrade` (pip installs). `--check` previews without installing; `--backup` takes a pre-pull `CHIPER_HOME` snapshot. |
...
# or:
CHIPER_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 hermes -z "…"
```
...
| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `CHIPER_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `hermes update`. |
...
:::tip WSL users
Use `hermes gateway run` instead of `hermes gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s hermes 'hermes gateway run'`. See [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-chiper-gateway-start-fails) for details.
:::
...
hermes slack manifest --write      # write to ~/.chiperflux/slack-manifest.json
...
On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the Hermes venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.chiperflux/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
Create a zip archive of your Hermes configuration, skills, sessions, and data. The backup excludes the chiper-agent codebase itself.
...
View, tail, and filter Hermes log files. All logs are stored in `~/.chiperflux/logs/` (or `<profile>/logs/` for non-default profiles).
...
Migrate your OpenClaw setup to Hermes. Reads from `~/.openclaw` (or a custom path) and writes to `~/.chiperflux`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.chiperflux/` (by default a single restore-point archive is written to `~/.chiperflux/backups/pre-migration-*.zip` before apply; restorable with `hermes import`). |
...
- **Pairing data snapshot.** Even when `--backup` is off, `hermes update` takes a lightweight snapshot of `~/.chiperflux/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `hermes backup restore --state pre-update` if a pull rewrites a file you were editing.
- **Legacy `hermes.service` warning.** If Hermes detects a pre-rename `hermes.service` systemd unit (instead of the current `chiper-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
