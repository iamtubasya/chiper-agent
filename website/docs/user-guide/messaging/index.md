...
Configure per-platform overrides in `~/.chiperflux/gateway.json`:
...
Control how much tool activity is displayed in `~/.chiperflux/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.chiperflux/config.yaml`:
...
```bash
CHIPER_BACKGROUND_NOTIFICATIONS=result
```
...
1. **Gateway log** (`~/.chiperflux/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
