...
Connect Chiper to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Chiper supports two WeCom integration modes:
...
> Run `chiper gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Chiper decrypts the message, queues it for the agent
...
(Use `chiper gateway start` only after `chiper gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Chiper and the token the
...
in `~/.chiperflux/.env` values around `=` will also break signature checks. After
fixing, restart `chiper gateway run`.
...
Check `chiper gateway run` logs for the bound host/port. If the adapter bound to
