...
sidebar_position: 4
title: "Slack"
description: "Set up Chiper Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Chiper Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Chiper instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Chiper uses the modern
...
The fastest path is to paste a manifest Chiper generates for you. It
...
### Option A: From a Chiper-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   chiper slack manifest --write
   ```
   This writes `~/.chiperflux/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "Chiper Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Chiper can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `chiper-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Chiper configuration issue.
:::
...
Chiper uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
chiper gateway setup    # Select Slack when prompted
```
...
Every Chiper command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Chiper command with its description.

Under the hood: Chiper ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/chiper-agent/blob/main/chiper_cli/commands.py)
...
When Chiper adds new commands (e.g. after `chiper update`), regenerate
...
3. Paste the new contents of `~/.chiperflux/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/chiper <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/chiper btw run the tests` — Chiper routes it the same way as `/btw
run the tests`. Free-form questions also work: `/chiper what's the
...
Slack never delivers them to Chiper.

As a workaround, Chiper recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Chiper treats it identically to the slash form and replies in the same
thread.
...
Chiper falls back to a text prompt, the prompt instructs you to reply
...
```bash
chiper slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Chiper behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Chiper Agent what time is it?`). In channels, Chiper replies in a thread attached to that message. |
| **Threads** | If you @mention Chiper inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.chiperflux/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Chiper in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Chiper will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Chiper will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Chiper Agent`).

---

## Multi-Workspace Support

Chiper can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Chiper also loads tokens from an **OAuth token file** at:

```
~/.chiperflux/slack_tokens.json
```
...
- When a message arrives, Chiper uses the correct workspace-specific client to respond.
...
Chiper supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Chiper Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Chiper Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Chiper now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Chiper Agent`)
...
- Tokens should be stored in `~/.chiperflux/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Chiper config directory
