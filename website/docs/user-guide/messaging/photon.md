...
Connect Chiper to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Chiper — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Chiper runs it in a
...
2. **Finds or creates** the `Chiper Agent` project on your account.
...
Runtime credentials are written to `~/.chiperflux/.env`
...
(device token, dashboard project id) lives in `~/.chiperflux/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Chiper
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Chiper replies with a pairing code. Approve it with:

```bash
chiper pairing approve photon <CODE>
```

Use `chiper pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.chiperflux/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.chiperflux/.env`):
...
By default Chiper responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Chiper` and
`@Chiper agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Chiper will reply.

## Status & troubleshooting

```bash
chiper photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Chiper uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run chiper photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `chiper photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `chiper photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `chiper photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Chiper sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Chiper wake words  | JSON list / comma / newline regex patterns for group mentions |
