...
description: "Set up Chiper Agent as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Chiper Agent to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
> Run `chiper gateway setup` and pick **Microsoft Teams** for a guided walk-through.
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Chiper strips automatically before processing.
...
```bash
# devtunnel (Microsoft)
devtunnel create chiper-bot --allow-anonymous
devtunnel port create chiper-bot -p 3978 --protocol https  # replace 3978 with TEAMS_PORT if changed
devtunnel host chiper-bot
...
```bash
CHIPER_UID=$(id -u) CHIPER_GID=$(id -g) docker compose up -d gateway
```
...
Alternatively, configure via `~/.chiperflux/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.chiperflux/.env` |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create chiper-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs chiper` for tracebacks |
...
- Store credentials in `~/.chiperflux/.env` with permissions `600` (`chmod 600 ~/.chiperflux/.env`)
