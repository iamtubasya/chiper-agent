...
description: "Set up Chiper Agent as a WhatsApp bot via Meta's official Business Cloud API"
---

# WhatsApp Business Cloud API Setup

Chiper can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The Chiper gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not a Chiper limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. Chiper degrades gracefully if absent.
...
2. **Add** → name (e.g. `chiper-bot`) → role: **Admin**.
...
6. Copy the token → update `WHATSAPP_CLOUD_ACCESS_TOKEN` in `~/.chiperflux/.env` → restart the gateway.

System User tokens don't expire unless you explicitly revoke them.

---

## Exposing Chiper to the internet

The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the Chiper gateway has to be reachable from Meta's servers.  Three common ways:
...
2. Generate a **Verify Token** — the wizard does this for you with `secrets.token_urlsafe(32)`; if you're configuring manually, run:
   ...
   ```
   Save it as `WHATSAPP_CLOUD_VERIFY_TOKEN` in `~/.chiperflux/.env`.
3. Start the Chiper gateway: `chiper gateway`.
...
## Allowlist (Chiper-side)

In addition to Meta's recipient whitelist, Chiper has its own per-platform allowlist that controls **which incoming messages the agent processes**.  Add to `~/.chiperflux/.env`:
...
The `chiper whatsapp-cloud` wizard prints these links at the end of setup. None of this is required for the bot to work — it's pure polish for how your bot appears to users.

---

## Configuration reference

All settings live in `~/.chiperflux/.env`.  Required values are in **bold**.
...
When the agent invokes any of these flows, Chiper uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
Chiper acknowledges inbound messages immediately:
...
Chiper TTS produces MP3. Two paths:
...
Chiper warns the agent about this window in its system prompt, so the model knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in Chiper. If you need it, please [open an issue](https://github.com/NousResearch/chiper-agent/issues) — it's planned but waiting on a clear demand signal.

### Group chats

The Cloud API has limited group support (capability-tier gated by Meta).  Chiper's `whatsapp_cloud` adapter currently handles **direct messages only** in v1.  If you need group chats, use the Baileys bridge.

### Outbound rate limit

Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  Chiper doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- **Verify token mismatch** — the token in `~/.chiperflux/.env`'s `WHATSAPP_CLOUD_VERIFY_TOKEN` must match exactly what you typed into Meta's dashboard.  Run the curl probe above to confirm the gateway's verify handshake works locally first.
- **Gateway not running** — check `chiper gateway` is up.
- **App Secret not set** — without it, Chiper refuses inbound POSTs with 503.  Meta interprets that as "can't validate."
...
- Wait for template support to land in Chiper.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `chiper tools list` and verify the platform is using `chiper-whatsapp` (the default Cloud adapter toolset, same as Baileys).

If the model emits tool-call-shaped text instead of a structured call, it usually means the toolset was effectively empty.  See `chiper_cli/platforms.py` for the platform → default toolset mapping.
...
```bash
chiper config set stt.provider openai
chiper config set stt.use_gateway true
...
- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that Chiper will accept as authentic.
...
| | Baileys (`chiper whatsapp`) | Cloud API (`chiper whatsapp-cloud`) |
|---|---|---|
...
Most users running Chiper for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
