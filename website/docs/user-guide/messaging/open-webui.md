...
CHIPER_API_MODEL_NAME='My Hermes Agent' \
bash scripts/setup_open_webui.sh
```
...
`hermes config set` auto-routes the flag to `config.yaml` and the secret to `~/.chiperflux/.env`. If the gateway is already running, restart it so the change takes effect:
