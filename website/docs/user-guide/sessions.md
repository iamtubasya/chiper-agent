...
Chiper Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.chiperflux/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Chiper stores session history so it can resume conversations, but it does not
...
Chiper explicitly injects for that turn.
...
For example, if a user sends an image and asks Chiper to make a meme from it,
Chiper may inspect that image once with vision and run an image-processing
...
`chiper sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`chiper` or `chiper chat`) |
...
chiper -c "my project"   # → resumes "my project #3"
```
...
chiper --resume 20250305_091523_a1b2c3d4
chiper -r 20250305_091523_a1b2c3d4

# Resume by title
chiper --resume "refactoring auth"

# Or with the chat subcommand
chiper chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `chiper sessions list`.

### Conversation Recap on Resume

When you resume a session, Chiper displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Chiper session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.chiperflux/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `chiper -r "<title>"` from the shell) and pick up where the platform left off.
...
Chiper automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `chiper sessions list` or `chiper sessions browse`.
...
```bash
chiper sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Chiper creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`chiper -c "my project"`), it automatically picks the most recent session in the lineage.
...
Chiper provides a full set of session management commands via `chiper sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
chiper sessions list

# Filter by platform
chiper sessions list --source telegram

# Show more sessions
chiper sessions list --limit 50
```
...
chiper sessions export backup.jsonl

# Export sessions from a specific platform
chiper sessions export telegram-history.jsonl --source telegram

# Export a single session
chiper sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
chiper sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
chiper sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
chiper sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
chiper sessions prune

# Custom age threshold
chiper sessions prune --older-than 30

# Only prune sessions from a specific platform
chiper sessions prune --source telegram --older-than 60

# Skip confirmation
chiper sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`chiper insights`](/reference/cli-commands#chiper-insights).
...
When Chiper cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Chiper uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Chiper in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.chiperflux/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.chiperflux/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.chiperflux/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.chiperflux/sessions/`. They are no longer written or
read by Chiper. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Chiper process in the same `CHIPER_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.chiperflux/config.yaml`:
...
chiper sessions delete <session_id>
