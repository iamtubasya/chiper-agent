...
`~/.local/bin/chiper-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: chiper-gateways {start|stop|restart|status|list}"
}
...
```bash
chiper-gateways start      # start every configured profile
...
tail -f ~/.chiperflux/logs/gateway.error.log

# Named profile
tail -f ~/.chiperflux/profiles/<name>/logs/gateway.log
tail -f ~/.chiperflux/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.chiperflux/logs/gateway.log ~/.chiperflux/profiles/*/logs/gateway.log
```
...
```
~/.chiperflux/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.chiperflux/` directly with the same three files.
...
caffeinate -i -w $(cat ~/.chiperflux/gateway.pid) &   # awake while default gateway runs
...
`chiper-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.chiperflux/.env ~/.chiperflux/profiles/*/.env
```
...
```bash
ps -ef | grep "chiper_cli.*-p <profile>"
cat ~/.chiperflux/profiles/<profile>/gateway.pid
