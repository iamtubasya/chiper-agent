...
:::note
You cannot delete the default profile (`~/.chiperflux`). To remove everything, use `hermes uninstall`.
:::
...
- `CHIPER_HOME` is the profile boundary. It controls Hermes config, `.env`,
...
Hermes also exposes `CHIPER_REAL_HOME` to subprocesses so scripts can still find
