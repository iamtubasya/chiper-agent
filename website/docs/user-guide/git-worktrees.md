...
title: "Git Worktrees"
description: "Run multiple Chiper agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Chiper Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Chiper so each session has a clean, isolated working directory.

## Why Use Worktrees with Chiper?

Chiper treats the **current working directory** as the project root:

- CLI: the directory where you run `chiper` or `chiper chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.chiperflux/config.yaml`
...
git worktree add ../repo-feature feature/chiper-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/chiper-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Chiper there:

```bash
cd ../repo-feature

# Start Chiper in the worktree
chiper
```
...
git worktree add ../repo-experiment-a feature/chiper-a
git worktree add ../repo-experiment-b feature/chiper-b
```
...
- Works on its own branch (`feature/chiper-a` vs `feature/chiper-b`).
...
- Chiper checkpoint data under `~/.chiperflux/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Chiper experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/chiper-checkpoints-docs`, `feature/chiper-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Chiper from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `chiper -w` (Automatic Worktree Mode)

Chiper has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `chiper/chiper-<hash>`).
...
```bash
chiper -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `chiper -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Chiper session its own clean checkout.
