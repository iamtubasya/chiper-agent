...
hermes curator backup         # take a manual snapshot of ~/.chiperflux/skills/
...
hermes curator list-archived    # list skills currently in ~/.chiperflux/skills/.archive/
...
Before every real curator pass, Hermes takes a tar.gz snapshot of `~/.chiperflux/skills/` at `~/.chiperflux/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:
