...
Type `@` followed by a reference to inject content directly into your message. Chiper expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Chiper env: `$CHIPER_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$CHIPER_HOME/skills/.hub/`
