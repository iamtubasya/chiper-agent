...
Chiper runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Chiper
...
1. Chiper captures a baseline of current diagnostics for the file.
...
…). Chiper auto-detects the binary on PATH or in
`<CHIPER_HOME>/lsp/bin/`.
...
`node_modules` tree — Chiper installs both packages together when you
run `chiper lsp install typescript` or auto-install fires on first
use.

## CLI

```
chiper lsp status          # service state + per-server install status
chiper lsp list            # registry, optionally --installed-only
chiper lsp install <id>    # eagerly install one server
chiper lsp install-all     # try every server with a known recipe
chiper lsp restart         # tear down running clients
chiper lsp which <id>      # print resolved binary path
```

`chiper lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <CHIPER_HOME>/lsp/bin
...
When `install_strategy: auto`, Chiper installs binaries into
`<CHIPER_HOME>/lsp/bin/`. NPM packages land in
`<CHIPER_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Chiper-owned and is
...
Servers are kept alive for the life of the Chiper process. There's
...
**`chiper lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<CHIPER_HOME>/lsp/bin/`. Run
`chiper lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `chiper lsp status`**
...
When `chiper lsp status` shows a `Backend warnings` section, install
...
`~/.chiperflux/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.chiperflux/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `chiper lsp restart` to clear the set;
