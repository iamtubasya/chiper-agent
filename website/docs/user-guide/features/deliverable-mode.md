...
When Chiper Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.chiperflux/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.chiperflux/config.yaml` (switchable per session
via `/personality`).
...
If you use Chiper' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.chiperflux/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Chiper Agent's deliverable
