...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Chiper's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Chiper instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
chiper memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `chiper` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Chiper syncs them back at session init.
...
The `chiper honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `chiper memory setup honcho` (or run `chiper memory setup` and pick it from the list); the `chiper honcho` subcommand then appears on the next invocation.

```bash
chiper memory setup honcho    # Configure Honcho directly (works before activation)
chiper honcho status          # Connection status, config, and key settings
chiper honcho setup           # Redirects to `chiper memory setup` (post-activation alias)
chiper honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
chiper honcho peer            # Show or update peer names + dialectic reasoning level
chiper honcho mode            # Show or set recall mode (hybrid/context/tools)
chiper honcho tokens          # Show or set token budget for context and dialectic
chiper honcho identity        # Seed or show the AI peer's Honcho identity
chiper honcho sync            # Sync Honcho config to all existing profiles
chiper honcho peers           # Show peer identities across all profiles
chiper honcho sessions        # List known Honcho session mappings
chiper honcho map             # Map current directory to a Honcho session name
chiper honcho enable          # Enable Honcho for the active profile
chiper honcho disable         # Disable Honcho for the active profile
chiper honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `chiper honcho`

If you previously used the standalone `chiper honcho setup`:
...
No re-login or re-setup needed. Run `chiper memory setup` and select "honcho" — the wizard detects your existing config.
