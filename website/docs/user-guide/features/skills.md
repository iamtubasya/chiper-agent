...
```bash
curl -fsSL https://chiper-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```
...
Bundles live in **`~/.chiperflux/skill-bundles/<slug>.yaml`** and look like this:
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.chiperflux/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.chiperflux/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.chiperflux/pending/skills/` and are
...
If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.chiperflux/.hub/taps.json`:
