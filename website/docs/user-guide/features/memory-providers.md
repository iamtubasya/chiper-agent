...
See [plugin README](https://github.com/NousResearch/chiper-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$CHIPER_HOME/memory_store.db` | SQLite database path |
...
- Knowledge tree stored at `$CHIPER_HOME/byterover/` (profile-scoped)
...
**Config:** `$CHIPER_HOME/supermemory.json`
...
- **Local storage providers** (Holographic, ByteRover) use `$CHIPER_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$CHIPER_HOME/` so each profile has its own credentials
