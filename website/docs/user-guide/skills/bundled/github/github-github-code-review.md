...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.chiperflux/.env ] && grep -q "^GITHUB_TOKEN=" ~/.chiperflux/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.chiperflux/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${CHIPER_HOME:-$HOME/.chiper}/skills/github/github-auth/scripts/gh-env.sh"
