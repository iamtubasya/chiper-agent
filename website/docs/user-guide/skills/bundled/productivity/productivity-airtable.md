...
4. Store the token in `~/.chiperflux/.env` (or via `chiper setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.chiperflux/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
