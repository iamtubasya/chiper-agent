...
| | |
|---|---|
| Source | Optional — install with `chiper skills install official/blockchain/hyperliquid` |
...
The script reads `~/.chiperflux/.env` for two optional defaults:
...
Helper script: `~/.chiperflux/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `~/.chiperflux/.env`.
