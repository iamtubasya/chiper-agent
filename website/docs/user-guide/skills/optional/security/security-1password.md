...
| | |
|---|---|
| Source | Optional — install with `chiper skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Chiper Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Chiper terminal calls (desktop app flow only)
...
### Service Account (recommended for Chiper)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.chiperflux/.env` (the skill will prompt for this on first load).
...
## Chiper Execution Pattern (desktop app flow)

Chiper terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/chiper-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/chiper-op.sock"
