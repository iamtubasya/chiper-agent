...
| | |
|---|---|
| Source | Optional — install with `chiper skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${CHIPER_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${CHIPER_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `chiper`)    |
