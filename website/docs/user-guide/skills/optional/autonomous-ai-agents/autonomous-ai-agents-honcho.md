...
description: "Configure and use Honcho memory with Chiper -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Chiper -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `chiper skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Chiper

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Chiper profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/chiper#running-honcho-locally-with-chiper

### Verify

```bash
chiper honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Chiper profile's AI peer
...
Honcho models conversations as interactions between **peers**. Chiper creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Chiper instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Chiper run |
| `global` | Single session across all directories |

Manual override: `chiper honcho map my-project-name`
...
Each Chiper profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block chiper.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `chiper.coder` host block in `honcho.json`
...
```bash
chiper honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Chiper when Honcho memory is active.
...
Config file: `$CHIPER_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `chiper honcho setup`. Ensure `memory.provider: honcho` is in `~/.chiperflux/config.yaml`.

### Memory not persisting across sessions
Check `chiper honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `chiper profile create <name> --clone`. For existing profiles: `chiper honcho sync`.
...
| Command | Description |
|---------|-------------|
| `chiper honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `chiper honcho status` | Show resolved config, connection test, peer info for active profile |
| `chiper honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `chiper honcho disable` | Disable Honcho for the active profile |
| `chiper honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `chiper honcho peers` | Show peer identities across all profiles |
| `chiper honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `chiper honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `chiper honcho sessions` | List known directory-to-session-name mappings |
| `chiper honcho map <name>` | Map current working directory to a Honcho session name |
| `chiper honcho identity` | Seed AI peer identity or show both peer representations |
| `chiper honcho sync` | Create host blocks for all Chiper profiles that don't have one yet |
| `chiper honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Chiper + Honcho |
| `chiper memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `chiper memory status` | Show active memory provider and config |
| `chiper memory off` | Disable external memory provider |
