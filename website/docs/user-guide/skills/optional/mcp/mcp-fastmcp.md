...
| | |
|---|---|
| Source | Optional — install with `chiper skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Chiper or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Chiper. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.chiperflux/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.chiperflux/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Chiper integration, either:

- configure the server in `~/.chiperflux/config.yaml` using the `native-mcp` skill, or
...
### Chiper cannot see the deployed server

The server-building part may be correct while the Chiper config is not. Load the `native-mcp` skill and configure the server in `~/.chiperflux/config.yaml`, then restart Chiper.
