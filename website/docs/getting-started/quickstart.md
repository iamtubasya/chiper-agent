...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `chiper_cli/models.py` may change between releases) | `hermes model` → MiniMax (OAuth) |
...
# ~/.chiperflux/chiper-agent on Linux/macOS or %LOCALAPPDATA%\hermes\chiper-agent on Windows):
...
Hermes ships with a catalog of bundled skills already installed in `~/.chiperflux/skills/`. You can add more from the Skills Hub, or write your own.
