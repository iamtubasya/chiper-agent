...
```bash
nix profile install github:NousResearch/chiper-agent#messaging
```

For all optional extras (voice, all providers, all platforms):

```bash
nix profile install github:NousResearch/chiper-agent#full
```
...
:::tip
Environment variables in `env` values are resolved from `$CHIPER_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
Tokens are stored in `$CHIPER_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.
...
For optional extras declared in chiper-agent's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set — on Nix, runtime installation into the read-only store is not possible.

```nix
# Enable Discord, Telegram, Slack
services.chiper-agent.extraDependencyGroups = [ "messaging" ];
```
...
nix build .#checks.x86_64-linux.managed-guard       # CHIPER_MANAGED blocks mutation
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$CHIPER_HOME/.env` at activation time |
