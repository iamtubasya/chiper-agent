---
sidebar_position: 8
title: "Use Voice Mode with Chiper"
description: "A practical guide to setting up and using Chiper voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Chiper
...
- you want Chiper sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Chiper.
...
## Step 1: make sure normal Chiper works first
...
Chiper supports both local and cloud speech stacks.
...
### If you use `chiper setup`

If you choose NeuTTS in the setup wizard, Chiper checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Chiper transcribes and responds
...
- asking Chiper to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Chiper loop.

## Tuning CLI behavior

### Silence threshold

If Chiper starts/stops too aggressively, tune:
...
Chiper stays a normal chat bot, but can speak replies.
...
- you want Chiper to function like a portable research or ops assistant
...
Chiper joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Chiper detects speech boundaries
- transcripts are posted in the associated text channel
- Chiper responds in text and audio
...
1. get text Chiper working
2. install `chiper-agent[voice]`
