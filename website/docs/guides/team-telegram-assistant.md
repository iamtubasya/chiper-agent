...
This tutorial walks you through setting up a Telegram bot powered by Chiper Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Chiper Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.chiperflux/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Chiper itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Chiper Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_chiper_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Chiper Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.chiperflux/.env`:
...
```
[Gateway] Starting Chiper Gateway...
...
# View live logs
journalctl --user -u chiper-gateway -f
...
chiper gateway start
chiper gateway stop
tail -f ~/.chiperflux/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `chiper gateway install` to update the plist.
:::
...
```bash
chiper gateway stop && chiper gateway start
```
...
chiper pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.chiperflux/.env`:
...
Control how much detail the bot shows when using tools. In `~/.chiperflux/config.yaml`:
...
Customize how the bot communicates by editing `~/.chiperflux/SOUL.md`:

For a full guide, see [Use SOUL.md with Chiper](/guides/use-soul-with-chiper).
...
```bash
# From the CLI
chiper cron list          # View all scheduled jobs
chiper cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u chiper-gateway` (Linux) or `~/.chiperflux/logs/gateway.log` (macOS) |
| Cron job output | `~/.chiperflux/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.chiperflux/cron/jobs.json` |
| Pairing data | `~/.chiperflux/pairing/` |
| Session history | `~/.chiperflux/sessions/` |
