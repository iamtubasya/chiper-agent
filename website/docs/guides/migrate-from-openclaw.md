...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.chiperflux/` (by default a single restore-point archive is written before apply, under `~/.chiperflux/backups/pre-migration-*.zip`; restorable with `hermes import`). |
