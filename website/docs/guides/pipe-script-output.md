...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `chiper send`."
---

# Pipe Script Output to Messaging Platforms

`chiper send` is a small, scriptable CLI that pushes a message to any
messaging platform Chiper is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | chiper send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `chiper
...
chiper send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
chiper send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
chiper send --list

# Filter by platform
chiper send --list telegram
```
...
Any platform Chiper ships adapters for works as a target:
...
`chiper send` resolves the message body in this order:

1. **Positional argument** — `chiper send --to telegram "hi"`
2. **`--file PATH`** — `chiper send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | chiper send --to telegram`

When stdin is a TTY (no pipe), Chiper does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  chiper send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `chiper send` reuses your Chiper config, the same script works on
any host where Chiper is installed — no need to export bot tokens into
...
`chiper send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  chiper send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | chiper send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.chiperflux/bin/chiper send \
...
```bash
./train.py --epochs 200 && \
  chiper send --to telegram "training done" || \
  chiper send --to telegram "training failed (exit $?)"
```
...
chiper send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(chiper send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `chiper send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `chiper send` calls
the platform's REST endpoint directly using credentials from
`~/.chiperflux/.env` and `~/.chiperflux/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `chiper gateway start` and retry.
...
# Machine-readable
chiper send --list --json
```

The listing is built from `~/.chiperflux/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`chiper gateway
...
| Approach | Multi-platform | Reuses Chiper creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `chiper send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`chiper send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `chiper send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `chiper send` shares with cron delivery.
