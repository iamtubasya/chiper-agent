...
```bash
chiper teams-pipeline validate
```
...
```bash
chiper teams-pipeline token-health
chiper teams-pipeline token-health --force-refresh
```
...
```bash
chiper teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
chiper teams-pipeline maintain-subscriptions
chiper teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Chiper cron (recommended if you already run the Chiper gateway)

Chiper ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.chiperflux/scripts/`. First create the script:

```bash
mkdir -p ~/.chiperflux/scripts
cat > ~/.chiperflux/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec chiper teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.chiperflux/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
chiper cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/chiper-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Chiper Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=chiper
EnvironmentFile=/etc/chiper/env
ExecStart=/usr/local/bin/chiper teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/chiper-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Chiper Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now chiper-teams-pipeline-maintain.timer
systemctl list-timers chiper-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/chiper teams-pipeline maintain-subscriptions >> /var/log/chiper/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.chiperflux/.env` at the top of a wrapper script that crontab calls.
...
```bash
chiper teams-pipeline subscriptions   # should show expirationDateTime advanced
chiper teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
chiper teams-pipeline list
chiper teams-pipeline list --status failed
chiper teams-pipeline show <job-id>
```

### Replay a stored job

```bash
chiper teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
chiper teams-pipeline fetch --meeting-id <meeting-id>
chiper teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `chiper teams-pipeline maintain-subscriptions --dry-run`
- inspect `chiper teams-pipeline list --status failed`
...
- run `chiper teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `chiper teams-pipeline run`
...
- [ ] `chiper teams-pipeline validate` returns an OK snapshot
- [ ] `chiper teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Chiper cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
