---
sidebar_position: 6
title: "Use MCP with Chiper"
description: "A practical guide to connecting MCP servers to Chiper Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Chiper

This guide shows how to actually use MCP with Chiper Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Chiper tool
- you want Chiper to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Chiper to internal APIs, databases, or company systems without modifying Chiper core

Do not use MCP when:
- a built-in Chiper tool already solves the job well
...
- Chiper remains the agent
- MCP servers contribute tools
- Chiper discovers those tools at startup or reload time
...
If you installed Chiper with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Chiper banner/status should show MCP integration when configured
- ask Chiper what tools it has available
...
## WSL2: bridge Chiper in WSL to Windows Chrome

This is the practical setup when:

- Chiper runs inside WSL2
...
In this setup, Chiper does **not** connect to Chrome directly. Instead:

- Chiper runs in WSL
- Chiper starts a local stdio MCP server
...
```text
Chiper (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Chiper stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Chiper core browser transport
...
```bash
chiper mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
chiper mcp test chrome-devtools-win
```

Then start a fresh Chiper session or run:
...
Once loaded, Chiper can use the MCP-prefixed browser tools directly. For example:
...
If Chiper runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Chiper from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Chiper from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Chiper:
...
2. Chiper-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Chiper will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Chiper to reason over a bounded workspace.
...
Now Chiper can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Chiper core.
...
Because Chiper now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
