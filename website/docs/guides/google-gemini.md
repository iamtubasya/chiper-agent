...
After running `hermes model`, your `~/.chiperflux/config.yaml` will contain:
...
Hermes could not find a usable API key. Add one of these to `~/.chiperflux/.env`:
...
Hermes may hide low-throughput Gemma models from the picker by default. If you intentionally want to evaluate one, set the model ID directly in `~/.chiperflux/config.yaml`.
