---
sidebar_position: 7
title: "Use SOUL.md with Chiper"
description: "How to use SOUL.md to shape Chiper Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Chiper

`SOUL.md` is the **primary identity** for your Chiper instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Chiper to feel like the same assistant every time you talk to it — or if you want to replace the Chiper persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Chiper should be
- what Chiper should avoid stylistically
- how Chiper should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Chiper is and how Chiper speaks
...
Chiper now uses only the global SOUL file for the current instance:

```text
~/.chiperflux/SOUL.md
```

If you run Chiper with a custom home directory, it becomes:
...
Chiper automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Chiper does not overwrite it
- if the file exists but is empty, Chiper adds nothing from it to the prompt

## How Chiper uses it

When Chiper starts a session, it reads `SOUL.md` from `CHIPER_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Chiper feels.
...
Chiper already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Chiper should sound.

# Avoid
What Chiper should not do.

# Defaults
How Chiper should behave when ambiguity appears.
```
...
```bash
nano ~/.chiperflux/SOUL.md
```

or

```bash
vim ~/.chiperflux/SOUL.md
```

Then restart Chiper or start a new session.
...
### I edited SOUL.md but Chiper still sounds the same

Check:
- you edited `~/.chiperflux/SOUL.md` or `$CHIPER_HOME/SOUL.md`
...
### Chiper is ignoring parts of my SOUL.md
