# OpenAI-Compatible API Server for Chiper Agent
...
SSE streaming. By exposing this endpoint, chiper-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  chiper-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.chiperflux/.env`
2. Run `chiper gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with chiper-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns chiper-agent as a model) |
...
Authorization: Bearer chiper-api-key-here
...
But chiper-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "chiper-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured chiper-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of chiper-agent's core prompt
...
Once implemented, chiper-agent works as a drop-in backend for:
