...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `CHIPER_HOME`. Mocks hide integration bugs.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.chiperflux/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.chiperflux/chiper-agent/venv` (for worktrees that share a venv with the
main checkout).
...
1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `chiper_cli/commands.py`:
```python
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `chiper_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `chiper_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
The skin engine (`chiper_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
chiper_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.chiperflux/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
Add to `_BUILTIN_SKINS` dict in `chiper_cli/skin_engine.py`:
...
Users create `~/.chiperflux/skins/<name>.yaml`:
...
### General plugins (`chiper_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.chiperflux/plugins/`, `./.chiperflux/plugins/`,
...
`post_setup(chiper_home, config)` for setup-wizard integration.
...
2. User: `$CHIPER_HOME/plugins/model-providers/<name>/`
...
lives in the `chiper-agent-dev` skill at
...
go to `~/.chiperflux/skills/.archive/` and are restorable.
...
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.chiperflux/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
- File lock at `~/.chiperflux/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
- **Board** is the hard boundary — workers are spawned with
  `CHIPER_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
in config.yaml (or `CHIPER_BACKGROUND_NOTIFICATIONS` env var):
...
`CHIPER_HOME` before any module imports. All `get_chiper_home()` references
...
2. **Use `display_chiper_home()` for user-facing messages.** Import from `chiper_constants`.
   ...
   print(f"Config saved to {display_chiper_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.chiperflux/config.yaml")
   ```
...
Existing call sites in `chiper_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `chiper_cli/curses_ui.py` — see
`chiper_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `CHIPER_HOME`.

### Tests must not write to `~/.chiperflux/`
The `_isolate_chiper_home` autouse fixture in `tests/conftest.py` redirects `CHIPER_HOME` to a temp dir. Never hardcode `~/.chiperflux/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_chiper_home()` resolve within the temp dir.
Use the pattern from `tests/chiper_cli/test_profiles.py`:
```python
@pytest.fixture
...
- The plugin disables itself in child processes (sentinel envvar
  `CHIPER_ISOLATE_CHILD=1`), so there's no fork-bomb risk.
...
| HOME / `~/.chiperflux/` | Your real config+auth.json | Temp dir per test |
