...
## 5. Chiper Configuration

### Config location

`$CHIPER_HOME/config.yaml` (defaults to `~/.chiperflux/config.yaml` when `CHIPER_HOME` is unset)
...
Restart the Chiper session for changes to take effect. The MCP connection is
