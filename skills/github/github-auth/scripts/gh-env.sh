#!/usr/bin/env bash
# GitHub environment detection helper for Chiper Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _chiper_env="${CHIPER_HOME:-$HOME/.chiperflux}/.env"; [ -f "$_chiper_env" ] && grep -q "^GITHUB_TOKEN=" "$_chiper_env" 2>/dev/null; then
