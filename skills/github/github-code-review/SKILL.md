...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _chiper_env="${CHIPER_HOME:-$HOME/.chiperflux}/.env"; [ -f "$_chiper_env" ] && grep -q "^GITHUB_TOKEN=" "$_chiper_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_chiper_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Chiper Agent\",
    \"comments\": [
...
```bash
source "${CHIPER_HOME:-$HOME/.chiperflux}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Chiper Agent. Code looks clean — good test coverage, no security concerns."
