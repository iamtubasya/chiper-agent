#!/usr/bin/env python3
"""Google Workspace API CLI for Chiper Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Chiper-facing JSON contract and falls back to the Python client
...
from _chiper_home import get_chiper_home
...
def _gws_binary() -> str | None:
    override = os.getenv("CHIPER_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Chiper Agent")
