...
`hermes dashboard` on port 9119 serves the **built** bundle from `chiper_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
