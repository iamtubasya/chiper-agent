"""
Chiper MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Chiper-specific extra)

Usage:
    chiper mcp serve
    chiper mcp serve --verbose
...
logger = logging.getLogger("chiper.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using CHIPER_HOME."""
    try:
        from chiper_constants import get_chiper_home
        return get_chiper_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("CHIPER_HOME", Path.home() / ".chiperflux")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from chiper_constants import get_chiper_home
        directory_file = get_chiper_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("CHIPER_HOME", Path.home() / ".chiperflux")
...
class EventBridge:
    ...
    This is the Chiper equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from chiper_constants import get_chiper_home
            db_file = get_chiper_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("CHIPER_HOME", Path.home() / ".chiperflux")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Chiper MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "chiper",
        instructions=(
            "Chiper Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Chiper MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
