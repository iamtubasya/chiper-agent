#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Chiper Skills Hub.
...
Used by chiper_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            chiper_meta = metadata.get("chiper", {})
            if isinstance(chiper_meta, dict):
                tags = chiper_meta.get("tags", [])
        if not tags:
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                chiper_meta = meta_block.get("chiper", {})
                if isinstance(chiper_meta, dict):
                    tags = chiper_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Chiper centralized index source
# ---------------------------------------------------------------------------

CHIPER_INDEX_URL = "https://chiper-agent.nousresearch.com/docs/api/skills-index.json"
CHIPER_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "chiper-index.json"
CHIPER_INDEX_TTL = 6 * 3600  # 6 hours


def _load_chiper_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for CHIPER_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if CHIPER_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - CHIPER_INDEX_CACHE_FILE.stat().st_mtime
            if age < CHIPER_INDEX_TTL:
                return json.loads(CHIPER_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(CHIPER_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Chiper index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Chiper index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        CHIPER_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        CHIPER_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class ChiperIndexSource(SkillSource):
    """Skill source backed by the centralized Chiper Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_chiper_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "chiper-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "chiper-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ChiperIndexSource(auth=auth), # Centralized index (search + resolved install paths)
