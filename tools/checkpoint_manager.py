...
-----------------------------------------------------------------------------
    ...
    ~/.chiperflux/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/chiper/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_chiper_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/chiper"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Chiper convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("CHIPER_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal Chiper infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``chiper checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `chiper checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "chiper@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Chiper Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``CHIPER_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at CHIPER_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "CHIPER_WORKDIR").write_text(
    ...
    * its ``last_touch`` is older than ``retention_days`` days.
    ...
    for child in base.iterdir():
        ...
        if delete_orphans:
            ...
            wd_marker = child / "CHIPER_WORKDIR"
            if wd_marker.exists():
                try:
...
# ---------------------------------------------------------------------------
# Public helpers for `chiper checkpoints` CLI
# ---------------------------------------------------------------------------
