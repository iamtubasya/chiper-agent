...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``chiper tools`` repaint, tool-registration scans,
    ...
    2. ``~/.chiperflux/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_chiper_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.chiperflux/.env`` first, then ``os.environ``.

    Wraps :func:`chiper_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from chiper_cli.config import get_env_value as _chiper_get_env_value

        value = _chiper_get_env_value(name)
...
def chiper_xai_user_agent() -> str:
    """Return a stable Chiper-specific User-Agent for xAI HTTP calls."""
    try:
        from chiper_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Chiper-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Chiper-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``chiper_cli.config.get_env_value`` so keys
    stored in ``~/.chiperflux/.env`` (the standard Chiper location) are honored —
