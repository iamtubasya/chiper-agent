...
transport, discovers their tools, and registers them into the chiper-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.chiperflux/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.chiperflux/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            chiper_home = os.path.expanduser(
                os.getenv(
                    "CHIPER_HOME", os.path.join(os.path.expanduser("~"), ".chiperflux")
                )
            )
            candidates = [
                os.path.join(chiper_home, "node", "bin", resolved_command),
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        # ~/.chiperflux/logs/mcp-stderr.log.
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local CHIPER_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_chiper_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_chiper_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local CHIPER_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_chiper_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.chiperflux/.env`` loaded at startup).
    """
    try:
        ...
        if _env_enabled("CHIPER_SAFE_MODE"):
            return {}
        config = load_config()
