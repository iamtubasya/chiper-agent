...
One ``CDPSupervisor`` runs per Chiper ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "chiper-dialog-bridge.invalid"
...
(() => {
  if (window.__chiperDialogBridgeInstalled) return;
  window.__chiperDialogBridgeInstalled = true;
  const ENDPOINT = "http://chiper-dialog-bridge.invalid/";
