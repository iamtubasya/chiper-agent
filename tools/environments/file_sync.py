...
_FORCE_SYNC_ENV = "CHIPER_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.chiperflux") -> list[tuple[str, str]]:
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.chiperflux", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``CHIPER_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, chiper_home: Path | None = None) -> None:
        ...
        lock_path = (chiper_home or get_chiper_home()) / ".sync.lock"
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.chiperflux/skills/a.md`` →
        ``~/.chiperflux/skills/a.md``, a new remote file at
        ``/root/.chiperflux/skills/b.md`` maps to ``~/.chiperflux/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
