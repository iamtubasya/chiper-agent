...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("CHIPER_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("CHIPER_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("CHIPER_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set CHIPER_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind CHIPER_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds CHIPER_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("CHIPER_CRON_SESSION"):
        return False
...
# ~/.chiperflux/config.yaml IS the security policy: approvals.mode, yolo, and the
...
DANGEROUS_PATTERNS = [
    ...
    # that should always require user consent, just like `chiper gateway restart`
    ...
    # In-place edit of a Chiper-managed security file (~/.chiperflux/config.yaml or
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    command = _rewrite_resolved_chiper_home(command)
    return command
...
def _rewrite_resolved_chiper_home(command: str) -> str:
    """Rewrite the resolved absolute Chiper home prefix to ``~/.chiperflux/``.
    ...
    try:
        ...
        home = get_chiper_home().expanduser()
        candidates = [
            str(home).rstrip("/"),
    ...
    for path in candidates:
        ...
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.chiperflux/")
    return command
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("CHIPER_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set CHIPER_INTERACTIVE or CHIPER_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("CHIPER_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("CHIPER_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("CHIPER_EXEC_ASK")
