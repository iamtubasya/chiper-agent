...
# 5. Symlinks the 'chiper' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Chiper Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/chiper-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink chiper into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up chiper command..."

CHIPER_BIN="$SCRIPT_DIR/venv/bin/chiper"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$CHIPER_BIN" "$COMMAND_LINK_DIR/chiper"
echo -e "${GREEN}✓${NC} Symlinked chiper → $COMMAND_LINK_DISPLAY_DIR/chiper"
...
# ============================================================================
# Seed bundled skills into ~/.chiperflux/skills/
# ============================================================================

CHIPER_SKILLS_DIR="${CHIPER_HOME:-$HOME/.chiperflux}/skills"
mkdir -p "$CHIPER_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.chiperflux/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$CHIPER_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  chiper status        # Check configuration"
if is_termux; then
    echo "  chiper gateway       # Run gateway in foreground"
else
    echo "  chiper gateway install # Install gateway service (messaging + cron)"
fi
echo "  chiper cron list     # View scheduled jobs"
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m chiper_cli.main setup
fi
