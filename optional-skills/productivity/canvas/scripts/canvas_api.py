#!/usr/bin/env python3
"""Canvas LMS API CLI for Chiper Agent.
...
def _check_config():
    ...
    if missing:
        chiper_env = os.path.join(
            os.environ.get("CHIPER_HOME", os.path.expanduser("~/.chiperflux")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {chiper_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Chiper Agent"
    )
