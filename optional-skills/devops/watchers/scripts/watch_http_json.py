...
Usage (via cron with --no-agent):
    ...
    chiper cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$CHIPER_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Chiper-Watcher/1.0"})
