...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("CHIPER_HOME", os.path.expanduser("~/.chiperflux")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("CHIPER_HOME", Path.home() / ".chiperflux")) / "skills" / "red-teaming" / "godmode"
...
CHIPER_HOME = Path(os.getenv("CHIPER_HOME", Path.home() / ".chiperflux"))
CONFIG_PATH = CHIPER_HOME / "config.yaml"
PREFILL_PATH = CHIPER_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Chiper models — already uncensored, just needs clean prompt
    "chiper": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "chiper" in model_lower or "nous" in model_lower:
        return "chiper"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Chiper config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.chiperflux/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Chiper for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Chiper for changes to take effect.")
