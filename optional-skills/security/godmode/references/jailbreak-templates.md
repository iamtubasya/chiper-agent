...
## 5. GODMODE FAST — Chiper 4 405B (Zero Refusal)

**Model:** `nousresearch/chiper-4-405b`
**Strategy:** Chiper models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Chiper Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.chiperflux/config.yaml`:
