...
Maps each role archetype to the Chiper skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public chiper-agent
...
## Chiper skills relevant to video production

### Visual / rendering skills (`chiper-agent/skills/creative/`)
...
### Design / pre-production skills (`chiper-agent/skills/creative/`)
...
### Audio / media skills (`chiper-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`chiper-agent/skills/devops/`)
...
These are **not** Chiper skills but external CLIs / APIs that profiles invoke.
...
## Built-in Chiper tools for media review

These are native Chiper tools — not invoked via terminal but through their own
...
key is present in `${CHIPER_HOME:-~/.chiperflux}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Chiper profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${CHIPER_HOME:-~/.chiperflux}/.env` → environment variable.
...
When a new Chiper-public video skill ships:
