...
chiper kanban watch --tenant <project-slug>

# Snapshot of the board
chiper kanban list --tenant <project-slug>
chiper kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
chiper kanban stats --tenant <project-slug>

# Visual dashboard (browser)
chiper dashboard

# Inspect a specific task (includes comments + events)
chiper kanban show <task-id>

# Follow a single task's event stream
chiper kanban tail <task-id>
```

Verify available subcommands with `chiper kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `chiper kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `chiper kanban block <id>` then `chiper kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `chiper kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `chiper kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `chiper profile list` (profile exists?), `chiper status` (gateway/dispatcher up?), and verify tenant. |
...
chiper kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
chiper kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(chiper kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
chiper kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
chiper kanban block <task-id>
chiper kanban archive <task-id>

# Diagnose what happened
chiper kanban show <task-id>      # task body, comments, recent events
chiper kanban tail <task-id>      # follow the live event stream
chiper kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `chiper kanban create` for the director
...
while true; do
    clear
    chiper kanban list --tenant <slug>
    echo "---"
    chiper kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `chiper kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `chiper profile list` and start
