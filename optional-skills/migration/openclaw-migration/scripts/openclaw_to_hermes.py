...
# become ``~/.chiperflux`` — the real Hermes home — not the broken ``~/.Hermes``).
...
def _case_preserving_replacement(replacement: str):
    ...
    ``~/.chiperflux/config.yaml`` (the real Hermes home) instead of the broken
    ``~/.Hermes/config.yaml``.
    """
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target", default=os.environ.get("CHIPER_HOME") or str(Path.home() / ".chiperflux"), help="Hermes home directory")
    parser.add_argument(
        "--workspace-target",
