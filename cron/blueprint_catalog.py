...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``chiper://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``chiper://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"chiper://blueprint/{quote(blueprint.key)}{qs}"
