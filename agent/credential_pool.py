...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``chiper model`` / ``chiper auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Chiper process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``chiper`` invocation that defaults to the active provider
        (e.g. setup wizard, ``chiper auth status``) would land on
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("chiper_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Chiper
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("chiper_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another Chiper process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Chiper profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `chiper model` / `chiper auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `chiper model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``chiper auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `chiper auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `chiper auth remove <provider> <N>` is stable across all source types.
    try:
        from chiper_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Chiper PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `chiper setup` ("Claude
        ...
        # Claude Code CLI, chiper_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.chiperflux/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `chiper setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"chiper_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_chiper_oauth_credentials

        for source_name, creds in (
            ("chiper_pkce", read_chiper_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `chiper auth add nous --label <name>`).  Fall back to the
        ...
        # Chiper auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from chiper_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.chiperflux/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `chiper auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `chiper auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Chiper auth store.  Without this gate the removal is instantly
        ...
        # Chiper owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``chiper model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``chiper auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.chiperflux/.env over os.environ — the user's config file is the
    # authoritative source for Chiper credentials. Stale env vars from parent
    ...
    # Honour user suppression — `chiper auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.chiperflux/.env.
    ...
    try:
        from chiper_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from chiper_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.chiperflux/.env over os.environ
