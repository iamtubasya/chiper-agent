...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - chiper_home (str): The active CHIPER_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.chiperflux``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "chiper").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'chiper memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], chiper_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'chiper memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``chiper_home`` is the active CHIPER_HOME directory path.
