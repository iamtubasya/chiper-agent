...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_CHIPER_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Chiper expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``chiper_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_chiper_home()`` themselves.
        """
        if "chiper_home" not in kwargs:
            from chiper_constants import get_chiper_home
            kwargs["chiper_home"] = str(get_chiper_home())
        for provider in self._providers:
            try:
