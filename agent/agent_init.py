...
from chiper_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: chiper config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.chiperflux/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from chiper_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind CHIPER_DEV_CREDITS.
    ...
    # both live under ~/.chiperflux/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from chiper_logging import setup_logging, setup_verbose_logging
    setup_logging(chiper_home=_ra()._chiper_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that chiper_logging.setup_logging() does not
        ...
        # handler level inside chiper_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from chiper_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from chiper_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `chiper model` to "
                        "select a provider, or run `chiper setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff CHIPER_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["CHIPER_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.chiperflux/sessions/ alongside gateway sessions
    chiper_home = get_chiper_home()
    agent.logs_dir = chiper_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.chiperflux/sessions/session_{sid}.json)
    ...
    try:
        from chiper_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from chiper_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "chiper"
                    except Exception:
                        pass
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from chiper_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from chiper_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by ChiperFlux Agent.  Choose a model with at least "
