...
Storage
-------
Bundles live in ``~/.chiperflux/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``CHIPER_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``chiper bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under CHIPER_HOME.

    Honors ``CHIPER_BUNDLES_DIR`` for tests; falls back to
    ``<CHIPER_HOME>/skill-bundles``.
    """
    override = os.environ.get("CHIPER_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_chiper_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `chiper bundles` CLI subcommand.
# ---------------------------------------------------------------------------
