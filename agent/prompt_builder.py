...
from chiper_constants import get_chiper_home, get_skills_dir, is_wsl
...
_CHIPER_MD_NAMES = (".chiper.md", "CHIPER.md")


def _find_chiper_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.chiper.md`` or ``CHIPER.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are ChiperFlux Agent, an intelligent AI assistant created by Nous Research. "
...
CHIPER_AGENT_HELP_GUIDANCE = (
    "You run on ChiperFlux Agent (by Nous Research). When the user needs help with "
    "Chiper itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://chiper-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `chiper-agent` skill with skill_view(name='chiper-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.chiperflux/kanban.db`. Your task id is in "
    "`$CHIPER_KANBAN_TASK`; your workspace is `$CHIPER_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $CHIPER_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `chiper kanban <verb>` for board operations. Use "
...
#
# The chiper-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to chiper-agent's Python
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Chiper "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Chiper WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Chiper itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Chiper runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Chiper itself is running. The host OS, home, and cwd "
                f"of the Chiper process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Chiper "
    ...
    # Chiper desktop GUI — any agent running under the desktop app should know
    # it. CHIPER_DESKTOP marks the backend powering the chat; CHIPER_DESKTOP_TERMINAL
    # marks a chiper launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("CHIPER_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("CHIPER_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Chiper desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Chiper
    ...
    extra = (os.getenv("CHIPER_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_chiper_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.chiperflux/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("CHIPER_PLATFORM")
        or get_session_env("CHIPER_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot ChiperFlux Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `chiper-agent` skill "
            "first. It has the actual commands (e.g. `chiper config set …`, `chiper tools`, "
            "`chiper setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: chiper setup, chiper setup tools, chiper setup terminal, chiper status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from CHIPER_HOME and return its content, or None.
    ...
    try:
        ...
        logger.debug("Could not ensure CHIPER_HOME before loading SOUL.md: %s", e)

    soul_path = get_chiper_home() / "SOUL.md"
...
def _load_chiper_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.chiper.md / CHIPER.md — walk to git root."""
    chiper_md_path = _find_chiper_md(cwd_path)
    if not chiper_md_path:
        return ""
    try:
        content = chiper_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = chiper_md_path.name
        try:
            rel = str(chiper_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".chiper.md", context_length=context_length,
            read_path=str(chiper_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", chiper_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .chiper.md / CHIPER.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from CHIPER_HOME is independent and always included when present.
    ...
    project_context = (
        _load_chiper_md(cwd_path, context_length)
    ...
    # SOUL.md from CHIPER_HOME only — skip when already loaded as identity
    if not skip_soul:
