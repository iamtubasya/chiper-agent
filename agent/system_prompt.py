...
upstream prefix cache warm.  See ``chiper-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Chiper never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want CHIPER_HOME persona while keeping
    ...
    # Pointer to the chiper-agent skill + docs for user questions about Chiper itself.
    stable_parts.append(CHIPER_AGENT_HELP_GUIDANCE)
    ...
    # CHIPER_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Chiper, any interactive coding surface in a code
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Chiper profile: default. Other profiles (if any) live "
            "under ~/.chiperflux/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Chiper profile: {active_profile}. This session reads "
            f"and writes ~/.chiperflux/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.chiperflux/skills/, ~/.chiperflux/plugins/, "
            f"~/.chiperflux/cron/, ~/.chiperflux/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Chiper never rebuilds or reinjects parts of it
