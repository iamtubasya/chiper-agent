...
Chiper seeds its credential pool from many places:
    ...
    chiper_pkce   — ~/.chiperflux/.anthropic_oauth.json
    ...
    manual        — user ran `chiper auth add`
...
unify here is **removal**:
    ...
    ``chiper auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), chiper_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.chiperflux/.env  → clear it
    ...
    """
    from chiper_cli.config import get_env_path, remove_env_value
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `chiper auth add anthropic` to re-enable if needed.",
    ])


def _remove_chiper_pkce(provider: str, removed) -> RemovalResult:
    """~/.chiperflux/.anthropic_oauth.json is ours — delete it outright."""
    from chiper_constants import get_chiper_home

    result = RemovalResult()
    oauth_file = get_chiper_home() / ".anthropic_oauth.json"
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `chiper auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `chiper auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``chiper auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``chiper auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `chiper model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Chiper auth store is not enough — _seed_from_singletons() would
    ...
    via ``chiper auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `chiper auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `chiper auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `chiper auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="chiper_pkce",
        remove_fn=_remove_chiper_pkce,
        description="~/.chiperflux/.anthropic_oauth.json",
    ))
    register(RemovalStep(
