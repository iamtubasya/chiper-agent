...
as ``kind: backend``) or ``~/.chiperflux/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``chiper tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$CHIPER_HOME/cache/videos/``, creating parents as needed."""
    from chiper_constants import get_chiper_home

    path = get_chiper_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$CHIPER_HOME/cache/videos/``.
