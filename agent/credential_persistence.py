...
``auth.json``.  They intentionally have no dependency on ``chiper_cli.auth`` so
...
# Sources Chiper owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Chiper-owned OAuth/device-code state)
