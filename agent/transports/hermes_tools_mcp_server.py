...
Scope (what we expose):
  ...
  - kanban_* (complete/block/comment/    — kanban worker + orchestrator
    ...
    unblock/link)                          write ~/.chiperflux/kanban.db)
...
Run with: python -m agent.transports.chiper_tools_mcp_server
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    "skills_list",
    "text_to_speech",
    # Kanban worker handoff tools — gated on CHIPER_KANBAN_TASK env var
    # (set by the kanban dispatcher when spawning a worker). Without these
    # in the callback, a worker spawned with openai_runtime=codex_app_server
    # could do the work but couldn't report completion back to the kernel,
    # making it hang until timeout. Stateless dispatch — they just read
    # the env var and write to ~/.chiperflux/kanban.db.
    "kanban_complete",
    "kanban_block",
    "kanban_comment",
    "kanban_heartbeat",
    ...
    # only — the kanban tool gates them on CHIPER_KANBAN_TASK being unset.
...
def _build_server() -> Any:
    """Create the FastMCP server with Chiper tools attached. Lazy imports
...
def main(argv: Optional[list[str]] = None) -> int:
    """Entry point for `python -m agent.transports.chiper_tools_mcp_server`."""
