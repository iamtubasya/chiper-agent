...
Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):
    ...
    [
        "product=chiper-agent",
        "client=chiper-client-v<__version__>",
    ]

The version is sourced live from ``chiper_cli.__version__`` so it auto-aligns
...
``chiper_cli.__version__`` is the canonical source of truth.
"""
...
def chiper_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=chiper-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=chiper-client-v{_chiper_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=chiper-agent", chiper_client_tag()]
