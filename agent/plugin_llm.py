...
Plugins built on ChiperFlux Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Chiper
...
:class:`~chiper_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Chiper' main config
...
handles every provider, fallback chain, and per-task override Chiper
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`chiper_cli.plugins.PluginContext`
