...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Chiper with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.chiperflux/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Chiper
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``chiper tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``chiper setup`` for
