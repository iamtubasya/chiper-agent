...
Chiper supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.chiperflux/config.yaml`:
...
Or set the env var: `CHIPER_KANBAN_DISPATCH_IN_GATEWAY=false`
