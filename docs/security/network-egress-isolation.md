...
When running Chiper inside Docker, the default `network_mode: host` gives the
...
The Chiper [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   CHIPER_UID=$(id -u) CHIPER_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=chiper,chiper-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://chiper-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Chiper trust model and vulnerability reporting
