...
Chiper middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Chiper'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Chiper calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Chiper retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Chiper guardrails,
...
- `telemetry_schema_version`: currently `chiper.observer.v1`
- `middleware_schema_version`: currently `chiper.middleware.v1`
...
Chiper stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Chiper runs
them as a nested chain in registration order. Middleware failures are fail-open:
Chiper logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Chiper applies middleware in this order:
...
For each tool call, Chiper applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Chiper pre-execution path against the effective arguments:
...
```bash
chiper plugins enable <plugin-name>
```

For isolated local testing, use one `CHIPER_HOME` for plugin enablement and the
agent run:

```bash
export CHIPER_HOME=/tmp/chiper-middleware-test
mkdir -p "$CHIPER_HOME"
chiper plugins enable <plugin-name>
chiper chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run chiper plugins enable <plugin-name>
uv run chiper chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["chiper_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/chiper-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Chiper expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Chiper treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Chiper preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Chiper does not convert downstream
