# Chiper Observer Hooks

Chiper observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Chiper' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Chiper catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Chiper only builds
