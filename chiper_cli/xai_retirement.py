...
Pure logic: walks a Chiper config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `chiper doctor` and a future `chiper migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Chiper config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Chiper config and return retirement issues.
