...
``$CHIPER_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$CHIPER_HOME``,
    not under ``profiles/``). The dispatcher in ``chiper_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``chiper gateway start`` (no ``-p``) targets. Without it,
    bare ``chiper gateway start`` inside the container would land on
    ...
    ``$CHIPER_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        chiper_home: The container's CHIPER_HOME (typically /opt/data).
            Profiles live under ``<chiper_home>/profiles/<name>/``;
            the default profile lives at ``<chiper_home>`` itself.
    ...
    # ``chiper gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(chiper_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(chiper_home)
    ...
    profiles_root = chiper_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `chiper profile create` (config.yaml
            # is not — that comes later via `chiper setup`). Use it as the
            ...
            # via ``chiper -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(chiper_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = chiper_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("CHIPER_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``chiper dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted CHIPER_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `chiper -p <profile> gateway start`
...
def _write_reconcile_log(
    chiper_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $CHIPER_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted CHIPER_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        chiper_home=chiper_home, scandir=scandir,
    )
    for a in actions:
        print(
