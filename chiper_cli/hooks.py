"""chiper hooks — inspect and manage shell-script hooks.
...
def hooks_command(args) -> None:
    """Entry point for ``chiper hooks`` — dispatches to the requested action."""
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.chiperflux/config.yaml.")
        print("See `chiper hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `chiper hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `chiper hooks test` and `chiper hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `chiper hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `chiper hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `chiper hooks doctor`.")
