...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or chiper -c.",
    ...
    "Ctrl+Z suspends Chiper to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "chiper -c resumes your most recent CLI session. chiper -c \"project name\" resumes by title.",
    "chiper -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "chiper -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "chiper chat -t web,terminal enables only specific toolsets for a focused session.",
    "chiper chat -s github-pr-workflow preloads a skill at launch.",
    "chiper chat -q \"query\" runs a single non-interactive query and exits.",
    "chiper chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "chiper chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "chiper --yolo bypasses all dangerous command approval prompts for the entire session.",
    "chiper chat --source telegram tags the session for filtering in chiper sessions list.",
    "chiper -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "chiper doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "chiper dump outputs a compact setup summary — great for bug reports.",
    "chiper config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "chiper config edit opens config.yaml in your default editor.",
    "chiper config check scans for missing or stale configuration options.",
    "chiper sessions browse opens an interactive session picker with search.",
    "chiper sessions stats shows session counts by platform and database size.",
    "chiper sessions prune --older-than 30 cleans up old sessions.",
    "chiper skills search react --source skills-sh searches the skills.sh public directory.",
    "chiper skills check scans installed hub skills for upstream updates.",
    ...
    "chiper backup creates a zip backup of your entire Chiper home directory.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as chiper -w).",
    ...
    "CHIPER_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    "Profile names become shell commands — 'chiper profile create coder' creates the 'coder' command.",
    "chiper profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "chiper skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .chiperflux/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: chiper cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "Context files (.chiperflux.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    # --- MCP ---
    "chiper mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "chiper mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    ...
    "Checkpoints use shadow repos in ~/.chiperflux/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "chiper chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "chiper plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "chiper auth add nous or chiper auth add openai-codex sets up OAuth-based providers.",
    ...
    "chiper status --deep runs deeper diagnostic checks across all components.",
    ...
    "Every interrupt during an agent run is logged to ~/.chiperflux/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'chiper profile create ls' would be rejected.",
    "chiper profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "chiper claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Chiper is working.',
    ...
    '/reload reloads ~/.chiperflux/.env into the running session — pick up new API keys without restarting.',
    ...
    "CHIPER_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.chiperflux/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.chiperflux/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'chiper curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "chiper curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'chiper curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'chiper auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'CHIPER_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "CHIPER_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'CHIPER_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'CHIPER_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'CHIPER_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'CHIPER_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.chiperflux/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.chiperflux/skills/ so drop-in skills appear without restarting the session.',
    ...
    'chiper -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'chiper chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'chiper chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'chiper chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "chiper chat --source tool tags programmatic chats so they don't clutter chiper sessions list.",
    'chiper dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'chiper sessions rename <ID> "new title" renames any past session; chiper sessions delete <ID> removes one.',
    'chiper import restores a session export or profile archive produced by sessions export or profile export.',
    ...
    'CHIPER_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.chiperflux/plugins/ — trust-gated by design.',
    "CHIPER_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'CHIPER_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'CHIPER_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'CHIPER_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'CHIPER_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'CHIPER_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'CHIPER_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'CHIPER_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'CHIPER_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'CHIPER_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'CHIPER_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    '--source tool chats are excluded from chiper sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.chiperflux/dashboard-plugins/.',
]
