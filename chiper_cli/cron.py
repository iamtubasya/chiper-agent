"""
Cron subcommand for chiper CLI.
...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(chiper\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*chiper)"
    r"|(systemctl\s+(restart|stop|start)\s+.*chiper)"
    r"|(p?kill\s+.*chiper.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'chiper cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: chiper gateway install", Colors.DIM))
        print(color("                    sudo chiper gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    chiper gateway install    # Install as a user service")
        print("    sudo chiper gateway install --system  # Linux servers: boot-time system service")
        print("    chiper gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `chiper gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: chiper cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
