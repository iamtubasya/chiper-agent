"""
Chiper Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.chiperflux/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Chiper PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing chiper-agent or chiper PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Chiper Agent" comment and following line
                if '# Chiper Agent' in line or '# chiper-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('chiper.*PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing chiper
                if 'chiper' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the chiper wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "chiper",
        Path("/usr/local/bin/chiper"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains chiper_cli reference)
                content = wrapper.read_text()
                if 'chiper_cli' in content or 'chiper-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(chiper_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``chiper`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Chiper home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (chiper_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``chiper gateway run`` processes
    ...
    try:
        from chiper_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from chiper_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``CHIPER_HOME`` and ``CHIPER_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\chiper.*git\bin``,
#      ``%LOCALAPPDATA%\chiper.*git\usr\bin``, ``%LOCALAPPDATA%\chiper.*node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\chiper.*git\`` and Node to
#      ``%LOCALAPPDATA%\chiper.*node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``chiper dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\chiper.*gateway-service\`` and sometimes
...
def _chiper_path_markers(chiper_home: Path) -> list[str]:
    """Path-entry substrings that identify Chiper-owned User-PATH entries."""
    root = str(chiper_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare chiper-agent install dir.
    markers = [root + "\\chiper-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if CHIPER_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "chiper".  We don't want to catch
    # unrelated entries like "chiper-foo" or "ephermeral", so we look for
    # backslash-chiper as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(chiper_home: Path) -> list[str]:
    """Strip Chiper-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _chiper_path_markers(chiper_home)
            kept: list[str] = []
...
def remove_chiper_env_vars_windows() -> list[str]:
    """Delete CHIPER_HOME and CHIPER_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("CHIPER_HOME", "CHIPER_GIT_BASH_PATH"):
                try:
...
def _is_default_chiper_home(chiper_home: Path) -> bool:
    """Return True when ``chiper_home`` points at the default (non-profile) root."""
    try:
        from chiper_constants import get_default_chiper_root
        return chiper_home.resolve() == get_default_chiper_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its CHIPER_HOME directory.

    We shell out to ``chiper -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current CHIPER_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m chiper_cli.main` so we don't depend on a `chiper`
    #    wrapper that may be half-removed mid-uninstall.
    chiper_invocation = [_sys.executable, "-m", "chiper_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                chiper_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's CHIPER_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``chiper uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$CHIPER_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from chiper_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(chiper_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Chiper Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Chiper Chat GUI installation was found.")
        print(f"  Checked: {chiper_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Chiper agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(chiper_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Chiper agent at {chiper_home / 'chiper-agent'}")
        print(f"  • Your config, sessions, and secrets under {chiper_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Chiper agent is still installed. Run 'chiper' to use the CLI,")
    print("or 'chiper uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.chiperflux/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.chiperflux/ for future reinstall
    """
    project_root = get_project_root()
    chiper_home = get_chiper_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie CHIPER_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_chiper_home(chiper_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.chiperflux data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Chiper Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {chiper_home / 'config.yaml'}")
    print(f"  Secrets: {chiper_home / '.env'}")
    print(f"  Data:    {chiper_home / 'cron/'}, {chiper_home / 'sessions/'}, {chiper_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their CHIPER_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Chiper data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Chiper code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``chiper.*node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$CHIPER_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in chiper_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\chiper.*git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(chiper_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Chiper-owned PATH entries in User environment")

        log_info("Removing CHIPER_HOME / CHIPER_GIT_BASH_PATH User env vars...")
        removed_env = remove_chiper_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Chiper-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing chiper command...")
    ...
    #     (only when they still point into this Chiper home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Chiper-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(chiper_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Chiper-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(chiper_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE CHIPER_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from chiper_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(chiper_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.chiperflux/, just remove the chiper-agent subdir
            if chiper_home in project_root.parents or project_root.parent == chiper_home:
    ...
    #     under CHIPER_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(chiper_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(chiper_home)
    ...
    # 5. Optionally remove ~/.chiperflux/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile CHIPER_HOME dirs live under
        ...
        try:
            if chiper_home.exists():
                shutil.rmtree(chiper_home)
                log_success(f"Removed {chiper_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {chiper_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {chiper_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://chiper-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://chiper-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Chiper Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m chiper_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import chiper_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``chiper_constants`` + ``chiper_cli.colors``
    (and lazily ``chiper_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m chiper_cli.uninstall")
    parser.add_argument(
        "--mode",
