"""
Interactive setup wizard for Chiper Agent.
...
_DOCS_BASE = "https://chiper-agent.nousresearch.com/docs"
...
# display_chiper_home imported lazily at call sites (stale-module safety during chiper update)
...
from chiper_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from chiper_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Chiper Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Chiper using environment variables or config commands:")
    print_info("  chiper config set model.provider custom")
    print_info("  chiper config set model.base_url http://localhost:8080/v1")
    print_info("  chiper config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'chiper setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from chiper_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'chiper setup')")


def _print_setup_summary(config: dict, chiper_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'chiper setup' to configure"))
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from chiper_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'chiper setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'chiper setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'chiper setup terminal'"))
    ...
    # Spotify (OAuth via chiper auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'chiper setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {chiper_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('chiper setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('chiper setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('chiper setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('chiper setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('chiper setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('chiper config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('chiper config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('chiper config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('chiper', Colors.GREEN)}              Start chatting")
    print(f"   {color('chiper gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('chiper doctor', Colors.GREEN)}       Check for issues")
    print()
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``chiper model``)
    ...
    provider added to ``chiper model`` is automatically available here.
    ...
    # Delegate to the shared chiper model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from chiper_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: chiper model")
    ...
    # on demand via `chiper auth add`, `chiper setup` vision, and
    # `chiper setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``chiper model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from chiper_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.chiperflux/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Chiper auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via chiper setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'chiper setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Chiper runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `chiper setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `chiper setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into CHIPER_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("CHIPER_MAX_ITERATIONS")
    ...
    print_info("  Run `chiper setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # CHIPER_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from chiper_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_chiper_home(Path(get_chiper_home()))
    except Exception:
        pass
...
def _profile_name_from_chiper_home(chiper_home) -> str | None:
    """Return the active profile name when CHIPER_HOME is a profile dir."""
    if chiper_home.parent.name == "profiles":
        return chiper_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Chiper delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `chiper update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://chiper-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under CHIPER_HOME, and print
    ...
    """
    try:
        from chiper_cli.slack_cli import _build_full_manifest
        from chiper_constants import get_chiper_home

        manifest = _build_full_manifest(
            bot_name="Chiper",
            bot_description="Your Chiper agent on Slack",
        )
        target = Path(get_chiper_home()) / "slack-manifest.json"
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Chiper delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Chiper to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from chiper_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://chiper-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://chiper-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'chiper setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     chiper config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_chiper_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo chiper gateway install --system")
                print_info("  Or run in foreground:  chiper gateway")
        else:
            from chiper_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   chiper gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   chiper gateway              # Run in foreground")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``chiper_cli.auth`` — lists every supported
    ...
    """
    try:
        from chiper_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from chiper_cli.config
    ...
    if section_key == "model":
        ...
        from chiper_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_chiper.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_chiper migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_chiper", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Chiper.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Chiper to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Chiper at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Chiper at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Chiper at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Chiper at your OpenClaw WhatsApp connection",
...
def _offer_openclaw_migration(chiper_home: Path) -> bool:
    ...
    print_info("Chiper can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: chiper claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: chiper claw migrate"
        )
        print_info(
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Chiper (use chiper claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print(color("│     ⚕ Chiper Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("  Run `chiper` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      chiper setup           — full or quick (auto-detected)
      chiper setup model     — just model/provider
      chiper setup tts       — just text-to-speech
      chiper setup terminal  — just terminal backend
      chiper setup gateway   — just messaging platforms
      chiper setup tools     — just tool configuration
      chiper setup agent     — just agent settings
    """
    from chiper_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Chiper Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Chiper Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Chiper Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, chiper_home)
            return
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'chiper setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(chiper_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Chiper?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, chiper_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {chiper_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'chiper config edit'")
    ...
    # Tune later with `chiper setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, chiper_home)


def _run_first_time_quick_setup(config: dict, chiper_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``chiper setup <section>``
    or switch providers with ``chiper model``.
    """
    from chiper_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'chiper setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    chiper setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  chiper setup gateway")
    print()

    _print_setup_summary(config, chiper_home)


def _run_quick_setup(config: dict, chiper_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'chiper setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Chiper to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'chiper setup gateway'.")
