...
Pure display functions with no ChiperCLI state dependency.
"""
...
from chiper_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built chiper — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/chiper-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/chiper-agent"
...
def _fetch_pypi_latest(package: str = "chiper-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Chiper update is available.

    Two paths: if ``CHIPER_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = chiper_home / ".update_check"
    embedded_rev = os.environ.get("CHIPER_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # CHIPER_REVISION (that's nix-only). Without this guard the checks below
    ...
    if embedded_rev:
        ...
        # $CHIPER_HOME/chiper-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = chiper_home / "chiper-agent"
        if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``chiper_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from chiper_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/chiper-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Chiper checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/chiper-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Chiper Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else CHIPER_CADUCEUS
    except Exception:
        _bskin = None
        _hero = CHIPER_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from chiper_cli.codex_runtime_switch import get_current_runtime
        from chiper_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from chiper_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built chiper; we know an update
    ...
    # Pip-install warning — `pip install chiper-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else CHIPER_AGENT_LOGO
        console.print(_logo)
        console.print()
