"""On-demand supply-chain audit for Chiper Agent installs.
...
def _discover_plugins(chiper_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.chiperflux/plugins``.
...
def run_audit(
    *,
    ...
    home = chiper_home or Path(get_chiper_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, chiper_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(chiper_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `chiper security audit`."""
    home = Path(get_chiper_home())
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, chiper_home=home
    )
    if total == 0:
