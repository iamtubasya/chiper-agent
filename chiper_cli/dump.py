"""
Dump command for chiper CLI.

Outputs a compact, plain-text summary of the user's Chiper setup
...
from chiper_cli.config import get_chiper_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.chiper_build_sha`` by the Dockerfile's
    ``CHIPER_GIT_SHA`` build-arg (see ``chiper_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``chiper dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from chiper_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(chiper_home: Path) -> int:
...
def _cron_summary(chiper_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = chiper_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"chiper_home:      {display_chiper_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `chiper auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `chiper auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["chiper-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(chiper_home)}")
    lines.append(f"  skills:             {_count_skills(chiper_home)}")
