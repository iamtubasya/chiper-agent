...
the existing module-level functions in chiper_cli.gateway and
chiper_cli.gateway_windows directly. This protocol is a thin facade
...
``chiper gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``chiper gateway start/stop/restart``).
    """
    ...
    # `chiper gateway start/stop/restart` fell through to host code that spawns
...
# in ``chiper_cli.gateway`` (systemd/launchd) and ``chiper_cli.gateway_windows``
...
# active (set via the ``chiper -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in chiper_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from chiper_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from chiper_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from chiper_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from chiper_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in chiper_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from chiper_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from chiper_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from chiper_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from chiper_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``chiper_cli.gateway_windows`` (Scheduled Task /
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's CHIPER_HOME blindly;
    derive the shared profile root from the current CHIPER_HOME and map the
    ...
    chiper_home = Path(os.environ.get("CHIPER_HOME", "/opt/data"))
    if chiper_home.parent.name == "profiles":
        root = chiper_home.parent.parent
    else:
        root = chiper_home
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``chiper -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`chiper profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        root CHIPER_HOME profile" (the implicit profile that exists at
        the top of $CHIPER_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``chiper_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$CHIPER_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``CHIPER_HOME``,
        ...
        allocator in ``chiper_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export CHIPER_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's CHIPER_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``chiper gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-CHIPER_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The CHIPER_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own CHIPER_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "chiper gateway run --replace"
        else:
            gateway_cmd = f"chiper -p {shlex.quote(profile)} gateway run --replace"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``chiper logs`` reads and what
        ...
        return (
            ...
            f': "${{CHIPER_HOME:=/opt/data}}"\n'
            f'log_dir="$CHIPER_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``chiper gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``chiper -p <profile> gateway start``.
