...
def _subscriptions_path() -> Path:
    return _chiper_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: chiper gateway run
"""
...
def webhook_command(args):
    """Entry point for 'chiper webhook' subcommand."""
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (chiper gateway run).\n")
...
def _cmd_test(args):
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (chiper gateway run)")
