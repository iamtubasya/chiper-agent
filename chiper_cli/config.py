"""
Configuration management for Chiper Agent.

Config files are stored in ~/.chiperflux/ for easy access:
- ~/.chiperflux/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.chiperflux/.env         - API keys and secrets

This module provides:
- chiper config          - Show current configuration
- chiper config edit     - Open config in editor
- chiper config set      - Set a specific value
- chiper config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``chiper config set`` (which rewrites
    ...
    ``config.yaml`` in place: chiper never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.chiperflux/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``chiper logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  chiper config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Chiper makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Chiper itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``chiper update``, the TUI build.
...
#   on every plugin install / ``chiper update``.
...
# * ``CHIPER_HOME`` / ``CHIPER_PROFILE`` / ``CHIPER_CONFIG`` /
#   ``CHIPER_ENV`` — Chiper runtime location flags. Writing these into
...
#
# IMPORTANT: ``CHIPER_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (CHIPER_GEMINI_CLIENT_ID,
# CHIPER_LANGFUSE_PUBLIC_KEY, CHIPER_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Chiper runtime location — never via dashboard env writer.
    # NOT a CHIPER_* blanket: integration credentials (CHIPER_GEMINI_*,
    # CHIPER_LANGFUSE_*, CHIPER_SPOTIFY_*, ...) ARE allowed.
    "CHIPER_HOME", "CHIPER_PROFILE", "CHIPER_CONFIG", "CHIPER_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Chiper runtime location "
            "(CHIPER_HOME, CHIPER_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.chiperflux/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "CHIPER_TOOL_PROGRESS", "CHIPER_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `chiper plugins enable
    # observability/langfuse` or `chiper tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "CHIPER_LANGFUSE_ENV",
    "CHIPER_LANGFUSE_RELEASE",
    "CHIPER_LANGFUSE_SAMPLE_RATE",
    "CHIPER_LANGFUSE_MAX_CHARS",
    "CHIPER_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from chiper_cli.colors import Colors, color
from chiper_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("CHIPER_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_chiper_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Chiper is running in package-manager-managed mode.

    Two signals: the CHIPER_MANAGED env var (set by the systemd service),
    or a .managed marker file in CHIPER_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade chiper-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``chiper_cli/`` — i.e. the git checkout for source
    installs, ``/opt/chiper`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$CHIPER_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Chiper was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$CHIPER_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. CHIPER_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.chiperflux``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$CHIPER_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.chiperflux:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$CHIPER_HOME``, a
    ...
    ``docker`` and ``chiper update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/chiper-agent`` image bakes a ``docker``
        stamp into ``/opt/chiper`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $CHIPER_HOME.
    try:
    ...
    #    host install whose shared $CHIPER_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``chiper update``.
    try:
        method = (
            (get_chiper_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``chiper_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$CHIPER_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/chiper`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Chiper lives in a ``uv tool`` layout.

    ``uv tool install chiper-agent`` places the install at
    ``.../uv/tools/chiper-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``chiper-agent`` happens to be uv-tool-installed on the machine while
    the *active* Chiper is a regular pip/venv install, causing
    ``chiper update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/chiper-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade chiper-agent"
    if method == "docker":
        return "docker pull nousresearch/chiper-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade chiper-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade chiper-agent"
        return "pip install --upgrade chiper-agent"
    return "chiper update"
...
# Long-form text for ``chiper update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# chiper_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Chiper, it doesn't update
...
✗ ``chiper update`` doesn't apply inside the Docker container.

Chiper Agent runs as a published image (nousresearch/chiper-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/chiper-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate chiper-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/chiper-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/chiper-agent/tags
  • Your config and session history live under ``$CHIPER_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``chiper update`` inside Docker.
...
def format_managed_message(action: str = "modify this Chiper installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("CHIPER_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Chiper installation is managed by NixOS "
            f"(CHIPER_MANAGED={env_hint}).\n"
            "Edit services.chiper-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Chiper installation is managed by Homebrew "
            f"(CHIPER_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade chiper-agent"
        )

    return (
        f"Cannot {action}: this Chiper installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Chiper."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from CHIPER_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, chiper_bin
    or None if container mode is not active, we're already inside the
    container, or CHIPER_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("CHIPER_DEV") == "1":
        return None
    ...
    container_mode_file = get_chiper_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "chiper-agent")
    exec_user = info.get("exec_user", "chiper")
    chiper_bin = info.get("chiper_bin", "/data/current-package/bin/chiper")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "chiper_bin": chiper_bin,
    }
...
# Re-export from chiper_constants — canonical definition lives there.
from chiper_constants import get_chiper_home  # noqa: F811,E402
...
def get_env_path() -> Path:
    ...
    return get_chiper_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_chiper_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the CHIPER_UID / CHIPER_GID env vars set by Docker deployments.

    Docker containers running Chiper commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level CHIPER_HOME once, but
    subdirectories created at runtime by ``ensure_chiper_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("CHIPER_UID", "").strip()
    gid_str = os.environ.get("CHIPER_GID", "").strip()
    try:
...
def _chown_to_chiper_uid(path) -> None:
    """Chown ``path`` to ``CHIPER_UID:CHIPER_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_chiper_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_chiper_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the chiper group can
    share state with the gateway service.

    The mode can be overridden via the CHIPER_HOME_MODE environment variable
    (e.g. CHIPER_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse CHIPER_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``CHIPER_UID``/``CHIPER_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("CHIPER_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_chiper_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Chiper runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("CHIPER_CONTAINER") or os.environ.get("CHIPER_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set CHIPER_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into CHIPER_HOME if the user doesn't have one yet."""
...
def ensure_chiper_home():
    """Ensure ~/.chiperflux directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_chiper_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_chiper_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"CHIPER_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Chiper-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Chiper
        ...
        # identity slot (SOUL.md). Empty by default. The CHIPER_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Chiper adds a coding operating brief
    ...
    "terminal": {
        ...
        #             CHIPER_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force CHIPER_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Chiper auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Chiper sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Chiper
        ...
        # bundled Chiper image, which drops to the `chiper` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Chiper sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Chiper should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``chiper chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.chiperflux/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: chiper sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .chiper.md, or .cursorrules before Chiper applies
    ...
    # single read_file page exceeds these limits, Chiper truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``chiper kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``chiper kanban decompose`` and the kanban
        ...
        # ``chiper profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `chiper model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `chiper` (and `chiper chat`) launches by default:
        ...
        # REPL and `--tui` (or CHIPER_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `chiper --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `chiper -c` muscle memory.  Default off so existing
        # users aren't surprised.  CHIPER_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `chiper --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/chiper`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``CHIPER_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``CHIPER_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``CHIPER_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``CHIPER_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/chiper``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Chiper appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.chiperflux/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.chiperflux/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Chiper feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Chiper auto-pauses the goal and
    ...
    # always goes to ~/.chiperflux/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${CHIPER_SKILL_DIR} and ${CHIPER_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `chiper curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `chiper curator run --consolidate` overrides
        ...
        # normally restored on every `chiper update`, so pruning them only
        ...
        # skipped), snapshot ~/.chiperflux/skills/ into
        # ~/.chiperflux/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `chiper curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for chiper-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Chiper Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (CHIPER_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.chiperflux/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or CHIPER_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `chiper doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``chiper_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Chiper to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via CHIPER_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `chiper -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `chiper` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `chiper kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     chiper-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Chiper tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.chiperflux/logs/.
    ...
    # update model picker lists without shipping a chiper-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://chiper-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `chiper model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.chiperflux/.env,
        ...
        # behavior: files must live under the Chiper cache, under
        ...
        # user. Bridged to CHIPER_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Chiper
        # cache (~/.chiperflux/cache/{documents,images,audio,video,screenshots})
        ...
        # to CHIPER_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # CHIPER_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to CHIPER_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.chiperflux/state.db.
    ...
    "sessions": {
        ...
        # default of ``chiper sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.chiperflux/sessions/session_{sid}.json`` on every turn
    ...
    # ``chiper update`` behaviour.
    "updates": {
        # Run a full ``chiper backup``-style zip of CHIPER_HOME before every
        # ``chiper update``.  Backups land in ``<CHIPER_HOME>/backups/`` and
        # can be restored with ``chiper import <path>``.  Defaults to true
        # after the #48200 incident: a ``chiper update --yes`` run that
        ...
        "backup_keep": 5,
        # What `chiper update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<CHIPER_HOME>/lsp/bin/`` on first use.
    ...
    # `chiper tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.chiperflux/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.chiperflux/.env (or your shell) and never in config.yaml.
            ...
            # ~/.chiperflux/bin/ on first use.  When False you must install
            ...
            # `chiper secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'chiper model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Chiper auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @chiper:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. CHIPER_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #chiper)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: chiper-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'chiper-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Chiper API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Chiper API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Chiper API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # CHIPER_TOOL_PROGRESS and CHIPER_TOOL_PROGRESS_MODE are deprecated —
    ...
    "CHIPER_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.chiper.config`` entries, then checks
    ...
    try:
        ...
        # should never break `chiper update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Chiper versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``chiper_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'chiper setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Chiper won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'chiper doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("CHIPER_HOME", "~/.chiperflux")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("CHIPER_TOOL_PROGRESS")
            old_mode = get_env_value("CHIPER_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from CHIPER_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from CHIPER_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("CHIPER_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from CHIPER_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$CHIPER_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_chiper_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `chiper plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `chiper curator status` has no
    ...
    #   3. Creates `~/.chiperflux/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_chiper_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_chiper_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from chiper_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: chiper config set <key> <value>")
    ...
    # metadata.chiper.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _chiper_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.chiperflux/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.chiperflux/config.yaml.
    ...
    (which change ``CHIPER_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — chiper auth) — OpenAI Codex
#   nous         (OAuth — chiper auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.chiperflux/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.chiperflux/.env.
    ...
    interactive menu render (`chiper tools`, `chiper setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `chiper tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Chiper env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.chiperflux/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.chiperflux/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.chiperflux/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.chiperflux/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Chiper — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Chiper vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.chiperflux/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Chiper Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from chiper_cli.auth import get_anthropic_key
    ...
    # Warn on stale CHIPER_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("CHIPER_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale CHIPER_MAX_ITERATIONS={_env_ghost} "
                f"(run 'chiper doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  chiper config edit     # Edit config file", Colors.DIM))
    print(color("  chiper config set <key> <value>", Colors.DIM))
    print(color("  chiper setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: chiper config set <key> <value>")
            print()
            print("Examples:")
            print("  chiper config set model anthropic/claude-sonnet-4")
            print("  chiper config set terminal.backend docker")
            print("  chiper config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'chiper config migrate' to add them")
        ...
        print("  chiper config           Show current configuration")
        print("  chiper config edit      Open config in editor")
        print("  chiper config set <key> <value>   Set a config value")
        print("  chiper config check     Check for missing/outdated config")
        print("  chiper config migrate   Update config with new options")
        print("  chiper config path      Show config file path")
        print("  chiper config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``chiper config`` UI so users
