...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "chiper CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``chiper gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "chiper_cli.main gateway",
        "chiper_cli.main --profile",
        "chiper_cli.main -p",
        "chiper_cli/main.py gateway",
        "chiper_cli/main.py --profile",
        "chiper_cli/main.py -p",
        "chiper gateway",
    ...
    current_home = str(get_chiper_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"chiper_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. CHIPER_HOME
        ...
        # CHIPER_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "chiper_home=" in command_lc
            and f"chiper_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``chiper update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Chiper profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Chiper profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "chiper_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``chiper update`` users
    # with no running gateway until they re-invoke ``chiper gateway``
    ...
    from chiper_cli._subprocess_compat import (
...
def _sync_chiper_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``CHIPER_HOME``.

    Under ``sudo``, ``CHIPER_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_chiper_home` falls back to ``/root/.chiperflux`` — the wrong
    profile. The unit file pins ``CHIPER_HOME`` for the actual gateway
    ...
    unit_home = env.get("CHIPER_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("CHIPER_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["CHIPER_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}chiper gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}chiper gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from chiper_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `chiper gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``chiper gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from chiper_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (CHIPER_HOME-scoped).
...
from chiper_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``chiper gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``CHIPER_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("CHIPER_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "chiper-gateway"
SERVICE_DESCRIPTION = "Chiper Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current CHIPER_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (CHIPER_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from chiper_constants import get_default_chiper_root

    home = get_chiper_home().resolve()
    default = get_default_chiper_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary CHIPER_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(chiper_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when CHIPER_HOME is a named profile.

    For ``~/.chiperflux/profiles/<name>``, returns ``"--profile <name>"``.
    ...
    home = Path(chiper_home or str(get_chiper_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_chiper_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(chiper_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".chiperflux"
    try:
        Path(chiper_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(chiper_home, default_root=target_root)
    except ValueError:
        return _profile_arg(chiper_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this CHIPER_HOME.

    Default ``~/.chiperflux`` returns ``chiper-gateway`` (backward compatible).
    Profile ``~/.chiperflux/profiles/coder`` returns ``chiper-gateway-coder``.
    Any other CHIPER_HOME appends a short hash for uniqueness.
    """
...
def print_legacy_unit_warning() -> None:
    ...
    print_info("  These run alongside the current chiper-gateway service and")
...
def remove_legacy_hermes_units(
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: chiper gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo chiper gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    chiper gateway uninstall")
    print_info("    sudo chiper gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo chiper gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo chiper gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo chiper gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.chiperflux`` → ``ai.chiperflux.gateway.plist`` (backward compatible).
    Profile ``~/.chiperflux/profiles/coder`` → ``ai.chiperflux.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.chiperflux.gateway-{suffix}" if suffix else "ai.chiperflux.gateway"
...
def _chiper_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current CHIPER_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_chiper_home() resolves to
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m chiper_cli.main``, so module
    ...
    dir, or a clone that ``chiper update`` later relocates/removes — the path
    ...
    ``CHIPER_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if CHIPER_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        chiper_home = _chiper_home_for_target_user(home_dir)
        profile_arg = _profile_arg_for_target_user(chiper_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's CHIPER_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(chiper_home) if chiper_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m chiper_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="CHIPER_HOME={chiper_home}"
Restart=always
RestartSec=5
...
"""
    ...
    chiper_home = str(get_chiper_home().resolve())
    profile_arg = _profile_arg(chiper_home)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir CHIPER_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved CHIPER_HOME
    ...
    exported a throwaway ``CHIPER_HOME=/tmp/...`` — writing it to the real
    ...
    Matches both systemd ``Environment="CHIPER_HOME=..."`` lines and launchd
    ``<key>CHIPER_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'CHIPER_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>CHIPER_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp CHIPER_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: CHIPER_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported CHIPER_HOME. "
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo chiper gateway uninstall --system")
    print_info("         chiper gateway install")
    print_info("         chiper gateway start")
...
def systemd_install(
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}chiper gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}chiper gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}chiper gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_chiper_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `chiper gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `chiper gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}chiper gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}chiper gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}chiper gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}chiper gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.chiperflux.gateway-{suffix}" if suffix else "ai.chiperflux.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``chiper gateway start/stop/status`` call).
...
# (the documented `nohup chiper gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m chiper_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active CHIPER_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "chiper_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup chiper gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    """
    from chiper_constants import display_chiper_home as _dhh
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: chiper gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup chiper gateway run --replace "
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  chiper gateway status             # Check status")
    from chiper_constants import display_chiper_home as _dhh
...
def launchd_stop():
    ...
    # `chiper gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        log_file = get_chiper_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``CHIPER_S6_SUPERVISED_CHILD``.
    """
    if os.environ.get("INVOCATION_ID"):
        return True
    if os.environ.get("CHIPER_S6_SUPERVISED_CHILD"):
        return True
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``chiper gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``chiper gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other CHIPER_HOME roots.
    """
    ...
    print("  Use 'chiper gateway restart' to replace it,")
    print("  or 'chiper gateway stop' first.")
    print("  Or use 'chiper gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("CHIPER_ALLOW_ROOT_GATEWAY")):
        return
    ...
    print(
        ...
        "$CHIPER_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set CHIPER_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `chiper gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set CHIPER_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `chiper gateway restart` which already
    ...
    # `chiper gateway start/restart` — leaving the gateway vulnerable to
    ...
    # CHIPER_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("CHIPER_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
...
def _all_platforms() -> list[dict]:
    ...
    ``chiper setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.chiperflux/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_chiper_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from chiper_cli.telegram_managed_bot import (
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'chiper pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Skipped — configure later with 'chiper gateway setup'"
                    )
            continue
...
def _setup_whatsapp():
    ...
    from chiper_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from chiper_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Skipped — configure later with 'chiper gateway setup'")
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from chiper_cli import gateway_windows
...
def _setup_weixin():
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `chiper gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_chiper_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `chiper pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `chiper pairing approve`."
        )
    elif access_idx == 1:
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``chiper_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from chiper_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.chiperflux/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.chiperflux/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: chiper gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: chiper gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: chiper gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo chiper gateway install --system"
                        )
                    print_info("  Or run in foreground:  chiper gateway run")
            elif is_wsl():
                ...
                print_info("  Run in foreground: chiper gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup chiper gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'chiper gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`chiper_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from chiper_cli.service_manager import (
    ...
    if profile is None:
        ...
        # CHIPER_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``chiper gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``chiper gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``chiper gateway run`` are unaffected.
      2. ``CHIPER_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``chiper gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``CHIPER_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("CHIPER_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("CHIPER_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${CHIPER_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if CHIPER_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or CHIPER_GATEWAY_NO_SUPERVISE=1) to opt out\n"
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: chiper gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
            ...
            from chiper_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
            ...
            print()
            print("To run the gateway: chiper gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: chiper gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: chiper gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        # slot — start them individually via `chiper -p <name> gateway
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "Use `chiper gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `chiper gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: chiper gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  chiper gateway install")
                else:
                    print("To install as a service:")
                    print("  chiper gateway install")
                    print("  sudo chiper gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  chiper gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup chiper gateway run > ~/.chiperflux/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'chiper gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  chiper gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  chiper gateway install  # Install as user service")
                    print(
                        "  sudo chiper gateway install --system  # Install as boot-time system service"
                    )
