...
Profiles live under ``~/.chiperflux/profiles/<name>/`` by default.

The "default" profile is ``~/.chiperflux`` itself — backward compatible,
zero migration needed.

Usage::

    chiper profile create coder          # fresh profile + bundled skills
    chiper profile create coder --clone  # also copy config, .env, SOUL.md, skills
    chiper profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    chiper -p coder chat                 # or via flag
    chiper profile use coder             # set as sticky default
    chiper profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See chiper_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.chiperflux``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   chiper-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `chiper backup` archives
...
# Marker file written by `chiper profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `chiper
...
# `chiper skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Chiper infrastructure directories that only the default profile
         (``~/.chiperflux``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_chiper_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.chiperflux) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "chiper-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "chiper", "default", "test", "tmp", "root", "sudo",
})

# Chiper subcommands that cannot be used as profile names/aliases
_CHIPER_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the chiper root, NOT to the current CHIPER_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where CHIPER_HOME points outside
    ``~/.chiperflux``, profiles live under ``CHIPER_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_chiper_home() / "profiles"


def _get_default_chiper_home() -> Path:
    """Return the default (pre-profile) CHIPER_HOME path.

    In standard deployments this is ``~/.chiperflux``.
    In Docker/custom deployments where CHIPER_HOME is outside ``~/.chiperflux``
    (e.g. ``/opt/data``), returns CHIPER_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_chiper_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Chiper installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its CHIPER_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_chiper_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, chiper subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _CHIPER_SUBCOMMANDS:
        return f"'{canon}' conflicts with a chiper subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            chiper_exe = shutil.which("chiper") or "chiper"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(chiper_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Chiper. If we
    ...
    try:
        ...
        # not be migrated. The next `chiper doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``chiper -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``chiper profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``chiper profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Chiper config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_chiper_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``chiper update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.chiperflux)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.chiperflux/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `chiper -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Chiper profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `chiper update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`chiper profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `chiper update`.\n",
                encoding="utf-8",
            )
        except OSError:
            pass  # best-effort — the feature still works via the empty skills/ dir

    # Cloned configs can be older than the running Chiper (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `chiper profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `chiper -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches CHIPER_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``chiper profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "CHIPER_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_chiper_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.chiperflux).\n"
            "To remove everything, use: chiper uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``CHIPER_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from chiper_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set CHIPER_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("CHIPER_HOME")
    try:
        os.environ["CHIPER_HOME"] = str(profile_dir)
        from chiper_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["CHIPER_HOME"] = old_home
        elif "CHIPER_HOME" in os.environ:
            del os.environ["CHIPER_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.chiperflux/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: chiper profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from CHIPER_HOME.

    Returns ``"default"`` if CHIPER_HOME is not set or points to ``~/.chiperflux``.
    Returns the profile name if CHIPER_HOME points into ``~/.chiperflux/profiles/<name>``.
    Returns ``"custom"`` if CHIPER_HOME is set to an unrecognized path.
    """
    from chiper_constants import get_chiper_home
    chiper_home = get_chiper_home()
    resolved = chiper_home.resolve()

    default_resolved = _get_default_chiper_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.chiperflux itself — its parent is ~/ and its
        # directory name is ".chiper", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: chiper profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.chiperflux itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.chiperflux). "
            "Specify a different name: chiper profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="chiper_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"chiper_{old_name}"
    legacy_old_host = f"chiper.{old_name}"
    new_host = f"chiper_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_chiper_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("chiper_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a CHIPER_HOME path string.

    Called early in the CLI entry point, before any chiper modules
    are imported, to set the CHIPER_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
