"""CLI subcommand: ``chiper send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"chiper send: {file_path} is not a text file. --file reads the "
            ...
            print(f"chiper send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"chiper send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"chiper send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"chiper send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"chiper send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `chiper gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.chiperflux/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    ...
    startup that ``chiper send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.chiperflux/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.chiperflux/config.yaml`` into
       ...
       via ``chiper config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``chiper send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    if message is None or not message.strip():
        print(
            "chiper send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `chiper send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `chiper send --list telegram`.",
    )
