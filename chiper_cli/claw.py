...
Usage:
    chiper claw migrate              # Preview then migrate (always shows preview first)
    chiper claw migrate --dry-run    # Preview only, no changes
    chiper claw migrate --yes        # Skip confirmation prompt
    chiper claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    chiper claw migrate --no-backup  # Skip pre-migration snapshot
    chiper claw cleanup              # Archive leftover OpenClaw directories
    chiper claw cleanup --dry-run    # Preview what would be archived
"""
...
from chiper_cli.config import get_chiper_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Chiper may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print()
    print_error(
        "Chiper gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'chiper gateway stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_chiper.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_chiper", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: chiper claw <command> [options]")
        print()
        print("Commands:")
...
def _cmd_migrate(args):
    """Run the OpenClaw → Chiper migration."""
    ...
    # migrate-chiper posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: chiper claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {chiper_home}")
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.chiperflux/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: chiper claw migrate --yes")
            return
    ...
    if not no_backup:
        try:
            from chiper_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(chiper_home=chiper_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: chiper import {backup_archive.name}")
        except Exception as e:
            print()
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: chiper claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: chiper claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  chiper claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  chiper claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  chiper config set OPENROUTER_API_KEY sk-or-v1-...")
