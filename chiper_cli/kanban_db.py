...
the profile's Chiper home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``CHIPER_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``CHIPER_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``chiper kanban boards
...
* ``CHIPER_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``CHIPER_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``CHIPER_KANBAN_DB``,
``CHIPER_KANBAN_WORKSPACES_ROOT``, and ``CHIPER_KANBAN_BOARD`` into
...
# ``CHIPER_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``CHIPER_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("CHIPER_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``CHIPER_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("CHIPER_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``CHIPER_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "CHIPER_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
# enough that kebab-case names like ``atm10-server`` or ``chiper-agent``
...
def kanban_home() -> Path:
    ...
    1. ``CHIPER_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_chiper_root()``, which already returns ``<root>``
       when ``CHIPER_HOME`` is ``<root>/profiles/<name>``, and returns
       ``CHIPER_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``CHIPER_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("CHIPER_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from chiper_constants import get_default_chiper_root
    return get_default_chiper_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``chiper kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``CHIPER_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``chiper kanban boards
    ...
    env = os.environ.get("CHIPER_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``chiper kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``CHIPER_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("CHIPER_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``CHIPER_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("CHIPER_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``CHIPER_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("CHIPER_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``chiper kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``CHIPER_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``CHIPER_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("CHIPER_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``CHIPER_KANBAN_DB`` env → ``CHIPER_KANBAN_BOARD`` env →
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``chiper kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``CHIPER_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from chiper_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``chiper --skills ...`` alongside the built-in
    ...
    # `chiper --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``chiper kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `chiper kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``chiper kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``chiper kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``chiper kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``chiper kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``CHIPER_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
...
#      via ``chiper kanban show <id>`` and the dashboard.
...
# without thrashing. Overridable via ``CHIPER_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Chiper
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``chiper kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Chiper profile.
    ...
    if not rows:
        return False
    try:
        from chiper_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from chiper_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Chiper profile.
        # `_default_spawn` invokes ``chiper -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Chiper
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `chiper kanban tail` — without
...
def _kanban_worker_skill_available(chiper_home: Optional[str]) -> bool:
    ...
    the worker activates a profile (``chiper -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset CHIPER_HOME means the worker falls back to the default root
    # home (``~/.chiperflux``), which ships the bundled skill.
    base = _Path(chiper_home) if chiper_home else (_Path.home() / ".chiperflux")
...
def _resolve_worker_cli_toolsets(chiper_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``CHIPER_KANBAN_TASK`` is set.
    """
    if not chiper_home:
        return None
    try:
        from chiper_constants import reset_chiper_home_override, set_chiper_home_override
        from chiper_cli.config import load_config
        from chiper_cli.tools_config import _get_platform_tools

        token = set_chiper_home_override(chiper_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for CHIPER_HOME=%r (%s)",
            chiper_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``chiper -p <profile> chat -q ...`` subprocess.
    ...
    ``CHIPER_KANBAN_DB`` / ``CHIPER_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject CHIPER_HOME so the worker reads the profile-scoped config.yaml
    ...
    try:
        env["CHIPER_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via CHIPER_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # CHIPER_HOME never had profiles created.
        pass
    if task.tenant:
        env["CHIPER_TENANT"] = task.tenant
    env["CHIPER_KANBAN_TASK"] = task.id
    env["CHIPER_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["CHIPER_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["CHIPER_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["CHIPER_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["CHIPER_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["CHIPER_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`chiper -p <name>`
    # rewrites CHIPER_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_chiper_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["CHIPER_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["CHIPER_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["CHIPER_KANBAN_BOARD"] = resolved_board
    # CHIPER_PROFILE is the author the kanban_comment tool defaults to.
    # `chiper -p <assignee>` activates the profile, but the env var is
    ...
    if _kanban_worker_skill_available(env.get("CHIPER_HOME")):
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("CHIPER_HOME"))
    if worker_toolsets:
    ...
    # `chiper kanban log` on a specific board reads its own file and
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``chiper kanban daemon`` is systemd-friendly.
...
def list_profiles_on_disk() -> list[str]:
    ...
    try:
        from chiper_constants import get_default_chiper_root
        default_root = get_default_chiper_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``chiper kanban assignees`` for the terminal.
