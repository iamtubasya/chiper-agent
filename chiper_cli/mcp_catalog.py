...
Catalog policy:
- Entries are added only by merging a PR into chiper-agent. Presence in the
...
from chiper_constants import get_chiper_home, get_optional_mcps_dir
...
from chiper_cli.cli_output import prompt as _prompt_input
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Chiper install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Chiper understands version {_MANIFEST_VERSION})"
        )
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Chiper
        understands. Update Chiper to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_chiper_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.chiperflux/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.chiperflux/.env via save_env_value()."""
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from chiper_cli.mcp_config import _probe_single_server
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `chiper auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`chiper auth {entry.auth.provider}` if you have not "
    ...
    from chiper_cli.mcp_config import _save_mcp_server
