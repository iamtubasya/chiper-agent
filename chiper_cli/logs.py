...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Chiper runs — try 'chiper chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_chiper_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_chiper_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_chiper_home()}/logs/")
        return

    print(f"Log files in {display_chiper_home()}/logs/:\n")
    found = False
