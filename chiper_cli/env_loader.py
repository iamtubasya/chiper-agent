...
# load_chiper_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_chiper_dotenv().
# Used by setup / `chiper model` flows to label detected credentials so
# users understand WHERE a key came from when their .env doesn't contain it
# directly (otherwise the "credentials detected ✓" line looks identical to
# the .env case and they don't know Bitwarden is wired up).
_SECRET_SOURCES: dict[str, str] = {}

# CHIPER_HOME paths we've already pulled external secrets for during this
# process.  ``load_chiper_dotenv()`` is called at module-import time from
# several hot modules (cli.py, chiper_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_chiper_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which CHIPER_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `chiper setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from chiper_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_chiper_dotenv(
    *,
    ...
    home_path = Path(chiper_home or os.getenv("CHIPER_HOME", Path.home() / ".chiperflux"))
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    ``home_path`` are no-ops.  ``load_chiper_dotenv()`` runs at import
    time from several hot modules (cli.py, chiper_cli/main.py,
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `chiper model`
