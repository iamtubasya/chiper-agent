"""Profile distributions — shareable, packaged Chiper profiles via git.

A distribution is a Chiper profile published as a git repository (or
...
* ``chiper profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``chiper skills install <url>`` — the URL install pattern we're mirroring,
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "chiper-agent", ".worktrees", "profiles", "bin", "node_modules",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``chiper profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Chiper profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Chiper distribution."
        )
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.chiperflux).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
    ...
    with tempfile.TemporaryDirectory(prefix="hermes_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `chiper profile update` to upgrade in place, "
