"""
Chiper Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``chiper desktop`` / ``chiper gui``)
     Built inside the agent checkout under ``$CHIPER_HOME/chiper-agent/``:
       ...
       - ``$CHIPER_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Chiper.app`` or ``~/Applications/Chiper.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Chiper`` (NSIS per-user)
...
the app name ("Chiper"), separate from ``$CHIPER_HOME``:
  - macOS:   ``~/Library/Application Support/Chiper``
  - Windows: ``%APPDATA%\\Chiper``
  - Linux:   ``$XDG_CONFIG_HOME/Chiper`` (default ``~/.config/Chiper``)
...
def _agent_root(chiper_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return chiper_home / "chiper-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Chiper"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Chiper"
...
def source_built_gui_artifacts(chiper_home: Path) -> "list[Path]":
    ...
    """
    agent_root = _agent_root(chiper_home)
    ...
    return [
        ...
        chiper_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Chiper" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Chiper.app"),
            home / "Applications" / "Chiper.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Chiper).
            local_base / "Programs" / "Chiper",
            # Older / alternate layout some builds used.
            local_base / "chiper-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Chiper")
    else:
        ...
        paths += [
            data_base / "applications" / "chiper.desktop",
            data_base / "applications" / "Chiper.desktop",
        ]
    return paths


def agent_is_installed(chiper_home: Path) -> bool:
    """Return True when a usable Python agent install exists under CHIPER_HOME.
    ...
    if (agent_root / "chiper_cli").is_dir():
        return True
...
def gui_is_installed(chiper_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(chiper_home):
        if p.exists():
            return True
...
def gui_install_summary(chiper_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = chiper_home if chiper_home is not None else get_chiper_home()
...
def uninstall_gui(chiper_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``chiper-agent/chiper_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$CHIPER_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove chiper' or "
            "'sudo dnf remove chiper'). AppImage builds are a single file you "
