...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_chiper_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:chiper_pkce",
    ...
    if provider == "nous":
        ...
        # <chiper-root>/shared/nous_auth.json (written by any previous
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``chiper auth add openai-codex`` overwrote
    ...
    raise SystemExit(f"`chiper auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Chiper reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `chiper auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `chiper auth` is called bare."""
