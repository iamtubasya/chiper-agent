...
at ``$CHIPER_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where Chiper keeps *its* uv binary.

    ``$CHIPER_HOME/bin/uv`` on POSIX, ``$CHIPER_HOME\\bin\\uv.exe`` on
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``chiper update`` runs
    the call site from the *old*, already-imported ``chiper_cli.main`` against
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``chiper update`` so the managed copy stays current.
...
def _install_uv(target: Path) -> None:
    ...
    ``$CHIPER_HOME/bin/`` instead of ``~/.local/bin/``.
    """
