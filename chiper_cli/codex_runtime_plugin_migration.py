"""Migrate Chiper' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Chiper' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Chiper mcp_servers.<n>.command/args/env  → codex stdio transport
  Chiper mcp_servers.<n>.url/headers       → codex streamable_http transport
  Chiper mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Chiper mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Chiper-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Chiper' own
...
# Chiper keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Chiper' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, chiper_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Chiper MCP server config to the codex inline-table dict
    ...
    if not isinstance(chiper_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(chiper_cfg.get("command"))
    has_url = bool(chiper_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (CHIPER_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Chiper)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Chiper' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Chiper' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Chiper
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="chiper-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a CHIPER_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed chiper-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.chiperflux`` symlink that happens to live under ``/private/var/folders``
...
def _build_chiper_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Chiper' own
    ...
    so a chiper installed under /opt/, /usr/local/, or a venv all work.
    CHIPER_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # CHIPER_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_chiper_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever CHIPER_HOME its launcher sets
    ...
    # — that would override the launcher's CHIPER_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("CHIPER_HOME", tmp_path) would
    ...
    chiper_home = os.environ.get("CHIPER_HOME") or ""
    if chiper_home and _looks_like_test_tempdir(chiper_home):
        chiper_home = ""
    if chiper_home:
        env["CHIPER_HOME"] = chiper_home
    # PYTHONPATH passes through so a worktree-launched chiper finds the
    ...
    env["CHIPER_QUIET"] = "1"
    env["CHIPER_REDACT_SECRETS"] = env.get("CHIPER_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_chiper_tools: bool = True,
) -> MigrationReport:
    """Translate Chiper mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        chiper_config: full ~/.chiperflux/config.yaml dict
        ...
        expose_chiper_tools: when True (default), register Chiper' own
    ...
    chiper_servers = (chiper_config or {}).get("mcp_servers") or {}
    if not isinstance(chiper_servers, dict):
    ...
    for name, cfg in chiper_servers.items():
    ...
    # Inject Chiper' own tool surface as an MCP server so the spawned
    ...
    if expose_chiper_tools:
        translated["chiper-tools"] = _build_chiper_tools_mcp_entry()
        if "chiper-tools" not in report.migrated:
            report.migrated.append("chiper-tools")
