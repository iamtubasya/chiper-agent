...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Chiper'
...
The actual runtime resolution happens in chiper_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "chiper"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Chiper' MCP servers + Codex's installed curated
        ...
        try:
            from chiper_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the chiper-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "chiper-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Chiper tool callback are always-on production
            ...
            if "chiper-tools" in mig_report.migrated:
                msg_lines.append(
                    "Chiper tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Chiper runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Chiper tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Chiper runtime.")
