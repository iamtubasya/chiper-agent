...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("CHIPER_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `chiper model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("CHIPER_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("CHIPER_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from chiper_cli.setup import prompt_checklist
    except Exception:
        return set()
