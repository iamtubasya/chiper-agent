...
# xAI's public API is OpenAI-compatible for the endpoints Chiper commonly
# uses. The Responses endpoint is included because Chiper' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Chiper-managed OAuth credentials."""

    auth_hint = "chiper auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`chiper auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`chiper auth reset xai-oauth` or re-authenticate with "
                    "`chiper auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `chiper auth add xai-oauth --type oauth`."
            )
