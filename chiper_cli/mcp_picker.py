"""MCP picker — interactive `chiper mcp picker` (also the default `chiper mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``chiper mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `chiper plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from chiper_cli.cli_output import prompt_yes_no
from chiper_cli.curses_ui import curses_single_select
from chiper_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Chiper session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from chiper_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `chiper mcp catalog`."""
    if not rows:
        print()
    ...
    # Surface manifest-version warnings so users know when their Chiper is
...
def show_catalog() -> None:
    """`chiper mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`chiper mcp picker` (and default `chiper mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`chiper mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `chiper mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
