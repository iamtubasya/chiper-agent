...
from chiper_cli.auth import AuthError, resolve_provider
from chiper_cli.colors import Colors, color
from chiper_cli.config import get_env_path, get_env_value, get_chiper_home, load_config
from chiper_cli.models import provider_label
from chiper_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from chiper_cli.nous_subscription import get_nous_subscription_features
from chiper_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``chiper config``'s
...
def show_status(args):
    """Show status of all Chiper Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Chiper Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: chiper model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: chiper auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: chiper auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: chiper model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from chiper_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from chiper_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   chiper gateway")
    ...
    jobs_file = get_chiper_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_chiper_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'chiper doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'chiper setup' to configure", Colors.DIM))
    print()
