...
from chiper_constants import get_default_chiper_root, get_chiper_home, display_chiper_home
...
# ``chiper-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/chiper-agent/``
# are not accidentally excluded.
_EXCLUDED_DIRS = {
    "chiper-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``chiper-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/chiper-agent/`` — must be preserved.
        if part == "chiper-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside CHIPER_HOME.
...
def run_backup(args) -> None:
    ...
    # Collect files
    print(f"Scanning {display_chiper_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "chiper-agent" and not is_root)
        ]
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.chiperflux/config.yaml` instead of `config.yaml`.
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_chiper_home()}")
...
# — `chiper update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(chiper_home: Optional[Path] = None) -> Path:
    home = chiper_home or get_chiper_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    chiper_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(chiper_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside CHIPER_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``chiper update``.
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(chiper_home), keep=keep)
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_chiper_home()}/state-snapshots/")
...
def _pre_update_backup_dir(chiper_home: Optional[Path] = None) -> Path:
    home = chiper_home or get_chiper_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of CHIPER_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<CHIPER_HOME>/backups/pre-update-<timestamp>.zip`` and
