...
/// desktop app handing off via `Chiper-Setup.exe --update`.
...
export const $chiperHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, chiperHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_chiper_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $chiperHome.set(chiperHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Chiper" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Chiper-Setup.exe --update);
...
export async function launchChiperDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_chiper_desktop', { installRoot })
}
