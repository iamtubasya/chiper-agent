...
const CHIPER_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type ChiperRefType = (typeof CHIPER_REF_TYPES)[number]
...
const ICON_PATHS: Record<ChiperRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as ChiperRefType] ?? ICON_FALLBACK
...
const CHIPER_DIRECTIVE_RE = new RegExp(
...
export const chiperDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(CHIPER_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as ChiperRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: ChiperRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...chiperDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Chiper directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.chiperDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.chiperDesktop?.readFileDataUrl(id)
