...
import { formatRefValue, chiperDirectiveFormatter } from './directive-text'
...
describe('chiperDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = chiperDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = chiperDirectiveFormatter.parse('@file:src/main.tsx the entry point')
