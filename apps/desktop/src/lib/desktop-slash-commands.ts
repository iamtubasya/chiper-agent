...
/** Why a known Chiper command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Chiper profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Chiper Agent version', surface: exec() },
...
function isKnownChiperSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Chiper' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownChiperSlashCommand(normalized)
}
