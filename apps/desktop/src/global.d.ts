...
declare global {
  interface Window {
    chiperDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<ChiperConnection>
      ...
      profile: {
        ...
        // backend under the new CHIPER_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: ChiperApiRequest) => Promise<T>
      notify: (payload: ChiperNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<ChiperReadFileTextResult>
      selectPaths: (options?: ChiperSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<ChiperPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<ChiperPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: ChiperTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<ChiperReadDirResult>
      ...
      worktrees?: (cwds: string[]) => Promise<Record<string, ChiperWorktreeInfo | null>>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: ChiperTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<ChiperTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: ChiperWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: ChiperPreviewFileChanged) => void) => () => void
...
export interface ChiperTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface ChiperTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `chiper update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  chiperRoot?: string
}
...
export interface ChiperConnection {
  baseUrl: string
...
export interface ChiperTitleBarTheme {
  background: string
  foreground: string
}

export interface ChiperWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface ChiperApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface ChiperNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface ChiperPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface ChiperReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface ChiperPreviewWatch {
  id: string
  path: string
}

export interface ChiperWorktreeInfo {
...
export interface ChiperReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface ChiperReadDirResult {
  entries: ChiperReadDirEntry[]
  error?: string
}

export interface ChiperPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface ChiperSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
