...
const SKIN_KEY = 'chiper-desktop-theme-v2'
const MODE_KEY = 'chiper-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'chiper-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'chiper-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'chiper-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.chiperTheme = skinName
  root.dataset.chiperMode = rendered
  ...
  window.chiperDesktop?.setTitleBarTheme?.({
  ...
  try {
    window.localStorage.setItem('chiper-boot-background', c.background)
    window.localStorage.setItem('chiper-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.chiperThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.chiperDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Chiper backend theme on connect. */
