import type { ConnectionState, GatewayEvent } from '@chiper/shared'
...
const isOpen = (gateway: ChiperGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<ChiperGateway | null>(null)
...
let primaryGateway: ChiperGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: ChiperGateway | null, profile = 'default'): void {
...
export function activeGateway(): ChiperGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<ChiperGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
