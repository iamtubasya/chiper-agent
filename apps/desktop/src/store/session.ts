...
import type { ChiperConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistBoolean, persistString, storedBoolean, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/chiper'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'chiper.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'chiper.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'chiper.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'chiper.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'chiper.desktop.composer.fast'

let configuredDefaultProjectDir = ''

function workspaceCwdKey(connection: ChiperConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.chiperDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.chiperDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<ChiperConnection | null>(null)
...
export const setConnection = (next: Updater<ChiperConnection | null>) => updateAtom($connection, next)
