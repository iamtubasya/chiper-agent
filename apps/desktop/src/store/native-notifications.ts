...
const STORAGE_KEY = 'chiper:native-notifications'
...
// "Backgrounded" = the user isn't on Chiper. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.chiperDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.chiperDesktop
