...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/chiper'
...
const CONFIGURED_CACHE_KEY = 'chiper-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'chiper-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Chiper is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the Nous Tool Gateway,
// mirroring chiper_cli/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
async function fetchProviderDefaultModel(
  ...
  // `chiper model` does (for Nous it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Chiper still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Chiper still cannot resolve a usable provider.'
}
...
async function openSignInUrl(url: string) {
  if (window.chiperDesktop?.openExternal) {
    try {
      await window.chiperDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Chiper still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Chiper still cannot reach ${url}.` }
    }
