...
const remoteConn = (over: Partial<ChiperConnection> = {}): ChiperConnection =>
  ({ baseUrl: 'https://chiper-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as ChiperConnection

const localConn = (over: Partial<ChiperConnection> = {}): ChiperConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as ChiperConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<ChiperConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { chiperDesktop: { getConnection } })
})
