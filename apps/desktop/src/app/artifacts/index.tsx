...
import { getSessionMessages, listAllProfileSessions } from '@/chiper'
...
import type { SessionInfo, SessionMessage } from '@/types/chiper'
  ...
  const openArtifact = useCallback(async (href: string) => {
    try {
      if (window.chiperDesktop?.openExternal) {
        await window.chiperDesktop.openExternal(href)
      } else {
