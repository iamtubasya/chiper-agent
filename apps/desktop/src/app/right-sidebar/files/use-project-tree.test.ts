...
import type { ChiperReadDirResult } from '@/global'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<ChiperReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { chiperDesktop: { readDir: typeof readDir } }).chiperDesktop = { readDir }
})
...
afterEach(() => {
  ...
  $connection.set(null)
  resetProjectTreeState()
  delete (window as unknown as { chiperDesktop?: unknown }).chiperDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): ChiperReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { chiperDesktop: unknown }).chiperDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: ChiperReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ChiperReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { chiperDesktop: unknown }).chiperDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.chiperDesktop is missing', async () => {
    delete (window as unknown as { chiperDesktop?: unknown }).chiperDesktop
