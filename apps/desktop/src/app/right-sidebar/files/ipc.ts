...
import type { ChiperReadDirEntry, ChiperReadDirResult } from '@/global'

export type ProjectTreeEntry = ChiperReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: ChiperReadDirEntry) {
...
async function filterIgnored(entries: ChiperReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ChiperReadDirResult> {
  if (!window.chiperDesktop) {
