...
import { ChiperGateway } from '@/chiper'
...
import type { RpcEvent } from '@/types/chiper'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.chiperDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ChiperGateway | null) => void
  refreshChiperConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: ChiperConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Chiper...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Chiper…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Chiper gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshChiperConfig().catch(() => undefined)
    ...
    const gateway = new ChiperGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshChiperConfig()
