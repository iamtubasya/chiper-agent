...
// bug that drives the REAL useGatewayBoot hook + REAL ChiperGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshChiperConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { chiperDesktop?: unknown }).chiperDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { chiperDesktop?: unknown }).chiperDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForChiper) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startChiper()'s remote branch awaits waitForChiper() for 45s before it
    ...
    ;(window as { chiperDesktop?: unknown }).chiperDesktop = desktop
    ...
    // After ~45s waitForChiper gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Chiper backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
