...
import { getProfiles, transcribeAudio } from '@/chiper'
...
async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.chiperDesktop?.readFileDataUrl
...
export function usePromptActions({
  ...
  // separate `chiper gateway` process performs the actual transfer, so we
