        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.chiperDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Chiper to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.chiperDesktop?.watchPreviewFile ||
      !window.chiperDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.chiperDesktop.onPreviewFileChanged(payload => {
    ...
    void window.chiperDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.chiperDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.chiperDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:chiper-preview')
