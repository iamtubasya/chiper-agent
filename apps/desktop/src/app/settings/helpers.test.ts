...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: ChiperConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: ChiperConfigRecord = {}
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // CHIPER_QWEN_ and CHIPER_GEMINI_ both share the CHIPER_ stem.
      expect(providerGroup('CHIPER_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
      expect(providerGroup('CHIPER_GEMINI_CLIENT_ID')).toBe('Gemini')
    })
