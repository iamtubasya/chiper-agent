...
import {
  getElevenLabsVoices,
  getChiperConfigDefaults,
  getChiperConfigRecord,
  getChiperConfigSchema,
  saveChiperConfig
} from '@/chiper'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, ChiperConfigRecord } from '@/types/chiper'
...
export function ConfigSettings({
  ...
  const [_defaults, setDefaults] = useState<ChiperConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getChiperConfigRecord(), getChiperConfigDefaults(), getChiperConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: ChiperConfigRecord) => {
