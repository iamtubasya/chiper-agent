...
import {
  ...
  getChiperConfigRecord,
  getRecommendedDefaultModel,
  saveChiperConfig,
  setEnvVar,
  setModelAssignment
} from '@/chiper'
import type { AuxiliaryModelsResponse, ModelOptionProvider, StaleAuxAssignment } from '@/chiper'
import { useI18n } from '@/i18n'
import { AlertTriangle, Cpu, Loader2 } from '@/lib/icons'
import { cn } from '@/lib/utils'
import { notifyError } from '@/store/notifications'
import { startManualLocalEndpoint, startManualProviderOAuth } from '@/store/onboarding'
import type { ChiperConfigRecord } from '@/types/chiper'

import { CONTROL_TEXT } from './constants'
import { getNested, setNested } from './helpers'
import { ListRow, LoadingState, Pill, SectionHeading } from './primitives'

// Chiper' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Chiper default (medium), shown as Medium.
...
// backend now surfaces the full `chiper model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in chiper_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const [config, setConfig] = useState<ChiperConfigRecord | null>(null)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `chiper model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Chiper runs the flow for you.`}
          </p>
        )}
