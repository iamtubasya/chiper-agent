...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../chiper'
...
import { useChiperConfig } from './session/hooks/use-chiper-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.chiperDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.chiperDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.chiperDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.chiperDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // chiper:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.chiperDesktop?.onDeepLink?.(payload => {
    ...
    void window.chiperDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.chiperDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshChiperConfig, sttEnabled, voiceMaxRecordingSeconds } = useChiperConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshChiperConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshChiperConfig])
