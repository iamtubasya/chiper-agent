...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'chiper:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.chiperDesktop?.saveImageFromUrl) {
        if (await window.chiperDesktop.saveImageFromUrl(src)) {
