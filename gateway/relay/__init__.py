"""Relay/connector support package for the Chiper gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``chiper gateway enroll`` writes them to
    ``~/.chiperflux/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``CHIPER_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def self_provision_if_managed() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.chiperflux/.env``
    ...
    try:
        from chiper_cli.auth import resolve_nous_access_token
