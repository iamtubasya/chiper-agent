...
The PID file lives at ``{CHIPER_HOME}/gateway.pid``.  CHIPER_HOME defaults to
``~/.chiperflux`` but can be overridden via the environment variable.  This means
separate CHIPER_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting CHIPER_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("CHIPER_GATEWAY_LOCK_DIR")
    if override:
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different CHIPER_HOME dirs).
    """
