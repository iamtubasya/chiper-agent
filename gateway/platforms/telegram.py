...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = float(os.getenv("CHIPER_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "CHIPER_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "CHIPER_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Chiper-created private-chat topic lanes are marked with
    ...
    # older PTB/clients. Streaming edits stay on Chiper' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        Chiper must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _try_edit_rich(
        ...
        try:
            ...
            await self._bot.edit_message_text(**payload)
        except Exception as exc:
            if self._is_rich_fallback_error(exc):
                if self._is_rich_capability_error(exc):
                    self._rich_send_disabled = True
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `chiper update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Chiper or OpenClaw instance is running "
            "with this token, then restart the gateway with 'chiper gateway restart'."
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # readable by the Chiper process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("CHIPER_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("CHIPER_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("CHIPER_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("CHIPER_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("CHIPER_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }

            disable_fallback = (os.getenv("CHIPER_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            try:
                ...
                from chiper_cli.commands import telegram_menu_commands
    ...
    def _build_provider_keyboard(self, providers: list):
        ...
        CLI ``chiper model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from chiper_cli.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from chiper_cli.models import PROVIDER_GROUPS
    ...
    # Scripts live in ~/.chiperflux/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".chiperflux" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@chiper_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Chiper bot profiles. A message like
