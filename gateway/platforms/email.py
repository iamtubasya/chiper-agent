"""
Email platform adapter for the Chiper gateway.

Allows users to interact with Chiper by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "chiper-agent" "version" "{_chiper_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "ChiperFlux Agent")
        ...
        msg_id = f"<chiper-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
