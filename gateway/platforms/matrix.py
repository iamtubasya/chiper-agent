...
# Uses get_chiper_home() so each profile gets its own Matrix store.
from chiper_constants import get_chiper_dir as _get_chiper_dir

_STORE_DIR = _get_chiper_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("CHIPER_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("CHIPER_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "ChiperFlux Agent" become "Agent".
        """
        if not body:
            return ""
