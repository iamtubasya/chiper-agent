...
There are two ways to add a platform to the Chiper gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.chiperflux/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Chiper code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `chiper gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `chiper_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Chiper core.
...
And add it to the `chiper-gateway` composite:

```python
"chiper-gateway": {
    "includes": [..., "chiper-your-platform"]
}
```
...
## 12. Status Display (`chiper_cli/status.py`)
...
## 13. Gateway Setup Wizard (`chiper_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ chiper_cli/ toolsets.py \
  --include="*.py" -l | sort -u
