...
chiper meet install                 # pip deps + Chromium (transcribe only)
chiper meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
chiper meet auth                    # optional; skips guest-lobby wait
chiper meet setup                   # preflight checks
```

`chiper meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or CHIPER_MEET_REALTIME_KEY in ~/.chiperflux/.env
```
...
pip install playwright websockets && python -m playwright install chromium
chiper plugins enable google_meet
chiper meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
chiper meet node approve my-mac ws://<mac-ip>:18789 <token>
chiper meet node ping my-mac                   # confirm reachable
```

Run `chiper meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Chiper agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `chiper meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `CHIPER_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `chiper_meet_src`). |
...
Local:
```
$CHIPER_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$CHIPER_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `chiper meet node approve`.
