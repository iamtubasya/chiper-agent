...
Let the chiper agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where chiper runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `chiper meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `chiper meet` CLI |
...
| `cli.py` | `chiper meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$CHIPER_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `chiper meet node {run,list,approve,remove,status,ping}` |
...
chiper meet install                                      # pip + Chromium
chiper meet setup                                        # preflight
chiper meet auth                                         # optional
chiper meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
chiper meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.chiperflux/.env
chiper meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
chiper meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
chiper meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.chiperflux/.env
chiper meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, chiper will **not** switch your system audio input automatically — the
...
chiper plugins enable google_meet
chiper meet node run --display-name my-mac --host 0.0.0.0 --port 18789
