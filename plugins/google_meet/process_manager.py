...
session-scoped state file under ``$CHIPER_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $CHIPER_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_chiper_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    guest_name: str = "ChiperFlux Agent",
    ...
    If a bot is already running for this chiper install, leave it first —
    ...
    env = os.environ.copy()
    env["CHIPER_MEET_URL"] = url
    env["CHIPER_MEET_OUT_DIR"] = str(out)
    env["CHIPER_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["CHIPER_MEET_HEADED"] = "1"
    if auth_state:
        env["CHIPER_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["CHIPER_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if CHIPER_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["CHIPER_MEET_MODE"] = mode
    if realtime_model:
        env["CHIPER_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["CHIPER_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["CHIPER_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["CHIPER_MEET_REALTIME_KEY"] = realtime_api_key
