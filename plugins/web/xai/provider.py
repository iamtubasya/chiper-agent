...
``{title, url, description, position}`` rows every other Chiper web
provider produces.
...
prefers Chiper-managed xAI Grok OAuth (via ``chiper auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.chiperflux/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``chiper tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `chiper auth` to sign in with "
