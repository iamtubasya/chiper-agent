...
Config (env vars or chiper config.yaml under retaindb:):
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        if explicit:
            ...
            chiper_home = str(kwargs.get("chiper_home", ""))
            profile_name = os.path.basename(chiper_home) if chiper_home else ""
            project = f"chiper-{profile_name}" if (profile_name and profile_name not in {"", ".chiperflux"}) else "default"
