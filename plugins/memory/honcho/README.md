...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/chiper>
...
```bash
chiper memory setup honcho   # configure Honcho directly (works on a fresh install)
chiper memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
chiper config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.chiperflux/.env
```

> `chiper honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `chiper memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$CHIPER_HOME/honcho.json` | Profile-local (isolated Chiper instances) |
| 2 | `~/.chiperflux/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Chiper profile: `chiper` (default) or `chiper_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `chiper honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Chiper session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`chiper-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`chiper`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-chiper-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening chiper in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Chiper session ID (timestamp + hex). Every `chiper` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Chiper profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`chiper`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Chiper profile: `chiper` (default) or `chiper_<profile>` (e.g. `chiper -p coder` -> host key `chiper_coder`). Older `chiper.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `CHIPER_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `chiper memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `chiper honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `chiper memory setup`) |
| `chiper honcho status` | Show resolved config for active profile |
| `chiper honcho enable` / `disable` | Toggle Honcho for active profile |
| `chiper honcho mode <mode>` | Change recall or observation mode |
| `chiper honcho peer --user <name>` | Update user peer name |
| `chiper honcho peer --ai <name>` | Update AI peer name |
| `chiper honcho tokens --context <N>` | Set context token budget |
| `chiper honcho tokens --dialectic <N>` | Set dialectic max chars |
| `chiper honcho map <name>` | Map current directory to a session name |
| `chiper honcho sync` | Create host blocks for all Chiper profiles |
