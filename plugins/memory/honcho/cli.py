...
Handles: chiper honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "chiper" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Chiper profiles and creates host blocks for any that don't
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Chiper profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $CHIPER_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_chiper_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        chiper_host.get("pinUserPeer"),
        chiper_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in chiper_host:
        aliases_src = chiper_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in chiper_host:
        prefix_src = chiper_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(chiper_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom chiper_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    chiper_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        chiper_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(chiper_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = chiper_host.get("userPeerAliases")
    prefix = chiper_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(chiper_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(chiper_host)
    chiper_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        chiper_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        chiper_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Chiper persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = chiper_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            chiper_host["apiKey"] = new_local_key
    ...
    # --- 3. Identity ---
    current_peer = chiper_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        chiper_host["peerName"] = new_peer

    current_ai = chiper_host.get("aiPeer") or cfg.get("aiPeer", "chiper")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        chiper_host["aiPeer"] = new_ai

    current_workspace = chiper_host.get("workspace") or cfg.get("workspace", "chiper")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        chiper_host["workspace"] = new_workspace
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, chiper_host)
    ...
    if run_mapping:
        peer_target = chiper_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(chiper_host)
            chiper_host["pinUserPeer"] = False
            if prior_aliases:
                chiper_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                chiper_host, current_prefix, prefix_from_root,
            ...
            if merged:
                chiper_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                chiper_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                chiper_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = chiper_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        chiper_host["observationMode"] = new_obs
    else:
        chiper_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(chiper_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        chiper_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        chiper_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = chiper_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        chiper_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = chiper_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        chiper_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                chiper_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(chiper_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            chiper_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        chiper_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        chiper_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        chiper_host["dialecticReasoningLevel"] = new_reasoning
    else:
        chiper_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = chiper_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        chiper_host["sessionStrategy"] = new_strat

    chiper_host["enabled"] = True
    chiper_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        chiper_config = load_config()
        chiper_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(chiper_config)
    ...
    print("\n  Other commands:")
    print("    chiper honcho status     -- show full config")
    print("    chiper honcho mode       -- change recall/observation mode")
    print("    chiper honcho tokens     -- tune context and dialectic budgets")
    print("    chiper honcho peer       -- update peer names")
    print("    chiper honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Chiper profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: chiper honcho setup\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: chiper honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        chiper = hosts.get(_host_key(), {})
        user = chiper.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = chiper.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = chiper.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = chiper.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Chiper' identity in Honcho. Seed with 'chiper honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: chiper honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: chiper honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = chiper.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = chiper.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = chiper.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Chiper asks Honcho's AI peer a question")
        ...
        print("\n  Set with: chiper honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    chiper honcho identity --show        — show both peer representations")
        print("    chiper honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Chiper + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Chiper\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Chiper persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  chiper honcho setup")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  chiper honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'chiper'")
        print("  with Honcho configured and no prior session history.")
        print("  (Chiper calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    chiper honcho migrate  — this step handles it interactively")
        if has_key:
    ...
    print()
    print("  In Chiper, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            for f in agent_files:
                print(f"    chiper honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  chiper honcho identity <path/to/SOUL.md>")
    ...
    print("    Chiper:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Chiper:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Chiper:   Honcho observes every message and updates representations")
    ...
    print("    Chiper:   per-session by default — each run gets its own session")
    print("              Map a custom name:  chiper honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. chiper honcho setup              — configure API key (required)")
        print("  2. chiper honcho migrate            — re-run this walkthrough")
    else:
        print("  1. chiper honcho status             — verify Honcho connection")
        print("  2. chiper                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. chiper honcho identity --show    — verify AI peer representation")
        print("  4. chiper honcho tokens             — tune context and dialectic budgets")
        print("  5. chiper honcho mode               — view or change memory mode")
    print()
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to chiper memory setup)",
    )
