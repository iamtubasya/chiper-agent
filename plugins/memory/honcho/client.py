...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Chiper profile."""
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $CHIPER_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.chiperflux/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_chiper_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_chiper_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Chiper system prompt
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'chiper honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `chiper honcho setup` to configure)."
            )
