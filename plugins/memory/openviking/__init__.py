...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Chiper peer ID in OpenViking (default: chiper)
...
_DEFAULT_AGENT = "chiper"
_AGENT_PROMPT_LABEL = "Chiper peer ID in OpenViking"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Chiper slash-skill scaffolding before sending content to OpenViking.
...
def _load_chiper_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("CHIPER_HOME", "")).expanduser() if os.environ.get("CHIPER_HOME") else Path.home() / ".chiperflux"
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Chiper session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Chiper only", "write values only to Chiper .env"),
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See chiper-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_chiper_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Chiper peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, chiper_home: str, config: dict) -> None:
        ...
        from chiper_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        chiper_home_path = Path(chiper_home)
        env_path = chiper_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_chiper_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Chiper run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See chiper-agent#28296.
    ...
    def _tool_read(self, args: dict) -> str:
        ...
        # (e.g. viking://user/chiper/.overview.md).
