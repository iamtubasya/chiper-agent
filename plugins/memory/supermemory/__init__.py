...
def _load_supermemory_config(chiper_home: str) -> dict:
    config = _default_config()
    config_path = Path(chiper_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, chiper_home: str) -> None:
    config_path = Path(chiper_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "chiper"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "chiper", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, chiper_home):
        ...
        _save_supermemory_config(sanitized, chiper_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from chiper_constants import get_chiper_home
        self._chiper_home = kwargs.get("chiper_home") or str(get_chiper_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._chiper_home)
