...
Config file: `$CHIPER_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `chiper` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `chiper-{identity}` → `chiper-coder`). |
...
All Supermemory API calls send `x-sm-source: chiper`, and document writes stamp
`metadata.sm_source: chiper`. This is a **functional routing key, not telemetry**:
it groups Chiper-written memories into a dedicated "Chiper" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Chiper profile:

```json
{
  "container_tag": "chiper-{identity}"
}
```

For a profile named `coder`, this resolves to `chiper-coder`. The default profile resolves to `chiper-default`. Without `{identity}`, all profiles share the same container.
