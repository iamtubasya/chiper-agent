...
Optional environment variables:
    ...
    CHIPER_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``chiper setup`` can describe it even when websockets is
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("CHIPER_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``chiper setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.chiperflux/.env`` via
    ``chiper_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "chiper_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.chiperflux/.env"
        )
        return
