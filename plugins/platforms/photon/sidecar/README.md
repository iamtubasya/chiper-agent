...
Small Node helper that bridges Chiper Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Chiper is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Chiper-Sidecar-Token`)
...
The Chiper plugin's `chiper photon setup` command runs `npm install`
here automatically.
