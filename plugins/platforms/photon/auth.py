...
Credential storage mirrors every other Chiper channel:
    ...
    * management metadata -> ``~/.chiperflux/auth.json`` under
...
def _auth_json_path() -> Path:
    ...
    try:
        ...
        return Path(os.path.expanduser("~/.chiperflux")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.chiperflux/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.chiperflux/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.chiperflux/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: chiper_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `chiper photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `chiper photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `chiper photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `chiper photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `chiper photon setup`)"
