...
Rules:
  ...
  - empty dirs    → always delete (under CHIPER_HOME)
...
try:
    ...
    def get_chiper_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("CHIPER_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".chiperflux").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$CHIPER_HOME/logs/``."""
    return get_chiper_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$CHIPER_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    ...
    try:
        path.resolve().relative_to(chiper_home)
        return True
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "chiper-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $CHIPER_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so CHIPER_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        chiper_home = get_chiper_home()
        for parent in ("cron", "cronjobs"):
            base = chiper_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside CHIPER_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    try:
        for top in chiper_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    chiper_home = get_chiper_home()
    try:
        rel = path.resolve().relative_to(chiper_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "chiper-agent",
        }:
            return None
