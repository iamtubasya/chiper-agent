...
Auto-tracks and cleans up ephemeral files created during Chiper Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$CHIPER_HOME` and `/tmp/chiper-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CHIPER_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `CHIPER_HOME` or `/tmp/chiper-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$CHIPER_HOME/disk-cleanup/` is itself excluded
- `$CHIPER_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
