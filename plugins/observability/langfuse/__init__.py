...
Optional env vars:
  CHIPER_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  CHIPER_LANGFUSE_RELEASE     - release/version tag
  CHIPER_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  CHIPER_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  CHIPER_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("CHIPER_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    public_key = _env("CHIPER_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("CHIPER_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("CHIPER_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("CHIPER_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset CHIPER_LANGFUSE_PUBLIC_KEY / CHIPER_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("CHIPER_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("CHIPER_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("CHIPER_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("CHIPER_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid CHIPER_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("CHIPER_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
