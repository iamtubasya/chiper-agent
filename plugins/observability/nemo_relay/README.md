...
```bash
env CHIPER_HOME=/tmp/hermes-nemo-relay-test \
  hermes plugins enable observability/nemo_relay
```
...
`CHIPER_HOME` is the Hermes profile/config home used by both
`hermes plugins enable ...` and the later `hermes chat ...` run. If unset,
Hermes uses the user's default home, usually `~/.chiperflux`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export CHIPER_HOME=/tmp/hermes-nemo-relay-test
hermes plugins enable observability/nemo_relay
...
`CHIPER_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Hermes does not
...
export CHIPER_HOME=/tmp/hermes-nemo-relay-docs/hermes-home
mkdir -p "$CHIPER_HOME"
...
```bash
export CHIPER_NEMO_RELAY_ATOF_ENABLED=1
export CHIPER_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/hermes-nemo-relay-docs/subagent/atof
export CHIPER_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export CHIPER_NEMO_RELAY_ATOF_MODE=overwrite
export CHIPER_NEMO_RELAY_ATIF_ENABLED=1
export CHIPER_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/hermes-nemo-relay-docs/subagent/atif
export CHIPER_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export CHIPER_NEMO_RELAY_ATIF_AGENT_NAME='Hermes Agent E2E'
...
export CHIPER_NEMO_RELAY_ATOF_ENABLED=1
export CHIPER_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/hermes-nemo-relay-docs/parallel/atof
export CHIPER_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export CHIPER_NEMO_RELAY_ATOF_MODE=overwrite
export CHIPER_NEMO_RELAY_ATIF_ENABLED=1
export CHIPER_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/hermes-nemo-relay-docs/parallel/atif
...
```bash
export CHIPER_NEMO_RELAY_PLUGINS_TOML=/tmp/hermes-middleware-test/plugins.toml
```
...
```bash
export CHIPER_HOME=/tmp/hermes-middleware-test/hermes-home
mkdir -p "$CHIPER_HOME" /tmp/hermes-middleware-test/nemo-relay
...
export CHIPER_NEMO_RELAY_PLUGINS_TOML=/tmp/hermes-middleware-test/nemo-relay/plugins.toml
