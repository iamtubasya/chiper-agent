...
Optional Chiper observability plugin that maps Chiper observer hooks to
...
not replace Chiper Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Chiper emit NeMo Relay lifecycle events for
the work Chiper already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Chiper Agent can:

- Preserve Chiper execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
chiper plugins enable observability/nemo_relay
```

The `CHIPER_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `CHIPER_HOME` that the
agent run will use:

```bash
env CHIPER_HOME=/tmp/chiper-nemo-relay-test \
  chiper plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`CHIPER_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`CHIPER_HOME` is the Chiper profile/config home used by both
`chiper plugins enable ...` and the later `chiper chat ...` run. If unset,
Chiper uses the user's default home, usually `~/.chiperflux`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export CHIPER_HOME=/tmp/chiper-nemo-relay-test
chiper plugins enable observability/nemo_relay
chiper chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `chiper` command you run is built from the
...
uv run chiper plugins enable observability/nemo_relay
uv run chiper chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/chiper_agent-*.whl
...
The plugin can configure exporters directly from `CHIPER_NEMO_RELAY_*`
...
```bash
export CHIPER_NEMO_RELAY_ATOF_ENABLED=1
export CHIPER_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export CHIPER_NEMO_RELAY_ATIF_ENABLED=1
export CHIPER_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `CHIPER_NEMO_RELAY_ATOF_FILENAME`
- `CHIPER_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `CHIPER_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `CHIPER_NEMO_RELAY_ATIF_AGENT_NAME`
- `CHIPER_NEMO_RELAY_ATIF_AGENT_VERSION`
- `CHIPER_NEMO_RELAY_ATIF_MODEL_NAME`
- `CHIPER_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Chiper at it:

```bash
export CHIPER_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Chiper Agent"
agent_version = "local"
```

When `CHIPER_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`CHIPER_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`CHIPER_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Chiper does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Chiper keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Chiper routes LLM and tool
...
For the full generic Chiper middleware contract, see
...
export CHIPER_HOME=/tmp/chiper-nemo-relay-docs/chiper-home
mkdir -p "$CHIPER_HOME"

cat > "$CHIPER_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Chiper session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export CHIPER_NEMO_RELAY_ATOF_ENABLED=1
export CHIPER_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/chiper-nemo-relay-docs/subagent/atof
export CHIPER_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export CHIPER_NEMO_RELAY_ATOF_MODE=overwrite
export CHIPER_NEMO_RELAY_ATIF_ENABLED=1
export CHIPER_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/chiper-nemo-relay-docs/subagent/atif
export CHIPER_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export CHIPER_NEMO_RELAY_ATIF_AGENT_NAME='Chiper Agent E2E'
export CHIPER_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export CHIPER_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"chiper.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Chiper Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Chiper dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/chiper-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/chiper-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/chiper-nemo-relay-docs/workdir/beta.txt
cd /tmp/chiper-nemo-relay-docs/workdir

export CHIPER_NEMO_RELAY_ATOF_ENABLED=1
export CHIPER_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/chiper-nemo-relay-docs/parallel/atof
export CHIPER_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export CHIPER_NEMO_RELAY_ATOF_MODE=overwrite
export CHIPER_NEMO_RELAY_ATIF_ENABLED=1
export CHIPER_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/chiper-nemo-relay-docs/parallel/atif
export CHIPER_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export CHIPER_NEMO_RELAY_ATIF_AGENT_NAME='Chiper Agent E2E'
export CHIPER_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Chiper sessions map to `agent` scopes.
- Chiper API request hooks map to `llm` scope start/end events.
- Chiper tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Chiper execution middleware to hand
...
Enable it for Chiper:

```bash
export CHIPER_NEMO_RELAY_PLUGINS_TOML=/tmp/chiper-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Chiper routes execution
through these boundaries:

```text
Chiper provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Chiper provider adapter next_call(...)

Chiper tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Chiper tool dispatcher next_call(...)
```
...
middleware for a local Chiper run. This path requires a NeMo Relay runtime that
...
```bash
export CHIPER_HOME=/tmp/chiper-middleware-test/chiper-home
mkdir -p "$CHIPER_HOME" /tmp/chiper-middleware-test/nemo-relay
...
cat > /tmp/chiper-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/chiper-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/chiper-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Chiper Middleware E2E"
agent_version = "local"
...
export CHIPER_NEMO_RELAY_PLUGINS_TOML=/tmp/chiper-middleware-test/nemo-relay/plugins.toml
