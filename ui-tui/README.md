...
Interpreter resolution order is: `CHIPER_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.chiperflux/.chiper_history` or under `CHIPER_HOME`.
