import type { RunExternalProcess } from '@chiper/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`chiper ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching chiper: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`chiper ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
