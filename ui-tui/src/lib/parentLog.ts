...
const logDir = join(process.env.CHIPER_HOME?.trim() || join(homedir(), '.chiper'), 'logs')
...
// a real ~/.chiperflux (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('chiper-tui: parent lifecycle log unavailable\n')
    }
  }
}
