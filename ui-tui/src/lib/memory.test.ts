...
const ENV_KEYS = ['CHIPER_AUTO_HEAPDUMP', 'CHIPER_HEAPDUMP_DIR', 'CHIPER_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  it('writes diagnostics only for auto-high without CHIPER_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without CHIPER_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when CHIPER_AUTO_HEAPDUMP=1', async () => {
    process.env.CHIPER_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.CHIPER_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of CHIPER_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  afterEach(() => {
    if (savedDir === undefined) {delete process.env.CHIPER_HEAPDUMP_DIR}
    else {process.env.CHIPER_HEAPDUMP_DIR = savedDir}

    if (savedMax === undefined) {delete process.env.CHIPER_HEAPDUMP_MAX_BYTES}
    else {process.env.CHIPER_HEAPDUMP_MAX_BYTES = savedMax}
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.CHIPER_HEAPDUMP_MAX_BYTES = String(2 * 1024)
