...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because CHIPER_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.CHIPER_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.chiper', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `chiper-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.CHIPER_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.CHIPER_HEAPDUMP_MAX_BYTES?.trim()
