...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.CHIPER_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Chiper on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via CHIPER_TUI_TRUECOLOR=1.
