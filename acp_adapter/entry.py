"""CLI entry point for the chiper-agent ACP adapter.

Loads environment variables from ``~/.chiperflux/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from CHIPER_HOME (default ``~/.chiperflux``)."""
    from chiper_cli.env_loader import load_chiper_dotenv

    chiper_home = get_chiper_home()
    loaded = load_chiper_dotenv(chiper_home=chiper_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", chiper_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.chiperflux/node/ "
...
def _run_setup() -> None:
    from chiper_cli.main import main as chiper_main
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting chiper-agent ACP adapter")
