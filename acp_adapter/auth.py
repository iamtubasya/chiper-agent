"""ACP auth helpers — detect and advertise Chiper authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "chiper-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Chiper runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Chiper can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Chiper.
    ...
    not have Chiper provider credentials configured yet, so Chiper always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Chiper using the currently configured "
