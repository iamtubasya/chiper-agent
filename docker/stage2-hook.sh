...
#
# Wired into the image as /etc/cont-init.d/01-chiper-setup by the
...
CHIPER_HOME="${CHIPER_HOME:-/opt/data}"
INSTALL_DIR="/opt/chiper"

# Drop to chiper via s6-setuidgid, but skip it when already non-root.
as_chiper() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid chiper "$@"; }
...
# starts non-root. The baked install tree under /opt/chiper is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $CHIPER_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass CHIPER_UID/CHIPER_GID — or the PUID/PGID aliases — which the
...
# `id -u chiper` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# chiper UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u chiper)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-chiper UID).
...
and the baked /opt/chiper install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e CHIPER_UID=\$(id -u) -e CHIPER_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the chiper user to that UID/GID at boot and chowns the data
...
# --- Bootstrap CHIPER_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid chiper mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom CHIPER_HOME paths whose parents only
# root can create (e.g. `CHIPER_HOME=/home/chiper/.chiper` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for CHIPER_UID/CHIPER_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  CHIPER_UID/
# CHIPER_GID still win when both are set.  See #15290, salvages #25872.
CHIPER_UID="${CHIPER_UID:-${PUID:-}}"
CHIPER_GID="${CHIPER_GID:-${PGID:-}}"

if [ -n "${CHIPER_UID:-}" ] && validate_uid_gid "$CHIPER_UID" && [ "$CHIPER_UID" != "$(id -u chiper)" ]; then
    echo "[stage2] Changing chiper UID to $CHIPER_UID"
    usermod -u "$CHIPER_UID" chiper
fi
if [ -n "${CHIPER_GID:-}" ] && validate_uid_gid "$CHIPER_GID" && [ "$CHIPER_GID" != "$(id -g chiper)" ]; then
    echo "[stage2] Changing chiper GID to $CHIPER_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$CHIPER_GID" chiper 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised chiper user (UID 10000)
...
# chiper process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes chiper. Idempotent across container
...
#   - chiper is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G chiper 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] chiper already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" chiper 2>/dev/null; then
        echo "[stage2] Added chiper to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group chiper failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When CHIPER_UID is remapped or the top-level $CHIPER_HOME isn't owned by
# the runtime chiper UID, restore ownership to chiper — but ONLY for the
# directories chiper actually writes to. The full $CHIPER_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of chiper-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_chiper_uid=$(id -u chiper)
needs_chown=false
if [ "$(stat -c %u "$CHIPER_HOME" 2>/dev/null)" != "$actual_chiper_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $CHIPER_HOME (targeted) to chiper ($actual_chiper_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $CHIPER_HOME: chown the directory itself (not its contents)
    # so chiper can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown chiper:chiper "$CHIPER_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $CHIPER_HOME failed (rootless container?) — continuing"
    # Chiper-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by chiper (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$CHIPER_HOME/$sub" ]; then
            chown -R chiper:chiper "$CHIPER_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $CHIPER_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
fi
...
# chiper user. Hosted/container instances keep mutable state under
# $CHIPER_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# CHIPER_DISABLE_LAZY_INSTALLS=1. Keeping /opt/chiper root-owned and
...
# Always reset ownership of $CHIPER_HOME/profiles to chiper on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> chiper …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as chiper and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$CHIPER_HOME/profiles" ]; then
    chown -R chiper:chiper "$CHIPER_HOME/profiles" 2>/dev/null || true
fi

# Always reset ownership of $CHIPER_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged chiper runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$CHIPER_HOME/cron" ]; then
    chown -R chiper:chiper "$CHIPER_HOME/cron" 2>/dev/null || true
fi

# Reset ownership of chiper-owned top-level state files on every boot.
# The targeted data-volume chown above only covers chiper-owned
# *subdirectories*; loose state files living directly under $CHIPER_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> chiper …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged chiper runtime then hits
...
# sweep so host-owned files in a bind-mounted $CHIPER_HOME are never
...
# entries of chiper_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$CHIPER_HOME/$f" ]; then
        chown chiper:chiper "$CHIPER_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the chiper runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$CHIPER_HOME/config.yaml" ]; then
    chown chiper:chiper "$CHIPER_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$CHIPER_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as chiper user ---
# Run as chiper via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $CHIPER_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_chiper mkdir -p \
    "$CHIPER_HOME/cron" \
    "$CHIPER_HOME/sessions" \
    "$CHIPER_HOME/logs" \
    "$CHIPER_HOME/logs/gateways" \
    "$CHIPER_HOME/hooks" \
    "$CHIPER_HOME/memories" \
    "$CHIPER_HOME/skills" \
    "$CHIPER_HOME/skins" \
    "$CHIPER_HOME/plans" \
    "$CHIPER_HOME/workspace" \
    "$CHIPER_HOME/home" \
    "$CHIPER_HOME/pairing" \
    "$CHIPER_HOME/platforms/pairing"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/chiper/.install_method (see Dockerfile), NOT written here into
# $CHIPER_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $CHIPER_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.chiperflux:/opt/data) and sometimes shared with a
...
# 'chiper update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $CHIPER_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/chiper stamp, so deleting the data-dir copy is safe).
if [ -f "$CHIPER_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$CHIPER_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$CHIPER_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$CHIPER_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_chiper cp "$INSTALL_DIR/$src" "$CHIPER_HOME/$dest"
    fi
}
...
if [ -f "$CHIPER_HOME/.env" ]; then
    chown chiper:chiper "$CHIPER_HOME/.env" 2>/dev/null || true
    chmod 600 "$CHIPER_HOME/.env" 2>/dev/null || true
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $CHIPER_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `chiper update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set CHIPER_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$CHIPER_HOME/config.yaml" ]; then
    s6-setuidgid chiper "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$CHIPER_HOME/auth.json" ] && [ -n "${CHIPER_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$CHIPER_AUTH_JSON_BOOTSTRAP" > "$CHIPER_HOME/auth.json"
    chown chiper:chiper "$CHIPER_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$CHIPER_HOME/auth.json"
fi
...
# can set CHIPER_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# CHIPER_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$CHIPER_HOME/gateway_state.json" ] && \
        [ "${CHIPER_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$CHIPER_HOME/gateway_state.json"
    chown chiper:chiper "$CHIPER_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$CHIPER_HOME/gateway_state.json"
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_chiper "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/chiper/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Chiper spawns for the
...
# main-wrapper.sh propagates it into the supervised ``chiper`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-chiper, dashboard, etc.).
