...
# root can create (e.g. `CHIPER_HOME=/home/hermes/.hermes` in a Compose
...
if [ -n "${CHIPER_UID:-}" ] && validate_uid_gid "$CHIPER_UID" && [ "$CHIPER_UID" != "$(id -u hermes)" ]; then
    echo "[stage2] Changing hermes UID to $CHIPER_UID"
    usermod -u "$CHIPER_UID" hermes
fi
if [ -n "${CHIPER_GID:-}" ] && validate_uid_gid "$CHIPER_GID" && [ "$CHIPER_GID" != "$(id -g hermes)" ]; then
    echo "[stage2] Changing hermes GID to $CHIPER_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$CHIPER_GID" hermes 2>/dev/null || true
fi
...
# directories hermes actually writes to. The full $CHIPER_HOME may be a
...
needs_chown=false
if [ "$(stat -c %u "$CHIPER_HOME" 2>/dev/null)" != "$actual_hermes_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $CHIPER_HOME (targeted) to hermes ($actual_hermes_uid)"
    ...
    chown hermes:hermes "$CHIPER_HOME" 2>/dev/null || \
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$CHIPER_HOME/$sub" ]; then
            chown -R hermes:hermes "$CHIPER_HOME/$sub" 2>/dev/null || \
...
# CHIPER_DISABLE_LAZY_INSTALLS=1. Keeping /opt/hermes root-owned and
...
# Always reset ownership of $CHIPER_HOME/profiles to hermes on every
...
if [ -d "$CHIPER_HOME/profiles" ]; then
    chown -R hermes:hermes "$CHIPER_HOME/profiles" 2>/dev/null || true
fi
...
if [ -d "$CHIPER_HOME/cron" ]; then
    chown -R hermes:hermes "$CHIPER_HOME/cron" 2>/dev/null || true
fi
...
for f in \
    ...
    if [ -e "$CHIPER_HOME/$f" ]; then
        chown hermes:hermes "$CHIPER_HOME/$f" 2>/dev/null || true
    fi
done
...
if [ -f "$CHIPER_HOME/config.yaml" ]; then
    chown hermes:hermes "$CHIPER_HOME/config.yaml" 2>/dev/null || true
...
seed_one() {
    ...
    if [ ! -f "$CHIPER_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_hermes cp "$INSTALL_DIR/$src" "$CHIPER_HOME/$dest"
    fi
}
...
if [ -f "$CHIPER_HOME/.env" ]; then
    chown hermes:hermes "$CHIPER_HOME/.env" 2>/dev/null || true
...
if [ ! -f "$CHIPER_HOME/auth.json" ] && [ -n "${CHIPER_AUTH_JSON_BOOTSTRAP:-}" ]; then
    ...
    chown hermes:hermes "$CHIPER_HOME/auth.json" 2>/dev/null || true
...
if [ ! -f "$CHIPER_HOME/gateway_state.json" ] && \
    ...
    chown hermes:hermes "$CHIPER_HOME/gateway_state.json" 2>/dev/null || true
