...
# by /etc/cont-init.d/01-chiper-setup (installed by the Dockerfile). This
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs chiper"
...
echo "[chiper] WARNING: docker/entrypoint.sh is a deprecated shim under " \
...
exec /opt/chiper/docker/stage2-hook.sh "$@"
